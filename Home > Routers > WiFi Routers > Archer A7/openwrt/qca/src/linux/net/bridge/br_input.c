...
forward:
	...
	case BR_STATE_LEARNING:
...
#ifdef CONFIG_ATHRS17_HNAT_WIFI
		if (!skb->is_hnat_frag && !skb->is_wifi_redirect && (p->state == BR_STATE_FORWARDING) && !skb->is_from_eth && (skb->pkt_type == PACKET_HOST) 
		...
		if (!skb->is_hnat_frag && !skb->is_wifi_redirect && (p->state == BR_STATE_FORWARDING) && skb->is_from_eth && (skb->pkt_type == PACKET_OTHERHOST)
			&& (skb->protocol == htons(ETH_P_IP)) && (p->dev->priv_flags & IFF_HNAT_WIFI_REDIRECT_PORT)) {
