...
packet_routed:
	...
	*((__be16 *)iph) = htons((4 << 12) | (5 << 8) | (inet->tos & 0xff));
	...
	skb->priority = sk->sk_priority;
//	skb->mark = sk->sk_mark;
	/* Forwarding SKBs from PPP interface to ETH interface,(l2tp)
	 * Lose their marks in this section, but htb's fw filter checks those marks.
	 */
	switch(sk->sk_protocol){
	case IPPROTO_L2TP:
		break;
	case IPPROTO_UDP:
		if(udp_sk(sk)->encap_type == UDP_ENCAP_L2TPINUDP)
			break;
	default:
		skb->mark = sk->sk_mark;
		break;
