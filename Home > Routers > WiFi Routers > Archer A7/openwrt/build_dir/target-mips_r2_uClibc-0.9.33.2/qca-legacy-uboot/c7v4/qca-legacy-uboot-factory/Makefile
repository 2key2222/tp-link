...
LIBS += nvram_mngr/libnvrammngr.a
endif

ifeq ($(WEBFAILSAFE_SERVER),1)
...
board953x_config		:	unconfig
	...
	@echo '#define CONFIG_PRODUCT_${PRODUCT_NAME}		1'	>>include/config.h
...
ifeq ($(TP_FIRMWARE_RECOVERY),1)
	@echo '#define TP_FIRMWARE_RECOVERY 1' >>include/config.h
	@echo "#define FIRMWARE_RECOVERY_NAME "\"$(strip ${FIRMWARE_RECOVERY_NAME})\" >>include/config.h
	@echo "#define FIRMWARE_RECOVERY_IP_ADDR "\"$(strip ${FIRMWARE_RECOVERY_IP_ADDR})\" >>include/config.h
	@echo "#define FIRMWARE_RECOVERY_SERVER_IP "\"$(strip ${FIRMWARE_RECOVERY_SERVER_IP})\" >>include/config.h
endif
ifdef CONFIG_UBOOT_MALLOC_LEN
	@echo "#define CONFIG_UBOOT_MALLOC_LEN "\(${CONFIG_UBOOT_MALLOC_LEN}\) >>include/config.h
endif
ifdef FIRMWARE_RECOVERY_GPIO_SCHEME
	@echo "#define FIRMWARE_RECOVERY_GPIO_SCHEME "\"$(strip ${FIRMWARE_RECOVERY_GPIO_SCHEME})\" >>include/config.h
endif
...
ifneq ($(NM_PTN_TABLE_BASE),)
	@echo "#define NM_PTN_TABLE_BASE "$(strip ${NM_PTN_TABLE_BASE}) >>include/config.h
endif
ifneq ($(NM_FLASH_SIZE),)
	@echo "#define NM_FLASH_SIZE "$(strip ${NM_FLASH_SIZE}) >>include/config.h
endif #}
ifneq ($(NM_PTN_NUM_MAX),)
	@echo "#define NM_PTN_NUM_MAX "$(strip ${NM_PTN_NUM_MAX}) >>include/config.h
endif
ifdef CFG_DOUBLE_BOOT_SECOND
	@echo '#define CFG_DOUBLE_BOOT_SECOND 1' >>include/config.h
endif
ifdef CFG_DOUBLE_BOOT_FACTORY
	@echo '#define CFG_DOUBLE_BOOT_FACTORY 1' >>include/config.h
ifeq ($(WEBFAILSAFE_SERVER),1)
	@echo '#define CONFIG_CMD_HTTPD 1' >>include/config.h
	@echo "#define WEBFAILSAFE_SERVER_IP_ADDR "\"$(strip ${WEBFAILSAFE_SERVER_IP_ADDR})\" >>include/config.h
endif
endif
