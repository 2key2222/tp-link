 ...
 const std::vector<std::string>& keys)
{
  ...
  if(group->getDownloadContext()->hasAttribute(CTX_ATTR_BT)) {
	...
	entryDict->put("btFile", group->getOption()->get(PREF_TORRENT_FILE).c_str());
  }
#endif // ENABLE_BITTORRENT
}
} // namespace
...
void changeGlobalOption(const Option& option, DownloadEngine* e)
{
  ...
  if(option.defined(PREF_ARIA2C_PRIORITY)) {
    setpriority(PRIO_PROCESS, 0, e->getOption()->getAsInt(PREF_ARIA2C_PRIORITY));
  }
