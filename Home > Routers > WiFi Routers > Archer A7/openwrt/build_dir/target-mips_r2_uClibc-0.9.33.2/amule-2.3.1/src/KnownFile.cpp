...
{
	wxCHECK_RET(Length, wxT("No input to hash from in CreateHashFromFile"));
	wxASSERT_MSG(Output || pShaHashOut, wxT("Nothing to do in CreateHashFromFile"));
	...
	MD4 md4_hasher_tmp;
	MD4Context hdc;
	md4_hasher_tmp.MD4Init(&hdc);
#endif //ENABLE_TOMCRYPT

	uint64	tmpOffset = offset;
	uint32	tmpLength = 0;
	uint32	leftlenth = Length;
	uint32	posCurrentEMBlock = 0;
	
	CScopedPtr<CAICHHashAlgo> pHashAlg(CAICHHashSet::GetNewHashAlgo());

//		printf("input %ld-", offset);
//		printf("%u-", Length);
//		printf("%u-\n", leftlenth);

	while (leftlenth > 0){

		if (leftlenth / EMBLOCKSIZE){
			tmpLength = EMBLOCKSIZE;
		}
		else if (leftlenth % EMBLOCKSIZE){
			tmpLength = leftlenth % EMBLOCKSIZE;
		}
		
//			printf("solve %ld-", tmpOffset);
//			printf("%u-", tmpLength);
//			printf("%u-\n", leftlenth);
		area.ReadAt(file, tmpOffset, tmpLength);
		CMemFile data(area.GetBuffer(), tmpLength);
		
		uint32 Required = tmpLength;
		uint32	nIACHPos = 0;
		byte   X[64*128];
		...
		if (Output != NULL){
#ifndef ENABLE_TOMCRYPT
			md4_hasher.Update(area.GetBuffer(), tmpLength);
#else
			md4_hasher_tmp.MD4Update(&hdc, area.GetBuffer(), tmpLength);
#endif //ENABLE_TOMCRYPT
	...
	if (Output != NULL){
#ifndef ENABLE_TOMCRYPT
		md4_hasher.Final(Output->GetHash());
//			printf("hash1 : ");
//			for(int j=0; j < MD4HASH_LENGTH; j++)
//				printf("%x", *(Output->GetHash()+j));
//			printf("\n");
#else
		md4_hasher_tmp.MD4Final(&hdc, Output->GetHash());
//			printf("hash2 : "); 
//			for(int j=0; j < MD4HASH_LENGTH; j++)
//				printf("%x", *(Output->GetHash()+j));
//			printf("\n");
...
void CKnownFile::CreateHashFromInput(const byte* input, uint32 Length, CMD4Hash* Output, CAICHHashTree* pShaHashOut )
{
	...
	if (Output != NULL){
		...
		MD4Context hdc;
		md4_hasher_tmp.MD4Init(&hdc);
		md4_hasher_tmp.MD4Update(&hdc, input, Length);
		md4_hasher_tmp.MD4Final(&hdc, Output->GetHash());
#endif //ENABLE_TOMCRYPT
