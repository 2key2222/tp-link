...
//					theStats::AddUpOverheadFileRequest(replypacket->GetPacketSize());
			...
			CMemFile data_out(128);
			data_out.WriteHash(reqfile->GetFileHash());
			AddDebugLogLineN( logRemoteClient, CFormat(wxT("Remote Client: OP_MULTIPACKET file name %s")) % reqfile->GetFileName());			
...
#else
					if ((theApp->uploadqueue->GetWaitingList().size() + 50) > thePrefs::GetQueueSize()) {
#endif
