...
//only called when shared file list CheckAICHHashes
	...
	if ((m_toHash & EH_AICH) && !TestDestroy()) {
		...
		if (AICHHashSet->VerifyHashTree(true) ) {
			AddDebugLogLineN( logHasher,
				CFormat(wxT("AICH VerifyHashTree ok for file: %s"))
					% m_filename );
...
void CCompletionTask::Entry()
{
	...
	if (!CPath::RenameFile(partfilename, newName)) {
		if (!CPath::CloneFile(partfilename, newName, true)) {
			//zengwei mod below
			//m_error = true;
			AddLogLineC(CFormat(_("error: can't clone partfile '%s' to new file renamed '%s'.")) % partfilename % newName.GetFullName());
			//return;
			//do not return if clone partfilename to newName just try another way
			dstName = CPath(wxT("Unknown"));		
			newName = targetPath.JoinPaths(dstName);
			for (unsigned count = 0; newName.FileExists(); ++count) {
				wxString postfix = CFormat(wxT("(%u)")) % count;
			...
			if (!CPath::CloneFile(partfilename, newName, true)) {
				m_error = true;
				AddLogLineC(CFormat(_("error: can't clone partfile '%s' to new file renamed '%s' another try.")) % partfilename % newName.GetFullName());
				return;
			}
			//zengwei mod above
		}
...
#if 0 //zengwei mod optimize
#  define BLOCK_SIZE	1048576		/* Write 1 MB blocks */
#else
#  define BLOCK_SIZE	102400		/* Write 100 KB blocks */
#endif //zengwei mod optimize
