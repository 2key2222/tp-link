...
#ifndef __WXMSW__ //zengwei add
#ifdef ENABLE_UBUS_RPC
void CDownloadQueue::AddCompletedHash(CMD4Hash hash)
{
	wxMutexLocker lock( m_mutex );
	m_completedHashList.push_back(hash);
}

void CDownloadQueue::RemoveCompletedHash(CMD4Hash hash)
{
	wxMutexLocker lock( m_mutex );

	for (MD4HashList::iterator it = m_completedHashList.begin(); it != m_completedHashList.end(); it++) {
		CMD4Hash tmp = *it;
		if (tmp == hash) {
			m_completedHashList.erase(it);
			break;
		}
	}
}

void CDownloadQueue::RemoveCompletedHashAll()
{
	wxMutexLocker lock( m_mutex );
...
bool CDownloadQueue::IsHashExistCompletedList(const CMD4Hash &hash)
{
	wxMutexLocker lock( m_mutex );
...
bool CDownloadQueue::GetCompletedHashByIndex(unsigned int index, CMD4Hash *h)
{
	wxMutexLocker lock( m_mutex );
	
	if ( index < m_completedHashList.size() ) {
		*h = m_completedHashList[index];
		return true;
	}
...
CPartFile* CDownloadQueue::GetDownloadFileByID(const CMD4Hash& filehash) const
{
	wxMutexLocker lock( m_mutex );
...
CPartFile* CDownloadQueue::RemoveCompletedByID(const CMD4Hash& filehash)
{
	// get part file in completed list
	for (FileList::iterator it = m_completedDownloads.begin(); it != m_completedDownloads.end(); ++it) {
        CPartFile * file = *it;
		if ( filehash == file->GetFileHash()) {
...
void CDownloadQueue::ProcessLocalRequests()
{
	...
	if ( (!m_localServerReqQueue.empty()) && (m_dwNextTCPSrcReq < ::GetTickCount()) ) {
		...
		const int iMaxFilesPerTcpFrame = 8;
		int iFiles = 0;
...
void CDownloadQueue::CheckDiskspace( const CPath& path )
{
	...
	if (free == static_cast<uint64>(wxInvalidOffset)) {
		...
		AddLogLineN(wxT("Disk space is low level."));		
	}
...
#if 1 //zengwei add restrict downfile number
	if (GetFileCount() >= thePrefs::GetMaxDownloadFileNum() && thePrefs::GetMaxDownloadFileNum() > 0){
		AddLogLineC(CFormat(_("Add download file number is up to max(%d).")) 
                        % thePrefs::GetMaxDownloadFileNum());
		return false;			
	}
#endif //zengwei add restrict downfile number
