...
public:
	...
	bool VerifyHashPartTree(CAICHHashAlgo* hashalg, bool bDeleteBadTrees);
	CAICHHashTree* FindHash(uint64 nStartPos, uint64 nSize)
	{
		uint8 buffer = 0;
		return FindHash(nStartPos, nSize, &buffer);
	}
	bool FindLevel(uint64 nFileSize, uint64 nStartPos, uint64 nSize, uint8* nLevel);
...
protected:
	...
	bool SetPartHash(CFileDataIO* fileInput, uint32 wHashIdent, sint8 nLevel, uint8 nLevelFlag);
};
...
public:
	...
	bool VerifyHashPartTree(bool bDeleteBadTrees);
	...
	void SetIsLeftBranchTree(bool val) 	{ m_pHashTree.m_bIsLeftBranch = val; }
