...
public:
	 ...
	 * set log level debug
	 */
	void SetLogDebug(bool value) {m_LogSetDebug = value;}

	//zengwei add 
	/**
	 * get log level debug
	 */
	bool GetLogSetDebug() const {return m_LogSetDebug;}

	//zengwei add 
	/**
	 * Is Logfile opend
	 */
	bool IsLogfileOpened() const {return applog != NULL;}
	...
	/**
	 * Check status of Logfile zengwei add
	 */
	void CheckLogFileStatus(const wxString& logfile);
	...
	/**
	 * File status Event handler zengwei add
	 */
	void OnLogFileStatusEvent(class CLogFileStatusEvent& evt);
	...
	CLogger() {
		...
		m_LogSetDebug = false; //zengwei add log level set
	}
...
private:
	...
	bool m_LogSetDebug;		//zengwei add log level set
...
//zengwie add
DECLARE_LOCAL_EVENT_TYPE(MULE_EVT_LOGFILESTATUS, -1)

/** This event is sent when a log-file size up to max. */
class CLogFileStatusEvent : public wxEvent
{
public:
	CLogFileStatusEvent(const wxString &logfile) 
		: wxEvent(-1, MULE_EVT_LOGFILESTATUS)
		// Deep copy, to avoid thread-unsafe reference counting. */
		, m_logfile(logfile.c_str(), logfile.Length()) 
	{}

	const wxString& GetLogFile() const {
		return m_logfile;
	}

	wxEvent* Clone() const {
		return new CLogFileStatusEvent(m_logfile);
	}
...
typedef void (wxEvtHandler::*MuleLogFileStatusEventFunction)(CLogFileStatusEvent&); //zengwei add
...
#define EVT_MULE_LOGGING(func) \
	...
	//zengwei add ! Event-handler for log file size up to max 
#define EVT_MULE_LOGFILESTATUS(func) \
		DECLARE_EVENT_TABLE_ENTRY(MULE_EVT_LOGFILESTATUS, -1, -1, \
		(wxObjectEventFunction) (wxEventFunction) \
		wxStaticCastEvent(MuleLogFileStatusEventFunction, &func), (wxObject*) NULL),
