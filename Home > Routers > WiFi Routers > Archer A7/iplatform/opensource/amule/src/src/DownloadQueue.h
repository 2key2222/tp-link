...
public:
	...
	/**
	 * Add a completed file hash to list.
	 */
	void	AddCompletedHash(CMD4Hash hash);

	/**
	 * Remove a completed file hash from list.
	 */
	void	RemoveCompletedHash(CMD4Hash hash);

	/**
	 * Remove all completed file hash from list.
	 */
	void	RemoveCompletedHashAll();

	/**
	 * Is a file hash in completed hash list.
	 */
	bool	IsHashExistCompletedList(const CMD4Hash &hash);


	/**
	 * Get a file hash in completed hash list by index.
	 */
	bool 	GetCompletedHashByIndex(unsigned int index, CMD4Hash *h);

	/**
	 * Get a file hash in completed hash list by index.
	 */
	uint16 	GetCompletedHashCount() { wxMutexLocker lock( m_mutex ); return m_completedHashList.size();}

	/**
	 * Returns a pointer to the file in download partfile list 
	 * with the specified hash, or NULL.
	 * 
	 * @param filehash The hash to search for.
	 * @return The corresponding download file or NULL.
	 */
	CPartFile* GetDownloadFileByID(const CMD4Hash& filehash) const;

	/**
	 * remove the file in completed partfile list  with the specified hash.
	 * 
	 * @param filehash The hash to search for.
	 * @return The corresponding download file or NULL.
	 */
	CPartFile* RemoveCompletedByID(const CMD4Hash& filehash);
...
private:
	...
	//zengwei add for completed hash
#ifndef __WXMSW__ 
#ifdef ENABLE_UBUS_RPC
	typedef std::deque<CMD4Hash> MD4HashList;
	MD4HashList		m_completedHashList;
#endif
#endif
