...
void uh_check_gzip_file_exist(char *path)
{
 struct stat stat1;
 char compress_path[128];
 char* sub;
 
  if (strncmp(path,"/www/webpages/",14)!=0)
  	return;
  ...
  sub = strstr(compress_path,"webpages");
  if (!sub)
  	return;

  strncpy(sub,"compress",8);// LEO_TODO
  strcat(compress_path,".gz");
  printf("[LEO]now the compress path:%s\n",compress_path);
  
  if(!stat(compress_path,&stat1))
  	strcpy(path,compress_path);
...
struct path_info * uh_path_lookup(struct client *cl, const char *url)
{
	...
	printf("[LEO]path_phys:%s\n",path_phys);

	/*for IE6,IE7, we prefer not to use gzip function ,as they are buggy */
	char * userAgent = uh_file_header_lookup(cl,"User-Agent");   
	if (userAgent !=NULL && !strstr(userAgent,"MSIE 6.0") && !strstr(userAgent,"MSIE 7.0"))
	{
	 /* actually this step is not nessesary,as all browser support gzip encoding */
	 char * acceptEncoding = uh_file_header_lookup(cl, "Accept-Encoding");
	 if (acceptEncoding!=NULL && strstr(acceptEncoding,"gzip")!=NULL)
	    uh_check_gzip_file_exist(path_phys);
	}
