...
static const char * uh_file_mime_lookup_for_compressed(const char *path)
{
	struct mimetype *m = &uh_mime_types[0];
	const char *e;
	const char*locate_gzip;

	locate_gzip = strstr(path,".gz");
    if (locate_gzip == NULL)
    {
     printf("[LEO]unknown mime type!!!\n");
	...
	while (m->extn)
	{
		...
		while (e >= path)
		{
			if ((*e == '.' || *e == '/') && !strncasecmp(&e[1], m->extn,locate_gzip-e-1))
				return m->mime;
...
bool uh_file_request(struct client *cl, struct path_info *pi)
{
	...
	{
		printf("[LEO]uh_file_request:pi:%s\n",pi->phys);
		...
		if (ok > 0)
		{
			...
			//if the file is "*.gz" format
			if (strncmp((char *)(pi->phys+strlen(pi->phys)-3),".gz",3)== 0)
			    gzip_flag = 1;
			
            if (gzip_flag == 1)
			{
            	ensure_out(uh_http_sendf(cl, NULL, "Content-Encoding: %s\r\n",
									 "gzip")); 
		
				ensure_out(uh_http_sendf(cl, NULL, "Content-Type: %s\r\n",
									 uh_file_mime_lookup_for_compressed(pi->name)));	
			}
			else
			{ 
			    ensure_out(uh_http_sendf(cl, NULL, "Content-Type: %s\r\n",
