...
GetImageMetadata(const char *path, char *name)
{
	...
	image_type_t image_type = IMAGE_TYPE_UNKOWN;
	mime_type_t *mime_type  = NULL;
...
no_exifdata:
	...
	if (strcmp(image_type->mime, "image/jpeg") == 0)
	{
...
error:
	...
	else if (strcmp(mime_type->mime, "image/tif") == 0)
	{
		TIFF *tif = NULL;
...
GetImageMetadata(const char *path, char *name)
{
	...
	{
		m.dlna_pn = strdup(mime_type->ext);
	}
...
GetVideoMetadata(const char *path, char *name)
{
	...
	ret = sql_exec(db, "INSERT into DETAILS"
	                   ...
	                   "VALUES"
	                   " (%Q, %lld, %ld, %Q, %Q, %Q, %Q, %Q, %Q, '%q', %Q, %Q, %Q, %Q, %Q, '%q');",
	                   path, (long long)file.st_size, file.st_mtime, m.duration,
	                   m.date, m.channels, m.bitrate, m.frequency, m.resolution,
                       m.title, m.creator, m.artist, m.genre, m.comment, m.dlna_pn, m.mime);
	if( ret != SQLITE_OK )
	{
		fprintf(stderr, "Error inserting details for '%s'!\n", path);
		ret = 0;
