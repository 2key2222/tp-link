...
void reply_close(struct smb_request *req)
{
	...
	if(fsp->is_directory) {
		...
		/* edit by chenqingpeng, 2015-08-14, sync to disk right after closing file */
		if(fsp->modified)
		{	
			syncFlag = 1;
		}
		...
		/* edit by chenqingpeng, 2015-08-14, sync to disk right after closing file */
		if((fsp->can_read && fsp->can_write)		/* create */
				||(fsp->modified)				/* modify */
				||(fsp->delete_on_close))		/* delete */
		{
			syncFlag = 1;
		}
		status = close_file(req, fsp, NORMAL_CLOSE);
			      ...
			      const struct smb_filename *smb_fname_dst)
{
	...
	if (!did_rename) {
		DEBUG(10, ("rename_open_files: no open files on file_id %s "
			   ...
			   smb_fname_str_dbg(smb_fname_dst)));
		/* add by chenqingpeng, 2015-08-14, to sync to disk when changed file/dir's name */
		sync_disk();
	}
