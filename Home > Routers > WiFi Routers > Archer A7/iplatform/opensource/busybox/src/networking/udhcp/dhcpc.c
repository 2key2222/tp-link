...
#if ENABLE_FEATURE_SMART_DHCP
#include <sys/socket.h>
#include <sys/ioctl.h>
#include <linux/if.h>
#include <linux/wireless.h>
#include "wlan_status.h"
#endif
...
enum {
	...
	OPT_B = 1 << 20,
    #if ENABLE_FEATURE_SMART_DHCP
    OPT_d = 1 << 21,  /*! -d, the dhcpc run only for detect dhcp-server */
	OPT_m = 1 << 22,
	OPTBIT_B = 22,
    #else
...
#define RELEASED        6
#if ENABLE_FEATURE_SMART_DHCP
/* after get IP,  state:BOUND-->DHCPS_DETECTING, detect dhcps timely */
#define DHCPS_DETECTING 7
#endif
...
#if ENABLE_FEATURE_SMART_DHCP
static int dev_status(const char* dev_name)
{
    int skfd = 0;
    struct ifreq ifr;
...
static int vap_status(const char* dev_name)
{
    struct iwreq iwr; 
    struct ieee80211_wdsconfig config;
    static int skfd = 0;
    ...
    memset(&config, 0, sizeof(struct ieee80211_wdsconfig));
    config.cmdtype = IEEE80211_WLANCONFIG_WDS_STATE_GET;
    strncpy(iwr.ifr_ifrn.ifrn_name, dev_name, sizeof(iwr.ifr_ifrn.ifrn_name));
	
    iwr.u.data.pointer = (void *) &config;    
    iwr.u.data.length = sizeof(config);
    if (ioctl(skfd, IEEE80211_IOCTL_CONFIG_GENERIC, &iwr) < 0)
    {
        return 0;
    }
    return config.data.wds_state;
}

static int sta_connect_check(int mode_mask)
{
    int band2g_connect = 0;
    int band5g_connect = 0;

    //udhcpc restart only mode change, we can't get sta name if wireless parameter changed.
	//so use mode to get sta name, maybe modify in future.
    if(1 == mode_mask)//ap mode
    {
        return dev_status("eth0");
    }
    else if(2 == mode_mask)//repeater mode
    {
        band2g_connect = (0 == dev_status("ath06"))? 0 : ((IEEE80211_S_RUN != vap_status("ath06")) ? 0 : 1);
        band5g_connect = (0 == dev_status("ath16"))? 0 : ((IEEE80211_S_RUN != vap_status("ath16")) ? 0 : 1);

        return (1 == band2g_connect) || (1 == band5g_connect);
    }
    else if(3 == mode_mask)//client mode
    {
        band2g_connect = (0 == dev_status("ath07"))? 0 : ((IEEE80211_S_RUN != vap_status("ath07")) ? 0 : 1);
        band5g_connect = (0 == dev_status("ath17"))? 0 : ((IEEE80211_S_RUN != vap_status("ath17")) ? 0 : 1);

        return (1 == band2g_connect) || (1 == band5g_connect);
    }
    else//unknow mode, should not happen
        return 0;
}
#endif
...
int udhcpc_main(int argc UNUSED_PARAM, char **argv)
{
    ...
    #if ENABLE_FEATURE_SMART_DHCP
    int dhcps_detecting_type = 0; 
    /* 0: non-detecting, 1: only detect, do not set dhcpip, 2: detect and set dhcpip, 3: detect and set dhcpip but not enter detecting state*/
    int bound_timeout = 0;         /* time to renew dhcpip after enter BOUND */
    int dhcps_detecting_time = 0; /* time that have spend in dhcps detectting*/
    int dhcps_detecting_pktnum = 0; /* packet counts that send for dhcps detect*/
    const int dhcps_detecting_resttime = 30;
    int mode_mask = 0;
    enum{
        DHCPS_UNKNOW = 0,
        DHCPS_NOT_FOUND,
    ...
    int dhcps_found = DHCPS_UNKNOW;/* status indicates whether connected with dhcps*/
    #endif
...
#if ENABLE_FEATURE_SMART_DHCP
    opt_complementary = "O::x::T+:t+:A+:d+:m+"
#else
    ...
    #if ENABLE_FEATURE_SMART_DHCP
	opt = getopt32(argv, "CV:H:h:F:i:np:qRr:s:T:t:SA:O:ox:fBd:m:"
    #else
		...
		if (retval == 0) {
			...
			case INIT_SELECTING:
				if (!discover_retries || packet_num < discover_retries) {
					...
					/* broadcast */
#if ENABLE_FEATURE_SMART_DHCP
                    int i = sta_connect_check(mode_mask);
                    if(dhcps_detecting_type > 0 && i == 1)
                    {
					...
					packet_num++;
                   #if ENABLE_FEATURE_SMART_DHCP
                   if(dhcps_detecting_type > 0 && 0 == ((packet_num)%10))
                   {
                        if(dhcps_found != DHCPS_NOT_FOUND)
                        {
                            log1("[SMART_DHCPS] DHCP server not found");
                            udhcp_run_script(NULL, "dhcps_not_found");
                            dhcps_found = DHCPS_NOT_FOUND;
                        }
                        if(1 == dhcps_detecting_type || 3 == dhcps_detecting_type)
                        {
                            timeout = dhcps_detecting_resttime; /* wait 30s to redetect dhcps */
                            packet_num = 0;
                        }
                    }
                   #endif
 ...
 leasefail:
			...
			case REQUESTING:
				...
				goto leasefail;
            #if ENABLE_FEATURE_SMART_DHCP
            case DHCPS_DETECTING:
               dhcps_detecting_time += timeout;
               if(dhcps_detecting_time < bound_timeout) 
               {
                   if(20 > dhcps_detecting_pktnum)
                   {
                        timeout = 3;
                        dhcps_detecting_pktnum += 1;
                        send_renew(xid, server_addr, requested_ip);              
                    }
                   /* There is no dhcps, enter INIT_SELECTING */
                   else /* if(10 <= dhcps_detecting_pktnum) */
                   {
                        timeout = 0;
                        dhcps_detecting_pktnum = 0;  /* start another detecting */
                        log1("[SMART_DHCPS] Renew fail, Entering rebind state");
                        state = REBINDING;
                   }
                   continue;
               }
               /* It's time to renew dhcpip, just switch to state RENEWING */
               timeout = bound_timeout; /* recover timeout value for init renew process */
            #endif
			case BOUND:
               #if ENABLE_FEATURE_SMART_DHCP
               /* if not time to renew enter state DHCPS_DETECTING */
               if(2 == dhcps_detecting_type && dhcps_detecting_time < bound_timeout)
               {
                   log1("[SMART_DHCPS] Entering detecting state");
                   state = DHCPS_DETECTING;
                   change_listen_mode(LISTEN_KERNEL);
                   timeout = 0;
                   continue;
               }
...
//FIXME: need to also check that last 10 bytes are zero
		...
		/* TODO: why we don't just fetch server's IP from IP header? */
                ...
                #if ENABLE_FEATURE_SMART_DHCP
                if (1 == dhcps_detecting_type)
                {
                    timeout = dhcps_detecting_resttime; /* redetect dhcps after 30s */
                    packet_num = 0;
                    if(dhcps_found != DHCPS_FOUND)
                    {
                        log1("[SMART_DHCPS] There is dhcp server.");
				        udhcp_run_script(NULL, "dhcps_found");
                        dhcps_found = DHCPS_FOUND;
                    }
                    continue;
                }
                #endif
        ...
        #if ENABLE_FEATURE_SMART_DHCP
        case DHCPS_DETECTING:
            /* There is dhcps, after 30 seconds, start another detecting
             * If enter DHCPS_DETECTING state, that means dhcpc passs BOUND state
             * do not need to inform to config the network
             */
            log1("[SMART_DHCPS] There is dhcp server");
            dhcps_detecting_time += timeout;
            if(dhcps_detecting_time >= bound_timeout)
            {
                /* switch to RENEW state directly */
                timeout = 0;
            }
            else
            {
                int bound_time_left = bound_timeout - dhcps_detecting_time;
                timeout = (dhcps_detecting_resttime < bound_time_left ? dhcps_detecting_resttime : bound_time_left);
            }
            dhcps_detecting_pktnum = 0;            
			continue;
        #endif
		case REQUESTING:
		case RENEWING:
 ...
 * the configuration process..." */
                ...
                #if ENABLE_FEATURE_SMART_DHCP
                if(2 == dhcps_detecting_type)
                {
                    /* after timeout seconds start to detect dhcps again, 
                     * after bound_timeout enter actually renew state
                     */
                    bound_timeout = lease_seconds / 2; 
                    timeout = (dhcps_detecting_resttime<bound_timeout)?dhcps_detecting_resttime:bound_timeout;  // use some MACRO would better
                    dhcps_detecting_time = 0;
                    dhcps_detecting_pktnum = 0;
                    dhcps_found = DHCPS_FOUND;
                    log1("[SMART_DHCPS] There is dhcp server and get dhcp ip success");
                }
                if(3 == dhcps_detecting_type)
                {
                    dhcps_found = DHCPS_FOUND;
                    log1("[SMART_DHCPS] There is dhcp server and get dhcp ip success");                    
                }
