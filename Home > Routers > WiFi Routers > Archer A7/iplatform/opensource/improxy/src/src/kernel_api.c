...
STATUS k_mcast_block_source(pi_addr *p_mcastgrp, pi_addr *p_origin, char* ifname)
{
    ...
    {
        for (i = 0; i < MAX_UP_INF_NUM; i++) {
            req.gsr_interface = get_up_if_index(i);
            setsockopt(socket, imp_family_to_level(p_mcastgrp->ss.ss_family), MCAST_BLOCK_SOURCE,
                    &req, sizeof(struct group_source_req));
        }
    }
    return STATUS_OK;
...
STATUS k_mcast_msfilter(pi_addr* p_addr, pa_list *p_addr_list, int fmode)
{
    ...
    /* Caution: function get_up_if_index is redefined now */
