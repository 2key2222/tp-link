...
static const char *emmc_reset_gpio="GPIO110";
static const char *sys_led_gpio="GPIO21";
static const char *reset_gpio="GPIO24";
static const char *wd_enable_gpio="GPIO23";
static const char *wd_feed_gpio="GPIO25";
...
{
	return board_gpio_out(sys_led_gpio, onoff);
}
...
int board_reset_gpio_get(void)
{
	...
	ret = gpio_lookup_name(reset_gpio, NULL, NULL, &gpio);
	if (ret) {
		printf("GPIO: '%s' not found\n", reset_gpio);
		return -1;
	}
...
{
	/* output gpio high to disable emmc reset pin */
	board_gpio_out(emmc_reset_gpio, 1);

	/* disable hardware watchdog */
	board_gpio_out(wd_enable_gpio, 1);
	board_gpio_out(wd_feed_gpio, 1);
