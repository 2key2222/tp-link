...
#ifdef CONFIG_DKMGT
	if (dkmgt_get_mac(macstring, 17+1) < 0 )
	{
		printf("Failed to get factory mac\n");
	}
	else
	{
		int i;
		char tmp[3];
		char *pMac = macstring; /* XX-XX-XX-XX-XX-XX */

		printf("Factory mac is : %s\n", macstring);
		for(i = 0; i < 6; i++)
		{
			tmp[0] = pMac[(i * 3) + 0];
			tmp[1] = pMac[(i * 3) + 1];
			tmp[2] = '\0';
			mac[i] = (uchar) (simple_strtoul(tmp, NULL, 16));
		}

		eth_setenv_enetaddr("ethaddr", (const uchar *)mac);
	}
#endif
}
