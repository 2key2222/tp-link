                     ...
                     rtk_uint32 data)
{
	...
	if (!name)
	{
        printf("miiphy_get_current_dev fail for _mdc_mdio_write \r\n");
		return ;
	}

	miiphy_write(name, phy_id, register_id, data);
    return;
}
                     ...
                     rtk_uint32 *pdata)
{
	...
	if (!name)
	{
        printf("miiphy_get_current_dev fail for _mdc_mdio_read \r\n");
		return ;
	}

	ret = miiphy_read(name, phy_id, register_id, &val);
	if (ret < 0)
	{
        printf("miiphy_read fail for _mdc_mdio_read \r\n");
	}
	else
	{
        *pdata = val;
