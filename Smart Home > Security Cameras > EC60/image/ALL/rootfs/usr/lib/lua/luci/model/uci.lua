...
function get_profile(section, option)
	local val
	inst_profile = inst_profile or cursor("/etc/profile.d")
	if section and option then
		val = inst_profile:get_first("profile", section, option)
	end
	return val
end
Cursor.get_profile = get_profile
