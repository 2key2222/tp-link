...
#ifndef __STUNT_NET_H__
#define __STUNT_NET_H__
...
#define LOGE(...)
	...
	#define PRINTF(fmt, ...) do{\
		char __buf[64];\
		struct tm __tm;\
		time_t	__utc = time(NULL);\
		localtime_r(&__utc, &__tm);\
		strftime(__buf, sizeof(__buf), "%Y-%m-%d %H:%M:%S", &__tm);\
		LOGE("[%s] %s:%d "fmt, __buf, __FUNCTION__, __LINE__, ##__VA_ARGS__);\
		fprintf(g_dbgfile?g_dbgfile:stdout, "%s [%s] %s:%d "fmt, __buf, \
			   	g_module_name, __FUNCTION__, __LINE__, ##__VA_ARGS__);\
	}while(0)
	#endif
#else
	#ifndef PRINTF
