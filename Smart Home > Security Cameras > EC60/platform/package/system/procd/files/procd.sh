...
_procd_set_param() {
	...
	case "$type" in
		...
		;;
		crash)
			local v k keys cmd cur
			json_select command
			json_get_keys keys
			_json_get_var cur JSON_CUR
			for k in $keys; do
				json_get_var v $k
				[ -z "$cmd" ] && cmd="((${2+sleep $2;}ubus call service event '{\"type\":\"crash.$(basename ${1:-$v})\",\"data\":{}}')&);exec"
				v=${v/\\/\\\\}
				cmd="$cmd \""${v/\"/\\\"}"\""
				unset ${JSON_PREFIX}T_${cur}_${k}
			done
			json_select ..
			_procd_append_param command /bin/sh -c "$cmd"
		;;
	esac
}

_procd_add_crash_trigger() {
	local script=$(readlink "$initscript")
	local name=$(basename ${script:-$initscript})
	local _json_no_warning=1
	json_select triggers
	if [ $? = 0 ]; then
		_procd_add_raw_trigger "crash.$1" "${3:-1}" /etc/init.d/$name ${2:-restart}
		json_select ..
	else
		_procd_open_trigger
		_procd_add_raw_trigger "crash.$1" "${3:-1}" /etc/init.d/$name ${2:-restart}
