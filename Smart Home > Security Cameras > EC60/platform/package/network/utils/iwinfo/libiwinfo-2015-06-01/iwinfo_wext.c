...
static int wext_get_signal(const char *ifname, int *buf)
{
	...
	{
		if ((stats.qual.updated & IW_QUAL_DBM) && stats.qual.level >= 0 && stats.qual.level <= 0x7f)
		{
			stats.qual.level = 0xff;  /* set rssi to -1 when it's greater than 0; level is uint8_t, 0xff == -1; */
		}
