...
#define IW_SCAN_HACK_CACHED		0x8000
#define IW_SCAN_HACK_FAST		0x4000
#define IW_SCAN_HACK_BAND_2G	0x1000
#define IW_SCAN_HACK_BAND_5G	0x2000
#define IW_SCAN_HACK_BAND_MASK	0x3000
...
static int check_scan_arg(char **buf, char *arg)
{
	int len = strlen(arg);
	if (!strncmp(*buf, arg, len)) {
		*buf += len;
		if (**buf == ':') {
			++*buf;
		}
...
int wext_get_scanlist(const char *ifname, char *buf, int *len)
{
	...
	char *arg_buf = buf;
	int buflen = 65535; /* Min for compat WE<17 */
	struct iw_range range;
	int has_range = 1;
	struct timeval tv;             /* Select timeout */
	int timeout = 20000000;     /* 20s */
	unsigned short giwscan_flags = 0;
	unsigned short siwscan_flags = 0;
	...
	if (*len > 0) {
		*len = 0;
		/* the arg in format like scan:fast:2g or last:cached:ssid=<xxx> */
		while (*arg_buf) {
			/* scan or scan result */
			if (check_scan_arg(&arg_buf, "scan")) {
				operate = 1;
			} else if (check_scan_arg(&arg_buf, "last")) {
				operate = 2;
			}
			/* scan parameters */
			else if (check_scan_arg(&arg_buf, "2g")) {
				siwscan_flags |= IW_SCAN_HACK_BAND_2G;
			} else if (check_scan_arg(&arg_buf, "5g")) {
				siwscan_flags |= IW_SCAN_HACK_BAND_5G;
			} else if (check_scan_arg(&arg_buf, "fast")) {
				siwscan_flags |= IW_SCAN_HACK_FAST;
			}
			/* scan result parameters */
			else if (check_scan_arg(&arg_buf, "cached")) {
				giwscan_flags |= IW_SCAN_HACK_CACHED;
			}
			else if (check_scan_arg(&arg_buf, "ssid")) {
				/* ssid always be the last argument */
				memset(ssid, 0, sizeof(ssid));
				strncpy(ssid, arg_buf + 1, IWINFO_ESSID_MAX_SIZE);
				arg_buf += strlen(arg_buf);
			}
		}
	}
	...
	if( wext_ioctl(ifname, SIOCGIWRANGE, &wrq) >= 0 )
	{
		...
		wrq.u.data.flags   = siwscan_flags;
		wrq.u.data.length  = 0;

		/* Initiate Scanning */
		if( operate == 2 || wext_ioctl(ifname, SIOCSIWSCAN, &wrq) >= 0 )
		{
			if (operate == 1) {
				return 0;
			}
			...
			while(1)
			{
				...
				ret = (operate == 2) ? 0 : select(last_fd + 1, &rfds, NULL, NULL, &tv);
		...
		realloc:
					...
					wrq.u.data.flags   = giwscan_flags;
