...
static int fit_check_fdt(const void *fit, int fdt_noffset, int verify)
{
	...
	if (verify) {
		MESSAGE("   Verifying Hash Integrity ... ");
...
int boot_relocate_fdt(struct lmb *lmb, char **of_flat_tree, ulong *of_size)
{
	...
	if (disable_relocation) {
		...
		MESSAGE("   Loading Device Tree to %p, end %p ... ",
			of_start, of_start + of_len - 1);
	...
	if (argc > 3 || genimg_has_config(images)) {
		...
		case IMAGE_FORMAT_FIT:
			...
			if (fit_check_format((const void *)fdt_addr)) {
				 ...
				 */
				fit_hdr = (void *)fdt_addr;
				MESSAGE("## Flattened Device Tree from FIT "
						"Image at %08lx\n",
						fdt_addr);
				...
				MESSAGE("   Trying '%s' FDT blob subimage\n",
						fit_uname_fdt);
				...
				if (fit_image_get_load(fit_hdr, fdt_noffset,
					...
					MESSAGE("   Loading FDT from 0x%08lx "
							"to 0x%08lx\n",
							(ulong)data,
							load_start);
...
#endif
			{
				...
				MESSAGE("## Flattened Device Tree blob at "
					"%08lx\n", (long)fdt_blob);
			}
			break;
		default:
		...
		MESSAGE("   Booting using the fdt blob at 0x%p\n", fdt_blob);
