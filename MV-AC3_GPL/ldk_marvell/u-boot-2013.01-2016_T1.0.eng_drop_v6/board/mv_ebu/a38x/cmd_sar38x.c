...
int do_cpureg(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
	const char *cmd;
	...
	if (strcmp(cmd, "write") == 0) 
	{
		...
		printf("\nreg write 0x%x = 0x%x\n\n",reg,data);
		MV_REG_WRITE(reg, data); 
		return 0;
	} 
	else if (strcmp(cmd, "read") == 0)
	{
		reg = simple_strtoul(argv[2], NULL, 16);
		data = MV_REG_READ(reg);
		printf("\nreg read 0x%x = 0x%x\n\n",reg,data);
		return 0;
	}
...
U_BOOT_CMD(cpureg, 4, 1, do_cpureg,
	"cpu reg read write",

	"cpureg read <field>          - print the requested <field> value\n\n"
	"cpureg write <field> <val>   - write the requested <field> <value>\n\n"
