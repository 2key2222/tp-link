...
static bool check_ifaddr(struct nlmsghdr *hdr, int ifindex)
{
	...
	/* Ignore to delete IPv6 link-local address */
	if (ifa->ifa_family == AF_INET6) {
		ret = ifa->ifa_scope != RT_SCOPE_LINK;
	}
	return ret && ifa->ifa_index == ifindex;
}
...
int system_if_get_flags(const char *ifname, unsigned int *flags)
{
	struct ifreq ifr;
