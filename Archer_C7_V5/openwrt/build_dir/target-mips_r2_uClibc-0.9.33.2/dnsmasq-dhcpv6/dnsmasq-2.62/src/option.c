		...
		if (strlen(a[leasepos]) > 0)
		  {
		    ...
		    /*	
		    	For AP200, we need a minimum lease time of 60s.
		       	Edit by xiejiabai@tp-link.net, 2015.12.15	
		    */		       		
		    #if 0
		    if (new->lease_time < 120)
		      new->lease_time = 120;
		    #endif
		    #define MINIMUM_LEASE_TIME 60
		    if (new->lease_time < MINIMUM_LEASE_TIME)
		    {
		    	new->lease_time = MINIMUM_LEASE_TIME;
		    }
		  }
	      }
	  }
