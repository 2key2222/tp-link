...
/*			   				       TYPE     				               */
/***************************************************************************/
struct ubus_object CamuleUbusRpc::m_ubus_object;
struct ubus_context* CamuleUbusRpc::context = NULL;
struct blob_buf CamuleUbusRpc::b;

struct blobmsg_policy CamuleUbusRpc::amule_add_link_policy[2] = {
	{"link_url", BLOBMSG_TYPE_STRING},
	{"path", BLOBMSG_TYPE_STRING}
};
struct blobmsg_policy CamuleUbusRpc::amule_file_op_policy[1] = {
	{"hash", BLOBMSG_TYPE_STRING}
};
struct blobmsg_policy CamuleUbusRpc::amule_server_op_policy[2] = {
	{"ip", BLOBMSG_TYPE_STRING},
	{"port", BLOBMSG_TYPE_INT32}
};
struct blobmsg_policy CamuleUbusRpc::amule_pref_policy[2] = {
...
/* callback function */
struct ubus_method CamuleUbusRpc::amule_ubus_object_methods[] = {
	...
	{ "amuleGetDlFilesStatus", AmuleUbusGetDlStatusHandle, amule_file_op_policy, ARRAY_SIZE(amule_file_op_policy)},
    { "amuleIsFileCompleted", AmuleUbusIsFileCompletedHandle, amule_file_op_policy, ARRAY_SIZE(amule_file_op_policy)},
	{ "amuleFileDlResume", AmuleUbusFileDlResumeHandle, amule_file_op_policy, ARRAY_SIZE(amule_file_op_policy)},
	{ "amuleFileDlPause", AmuleUbusFileDlPauseHandle, amule_file_op_policy, ARRAY_SIZE(amule_file_op_policy)},
	{ "amuleFileDlDelete", AmuleUbusFileDlDeleteHandle, amule_file_op_policy, ARRAY_SIZE(amule_file_op_policy)},
	...
	{ "amuleServerList", AmuleUbusServerListHandle, NULL, 0},
...
struct ubus_object_type CamuleUbusRpc::amule_ubus_object_type =
 ...
 */
int CamuleUbusRpc::AmuleUbusShutdownHandle(struct ubus_context *ctx, struct ubus_object *obj,
 ...
 */
int CamuleUbusRpc::AmuleUbusAddLinkHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
 ...
 */
int CamuleUbusRpc::AmuleUbusShowDlHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	void *a, *t;
	...
	for (unsigned int i = 0; i < theApp->sharedfiles->GetFileCount(); ++i) {
        ...
        //in sharelist not in downloadqueue, must be completed file in sharing
        if (kfile && !theApp->downloadqueue->GetDownloadFileByID(kfile->GetFileHash())){
			a = blobmsg_open_table(&b, NULL);
			blobmsg_add_string(&b, "file_hash", unicode2UTF8(kfile->GetFileHash().Encode()));
			blobmsg_add_u64(&b, "file_size", kfile->GetFileSize());
			blobmsg_add_string(&b, "file_status", "complete");
			blobmsg_add_u64(&b, "file_completed", kfile->GetFileSize());
            ...
            for (; it != clients.end(); ++it) {
                ...
                if (cur_client && cur_client->GetUploadFileID() == kfile->GetFileHash()){
                    upRate += cur_client->GetUploadDatarate();
                }
            }
 ...
 */
int CamuleUbusRpc::AmuleUbusGetDlStatusHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	bool flag = false;
	...
	blobmsg_parse(amule_file_op_policy, 1, tb, blob_data(msg), blob_len(msg));
	...
	blob_buf_init(&b, 0);
	if ( !pfile ) {
        CKnownFile *kfile = theApp->sharedfiles->GetFileByID(temHash);
		if (kfile){
			blobmsg_add_u32(&b, "errorcode", RET_OK);
 ...
 */
int CamuleUbusRpc::AmuleUbusIsFileCompletedHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	bool flag = false;
	struct blob_attr *tb[1];
		
	UBUS_DEBUG("=============begin: AmuleUbusIsFileCompletedHandle============= ");
	
	if (!ctx) {
		AddLogLineC(_("AmuleUbusIsFileCompletedHandle ctx is not inited!"));
		return -1;
	}
	...
	if (!tb[0] || !blob_data(tb[0])) {
		AddLogLineC(_("AmuleUbusIsFileCompletedHandle argument hash invalid."));
		return -1;
	}

	char * hash = blobmsg_get_string(tb[0]);
	UBUS_DEBUG("AmuleUbusIsFileCompletedHandle hash = %s\n ",hash);
	
	CMD4Hash temHash;
	temHash.Decode(UTF82unicode(hash));
	UBUS_DEBUG("AmuleUbusIsFileCompletedHandle decode hash = %s\n ", temHash.GetHash());

	blob_buf_init(&b, 0);
	blobmsg_add_u32(&b, "errorcode", RET_OK);
    blobmsg_add_string(&b, "file_hash", hash);
    if (theApp->downloadqueue->IsHashExistCompletedList(temHash)) {
        blobmsg_add_string(&b, "file_completed", "yes");
    }else {
        blobmsg_add_string(&b, "file_completed", "no");
    }

	/* send message back */
	ubus_send_reply(ctx, req, b.head);
	UBUS_DEBUG("=============end: AmuleUbusIsFileCompletedHandle============= ");
    return 0;
}
 ...
 */
int CamuleUbusRpc::AmuleUbusFileDlResumeHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	struct blob_attr *tb[1];
	int errorcode = RET_OK;
		
	UBUS_DEBUG("=============begin: AmuleUbusFileDlResumeHandle============= ");
 ...
 */
int CamuleUbusRpc::AmuleUbusFileDlPauseHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	struct blob_attr *tb[1];
	int errorcode = RET_OK;
 ...
 */
int CamuleUbusRpc::AmuleUbusFileDlDeleteHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	struct blob_attr *tb[1];
	...
	CPartFile *pfile = theApp->downloadqueue->GetDownloadFileByID(temHash);
		
	if ( pfile ) {
        UBUS_DEBUG("AmuleUbusFileDlDeleteHandle part file delete.");
        
		//delete part file and start next part file
		if ( thePrefs::StartNextFile() && (pfile->GetStatus() != PS_PAUSED) ) {
			theApp->downloadqueue->StartNextFile(pfile);
		}	
		pfile->Delete();
	} else {
        UBUS_DEBUG("AmuleUbusFileDlDeleteHandle complete file delete.");
		//AddLogLineN(CFormat(_("AmuleUbusFileDlDeleteHandle not found in part file list: %s")) 
		//          % temHash.Encode());

		theApp->uploadqueue->SuspendUpload(temHash, true);

        //delete from share list
        CKnownFile *kf = theApp->sharedfiles->GetFileByID(temHash);
        if ( kf ) {
    	    theApp->sharedfiles->RemoveFile(kf);
        }        

        //delete from complete list
        theApp->downloadqueue->RemoveCompletedByID(temHash);
	}

    theApp->downloadqueue->RemoveCompletedHash(temHash);
 ...
 * callback for ubus, handle ubus client request: delete file download.
 */
int CamuleUbusRpc::AmuleUbusFileDlDeleteAllHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{	
	...
	for (unsigned int i = 0; i < theApp->sharedfiles->GetFileCount(); ++i) {
        ...
        //in sharelist not in downloadqueue, must be completed file in sharing
        if (kfile && !theApp->downloadqueue->GetDownloadFileByID(kfile->GetFileHash())){
            theApp->uploadqueue->SuspendUpload(kfile->GetFileHash(), true);
            
            //delete from share list
            CKnownFile *kf = theApp->sharedfiles->GetFileByID(kfile->GetFileHash());
            if ( kf ) {
                theApp->sharedfiles->RemoveFile(kf);
            }        

            //delete from complete list
            theApp->downloadqueue->RemoveCompletedByID(kfile->GetFileHash());
		}
	}

    theApp->downloadqueue->RemoveCompletedHashAll();
 ...
 */
int CamuleUbusRpc::AmuleUbusServerAddHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	struct blob_attr *tb[2];
 ...
 */
int CamuleUbusRpc::AmuleUbusServerRemoveAllHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{		
 ...
 */
int CamuleUbusRpc::AmuleUbusServerRemoveHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	struct blob_attr *tb[2];
 ...
 */
int CamuleUbusRpc::AmuleUbusServerConHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
	struct blob_attr *tb[2];
	int errorcode = RET_OK;
 ...
 */
int CamuleUbusRpc::AmuleUbusServerDisconHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{		
 ...
 */
int CamuleUbusRpc::AmuleUbusServerStatusHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{		
 ...
 */
int CamuleUbusRpc::AmuleUbusServerListHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{		
	void *a, *t;
	
	UBUS_DEBUG("=============begin: AmuleUbusServerListHandle============= ");
	
	if (!ctx)
	{
		AddLogLineC(_("AmuleUbusServerListHandle ctx is not inited!"));
		return -1;
	}
			
	size_t serverCnt = theApp->serverlist->GetServerCount();
	if (serverCnt!= 1) {
		AddLogLineC(CFormat(_("AmuleUbusServerListHandle server list num %d"))
					% serverCnt);
	}

	blob_buf_init(&b, 0);
	blobmsg_add_u32(&b, "errorcode", RET_OK);
	t = blobmsg_open_table(&b, "server_list");
	...
	for (
		...
		const CServer *tmp = *it;
		a = blobmsg_open_table(&b, NULL);
		blobmsg_add_string(&b, "server", unicode2UTF8(
			(Uint32_16toStringIP_Port(tmp->GetIP(), tmp->GetPort()))));
		blobmsg_close_table(&b, a);
	}
	...
	UBUS_DEBUG("=============end: AmuleUbusServerListHandle============= ");
 ...
 */
int CamuleUbusRpc::AmuleUbusSetPrefHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{
 ...
 */
int CamuleUbusRpc::AmuleUbusGetPrefHandle(struct ubus_context *ctx, struct ubus_object *obj,
			struct ubus_request_data *req, const char *method, struct blob_attr *msg)
{		
	UBUS_DEBUG("=============begin: AmuleUbusServerStatusHandle============= ");
...
/**
 * reconnect when connect lost.
 */
void CamuleUbusRpc::AmuleUbusReconnect(struct uloop_timeout *timeout)
{
	int t = 2;
	char *ubus_socket = "/var/run/ubus.sock";
	static struct uloop_timeout retry;
	
	retry.cb = AmuleUbusReconnect;

	if (!context)
	{
		UBUS_DEBUG("cloud_client_ctx is NULL\n");
		return ;
	}

	if (0 != ubus_reconnect(context, ubus_socket))
	{
		UBUS_DEBUG("failed to reconnect, trying again in %d seconds\n", t);
		uloop_timeout_set(&retry, t * 1000);
		return;
	}

	UBUS_DEBUG("reconnected to ubus, new id: %08x\n", context->local_id);
 ...
 */
void CamuleUbusRpc::AmuleUbusConnectionLost(struct ubus_context *ctx)
{
	AmuleUbusReconnect(NULL);
	return;
}
...
/**
 * process finish download a file.
 */
void CamuleUbusRpc::AmuleUbusDownloadFin(CMD4Hash hash)
{
	theApp->downloadqueue->AddCompletedHash(hash);

	//push completed state to ubus rpc
	wxString a;
	a << _("[ -e /lib/offline_download/offline_download_op.sh ] && ") <<
		CFormat(_(". /lib/offline_download/offline_download_op.sh && amule_complete %s"))
		% hash.Encode();
	system(unicode2char(a));
...
void* CamuleUbusRpc::Entry()
{
    ...
    context = ubus_connect(ubus_socket);
    if (!context)
    {
    ...
    context->connection_lost = AmuleUbusConnectionLost;
	
	/* register ubus object */
    ret = ubus_add_object(context, &m_ubus_object);
    if (ret != 0)
    {
...
{
    uint16 completeHashCount = 0;
    
#ifndef __WXMSW__
#ifdef ENABLE_UBUS_RPC
    completeHashCount = downloadqueue->GetCompletedHashCount();
#endif
#endif

	AddLogLineN(CFormat(_("sharelist %d downfilelist %d upwaitlist %d uploadinglist %d "
				" ClientCount %d listensocketnum %d Serverlist %d creditslist %d CompletedHash %d\n"))
				% sharedfiles->GetFileCount()
...
void CamuleApp::OnCoreTimer(CTimerEvent& WXUNUSED(evt))
{
	...
	if (msCur-msPrev5 > 3000) {  // zengwei mod to every 3 seconds
		...
		theLogger.CheckLogFileStatus(ConfigDir + m_logFile); //zengwei add
	}
...
void CamuleApp::OnFinishedHashing(CHashingEvent& evt)
{
	...
	if (owner) {
		...
		if ((sharedfiles->GetFileCount() > thePrefs::GetMaxDownloadFileNum() || 
			    (kfile == NULL && sharedfiles->GetFileCount() == thePrefs::GetMaxDownloadFileNum()))
			&& thePrefs::GetMaxDownloadFileNum() > 0){
			AddDebugLogLineN(logKnownFiles, 
               CFormat(wxT("share full file number(%d) is up to max(%d).")) 
               % sharedfiles->GetFileCount() % thePrefs::GetMaxDownloadFileNum());
			delete result;
			return;			
		}
