...
CEncryptedStreamSocket::CEncryptedStreamSocket(wxSocketFlags flags, const CProxyData *proxyData) : CSocketClientProxy(flags, proxyData)
{
	...
	memset(m_cryptDHA, 0, DHAGREEMENT_A_BITS/8);
	m_ncryptDHALen = 0;
...
void CEncryptedStreamSocket::StartNegotiation(bool bOutgoing)
{
	...
	if (!bOutgoing) {
...
#else		
		byte *dh_cryptA_start;
		byte dh_yc[PRIMESIZE_BYTES];
		size_t dh_cryptA_len;

		memset(dh_yc, 0, PRIMESIZE_BYTES);

		//get csecret
		if (get_random_fread(m_cryptDHA, DHAGREEMENT_A_BITS/8)) {
			wxFAIL;
			return;
		}

		dh_cryptA_start = (byte *)m_cryptDHA;
		dh_cryptA_len = DHAGREEMENT_A_BITS/8;
		while (dh_cryptA_len > 1 && *dh_cryptA_start == 0) {
			dh_cryptA_start++;
			dh_cryptA_len--;
		}
		...
		/* Yc = a^csecret mod q */
		byte a[]={0x2};
		if (crypto_mod_exp(a, 1, dh_cryptA_start, dh_cryptA_len, dh768_p, PRIMESIZE_BYTES, dh_yc, PRIMESIZE_BYTES)){
			wxFAIL;
			return;					
		}
				
		fileRequest.Write(dh_yc, PRIMESIZE_BYTES);
#endif //ENABLE_TOMCRYPT
...
//						AddDebugLogLine(DLP_LOW, false, _T("CEncryptedStreamSocket: Client %s preffered unsupported encryption method (%i)"), DbgGetIPString(), m_dbgbyEncryptionRequested);
				...
				case ONS_BASIC_SERVER_DHANSWER: {
					...
					m_pfiReceiveBuffer.SetKey(md5);
#else					
					wxASSERT( m_ncryptDHALen != 0 );
					uint8_t aBuffer[PRIMESIZE_BYTES + 1];
					m_pfiReceiveBuffer.Read(aBuffer, PRIMESIZE_BYTES);

					/* x = Yb^csecret mod q */
					uint8_t *dh_cryptA_start;
					uint8_t cryptResult[PRIMESIZE_BYTES + 1];
					size_t dh_cryptA_len;

					memset(cryptResult, 0, PRIMESIZE_BYTES + 1);

					dh_cryptA_start = (byte *)m_cryptDHA;
					dh_cryptA_len = m_ncryptDHALen;
					while (dh_cryptA_len > 1 && *dh_cryptA_start == 0) {
						dh_cryptA_start++;
						dh_cryptA_len--;
					}
					...
					//get the result
					if (crypto_mod_exp(aBuffer, PRIMESIZE_BYTES, dh_cryptA_start, dh_cryptA_len, 
										dh768_p, PRIMESIZE_BYTES, cryptResult, PRIMESIZE_BYTES)){
						OnError(ERR_ENCRYPTION);
						return (-1);				
					}
					...
					cryptResult[PRIMESIZE_BYTES] = MAGICVALUE_REQUESTER;
					MD5Sum md5(cryptResult, sizeof(cryptResult));
					m_pfiSendBuffer.SetKey(md5);
					cryptResult[PRIMESIZE_BYTES] = MAGICVALUE_SERVER;
					md5.Calculate(cryptResult, sizeof(cryptResult));
					m_pfiReceiveBuffer.SetKey(md5);
#endif //ENABLE_TOMCRYPT
