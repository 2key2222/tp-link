...
void fib_route_scan(struct net *net, uint32_t lan_ip, uint32_t lan_mask, void (*update_static_route)(uint32_t, uint32_t, uint32_t))
{
	uint32_t dip, mask, gateway;
	struct trie *t;
	struct leaf *l;
	struct leaf_info *li;
	struct hlist_node *node;
	...
	while (l) {
		...
		hlist_for_each_entry_rcu(li, node, &l->list, hlist) {
			...
			gateway = 0;
			list_for_each_entry_rcu(fa, &li->falh, fa_list) {
				if (fa->fa_type == RTN_BROADCAST || fa->fa_type == RTN_MULTICAST)
					continue;

				if (fa->fa_info->fib_nh->nh_gw) {
					gateway = htonl(fa->fa_info->fib_nh->nh_gw);
					break;
				}
			}				
			if (dip && ((dip & lan_mask) != (lan_ip & lan_mask))) {
				update_static_route(dip, mask, gateway);
			}	
		}
		l = trie_nextleaf(l);
	}
...
EXPORT_SYMBOL_GPL(fib_route_scan); 
#endif
