...
long CyaSSL_set_mode(CYASSL* ssl, long mode)
    {
        /* SSL_MODE_ACCEPT_MOVING_WRITE_BUFFER is CyaSSL default mode */
        CYASSL_ENTER("SSL_set_mode");
        if (mode & SSL_MODE_ACCEPT_MOVING_WRITE_BUFFER == SSL_MODE_ENABLE_PARTIAL_WRITE){
            ssl->options.partialWrite = 1;
            }
        return mode;
    }
...
#define BN_BITS2        32
char *CyaSSL_BN_bn2hex(const BIGNUM *bn)
{
	int i,j,v,z = 0;
	char *buf;
	char *p;
	mp_int *m = (mp_int *)(bn->internal);
	char* decoded = NULL;

	decoded = (char*)XMALLOC(m->used * 4 * 2 + 2, NULL, DYNAMIC_TYPE_TMP_BUFFER);
	if (decoded == NULL)
		return NULL;
	p = decoded;
	//if (m->sign) *(p++)='-';
	if (BN_is_zero(bn))
		*(p++)='0';
	for (i = m->used - 1; i >= 0; i--) {
		for (j = DIGIT_BIT - 4; j >= 0; j -= 4) {
			/* strip leading zeros */
			v = ((int)(m->dp[i] >> (long)j)) & 0xf;
			if (z || (v != 0)) {
				*(p++) = Hex[v & 0x0f];
				z = 1;
			}
