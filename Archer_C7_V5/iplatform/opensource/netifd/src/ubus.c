...
/* connection control management object */
...
static struct ubus_method connctl_object_methods[] = {
	{ .name = "start", .handler = netifd_handle_connctl },
	{ .name = "stop", .handler = netifd_handle_connctl },
	{ .name = "status", .handler = netifd_handle_connctl },
};

static struct ubus_object_type connctl_object_type =
	UBUS_OBJECT_TYPE("netifd_connctl", connctl_object_methods);

static struct ubus_object connctl_object = {
	.name = "network.connctl",
	.type = &connctl_object_type,
	.methods = connctl_object_methods,
	.n_methods = ARRAY_SIZE(connctl_object_methods),
};
                      ...
                      struct blob_attr *msg)
{
	...
	iface = container_of(obj, struct interface, ubus);
	interface_set_connectable(iface, true);
	interface_set_up(iface);
                         ...
                         struct blob_attr *msg)
{
	...
	iface = container_of(obj, struct interface, ubus);
	interface_set_connectable(iface, false);
	interface_set_down(iface);
...
static int
netifd_handle_up_vif(struct ubus_context *ctx, struct ubus_object *obj,
		       struct ubus_request_data *req, const char *method,
		       struct blob_attr *msg)
{
	struct interface *iface;
...
static struct ubus_method iface_object_methods[] = {
	...
	{ .name = "up_vif", .handler = netifd_handle_up_vif },
};
...
netifd_ubus_init(const char *path)
{
	...
	netifd_add_object(&connctl_object);
