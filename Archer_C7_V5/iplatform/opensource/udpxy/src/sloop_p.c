             ...
             const char* mcast_addr )
{
	...
	(void)tmfprintf( g_flog, "Init server ctx, max clients = [%u], ipaddr = [%s], port = [%d], mcast_addr =[%s]\n",
                        g_uopt.max_clients, ipaddr, port, mcast_addr );
	...
	(void)tmfprintf( g_flog, "Setup listener, ipaddr = [%s], port = [%d], lsockfd = [%d], lq_backlog = [%d]\n",
                        ipaddr, port, &g_srv.lsockfd, g_uopt.lq_backlog );
    ...
    while (1) {
		...
		(void)tmfprintf( g_flog, "lsockfd = [%d], cpipe[0] = [%d], cpipe[1] = [%d], nasock = [%d]\n",
                        &g_srv.lsockfd, g_srv.cpipe[0], g_srv.cpipe[1], nasock );
