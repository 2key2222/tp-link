...
static timer_t l_syncTime = 0;
typedef void (* sighandler)();

static timer_t smb_timerDelayRun(sighandler func, int nDelay)
{
	timer_t tid;
	struct sigevent se;
	...
	{
		DEBUG(1, ("Failed timer_creat.\n"));
		return (timer_t)-1; 
	}
	
	ts.it_value.tv_sec = nDelay;
...
void sync_disk()
{
	if (l_syncTime != (timer_t)0)
	{
  		if (0 != timer_delete(l_syncTime))
  		{
			DEBUG(1, ("Fail to delete sync timer.\n"));
		}
		l_syncTime = (timer_t)0;
  	}

	l_syncTime = smb_timerDelayRun(timer_sync, 2);
	if (l_syncTime <= (timer_t)0)
	{
 		DEBUG(1,("Fail to set sync timer.\n"));
	}
}