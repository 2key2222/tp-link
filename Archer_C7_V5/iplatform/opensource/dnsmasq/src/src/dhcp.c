...
void dhcp_packet(time_t now, int pxe_fd)
{
  ...
  if (pxe_fd)
    ...
    {
      /*  If permanent ARP entry is already existed, it is not right to call 
        ioctl to add a completed ARP entry to overwrite the permanent entry.
        Add arp entry detection here to prevent IP MAC binding list being overwritten 
        by a client with same list in IP MAC binding list when this client sends unicast packet 
        to reconnect to the network.
        Modify start by hjj, 2015-02-13 
      */
      struct sockaddr_in *sin;
      sin = (struct sockaddr_in *)&arp_req.arp_pa;
      sin->sin_family = AF_INET;
      sin->sin_addr = mess->yiaddr;
      
      int ret = ioctl(daemon->dhcpfd, SIOCGARP, &arp_req);
      if (ret < 0 || (ret >= 0 && !(arp_req.arp_flags & ATF_PERM)))
        {
...
int ip_mac_is_binding(struct in_addr addr, char *iface_name, unsigned char * emac, int emac_len)
{
#ifdef HAVE_LINUX_NETWORK
  struct arpreq arp_req;
  struct sockaddr_in *sin;
  /* param check */
  if (NULL == iface_name || NULL == emac || emac_len <= 0)
    return 0;

  strncpy(arp_req.arp_dev, iface_name, 16);
  sin = (struct sockaddr_in *)&arp_req.arp_pa;
  sin->sin_family = AF_INET;
  sin->sin_addr = addr;
  arp_req.arp_flags = 0;
  ...
  int ret = ioctl(daemon->dhcpfd, SIOCGARP, &arp_req);
  if (ret >= 0 && (arp_req.arp_flags & ATF_PERM))
    {
      /* IP MAC Binding entry use permanent ARP entry, check mac addr */
      if (emac_len != 0 && memcmp(emac, arp_req.arp_ha.sa_data, emac_len))
        return 1;
    }
#endif
  return 0;
...
int address_allocate(struct dhcp_context *context,
		     ...
		     struct dhcp_netid *netids, time_t now, char *iface_name)   
{
  ...
  for (pass = 0; pass <= 1; pass++)
      ...
      else
	{
	  ...
	  do {
	    ...
	    if (!d &&
		!ip_mac_is_binding(addr, iface_name, hwaddr, hw_len) && 
