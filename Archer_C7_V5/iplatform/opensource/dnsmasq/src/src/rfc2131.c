...
/*	If a DHCP DECLINED packet received,
	the client ip will be  marked as non-allocatable 
	for a period of DCLINE_BACKOFF.
	Edited by xiejiabai@tp-link.net,2015.12.18
*/
#define CONFIG_DHCP_DECLINE_IP_BACKOFF 1
#ifdef CONFIG_DHCP_DECLINE_IP_BACKOFF 
struct dhcp_config *record_decline_ip(struct in_addr decline_ip,time_t now);
void release_decline_ip(time_t now);
/*
* fn  	struct dhcp_config *record_decline_ip(struct in_addr decline_ip,time_t now)
* brief record ip as a config in decline packet
* param[in] decline_ip - the ip in decline packet
* param[in] now - the current time
* return - the header of dhcp config list
* success - the header of dhcp config list
* failure - NULL
*/
struct dhcp_config *record_decline_ip(struct in_addr decline_ip,time_t now)
{
	if (!config_find_by_address(daemon->dhcp_conf, decline_ip))
	{
		struct dhcp_config *new = (struct dhcp_config *)malloc(sizeof(struct dhcp_config));
		if (!new) 
		{
			return NULL;
		}
		...
		new->flags = CONFIG_ADDR | CONFIG_DECLINED;
		new->hwaddr = NULL;
		new->netid = NULL;
		new->clid = NULL;
		new->decline_time = now;
		new->addr = decline_ip;
		daemon->dhcp_conf = new;
		
		return daemon->dhcp_conf;	
	}
	return NULL;
}

/*
* fn  	release_decline_ip(time_t now)
* brief release a decline ip record in config list if decline backoff time out. 
* param[in] now - the current time
* return - none
*/
void release_decline_ip(time_t now)
{
	struct dhcp_config *config, *pre;
	for(pre=config=daemon->dhcp_conf; config; pre=config, config=config->next)
	{
		if ( (have_config(config, CONFIG_ADDR)) && (have_config(config, CONFIG_DECLINED))
				&& (difftime(now, config->decline_time) > (float)DECLINE_BACKOFF) && (!config->hwaddr) 
					&& (!config->clid) )
		{
			pre->next = config->next;
			if (daemon->dhcp_conf==config)
			{
				daemon->dhcp_conf=config->next;
			}
			free(config);
		}
	}
      ...
      if (!message)
	{
	  ...
	  if (have_config(config, CONFIG_ADDR))
		  ...
		  !address_available(context, lease->addr, tagif_netid))
		{
		   ...
		   if (!address_allocate(context, &mess->yiaddr, mess->chaddr, mess->hlen, tagif_netid, now, iface_name))
  ...
  */
  ...
  switch (mess_type)
...
#ifdef CONFIG_DHCP_DECLINE_IP_BACKOFF     
      record_decline_ip(option_addr(opt), now);
#endif     
      ...
      else 
	{
	  ...
	  else if (!address_allocate(context, &mess->yiaddr, emac, emac_len, tagif_netid, now, iface_name))
	    message = _("no address available");      
	  
	  /*
	    ip mac binding priority is higher than reservation address/lease already existed/DHCP_REQUESTED IP in DHCPDISCOVER.
	    Check ip mac binding addr conflict. If yiaddr conflicts with ip mac binding addr, realloc a new address. 
	    Add by hjj 2015.03.03
	  */
	  if (!message && ip_mac_is_binding(mess->yiaddr, iface_name, emac, emac_len))
		  if (!address_allocate(context, &mess->yiaddr, emac, emac_len, tagif_netid, now, iface_name))
	    message = _("no address available");      
	}
      ...
      if (!message)
	{
	  ...
	  /*  check if the address has been reserved as an ip mac binding address for another host 
	      Add by hjj 2015.03.03
	  */
	  else if (ip_mac_is_binding(mess->yiaddr, iface_name, emac, emac_len))
	    message = _("address binding");
  ...
  if (context)
	...
	option_put(mess, end, OPTION_BROADCAST, INADDRSZ, ntohl(context->broadcast.s_addr));
	 ...
	 default when using subnet select.  User must configure by steam in that case. */
	  ...
	  /* If there is no default router(gateway, list dhcp_option '3,0.0.0.0') in the configura 
	     entry, tp-link DHCP Server do not add option: (3) Router in ACK packet, in order to 
	     achieve the purpose of not configured default gateway. Removed by xlw, 2015-02-06 */
#if 0
      if (context->router.s_addr &&
	  in_list(req_options, OPTION_ROUTER) &&
	  !option_find2(OPTION_ROUTER)){
	    my_syslog(MS_DHCP | LOG_INFO, _("[%s %d]s_addr 0x%x"),__func__,__LINE__,ntohl(context->router.s_addr));
