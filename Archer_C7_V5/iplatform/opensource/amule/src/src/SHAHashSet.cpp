...
// recursive
bool CAICHHashTree::FindLevel(uint64 nFileSize, uint64 nStartPos, uint64 nSize, uint8* nLevel)
{	
	uint64 nTmpDataSize = nFileSize;
	uint64 nTmpBaseSize = (nFileSize <= PARTSIZE) ? EMBLOCKSIZE : PARTSIZE;
	uint64 nTmpStartPos = nStartPos;
	
	uint64 nBlocks = 0; 
	uint64 nLeft = 0;
	uint64 nRight = 0;
	bool bIsLeftBranch = true; //root

	(*nLevel) = 0;

	if (nStartPos == 0 && nSize == nFileSize) {
		// this is the searched hash
		(*nLevel) = 1;
		return bIsLeftBranch;
	}

	while(1){				
		(*nLevel)++;
		
		AddDebugLogLineN(logSHAHashSet, CFormat(_("nTmpDataSize %ld nTmpBaseSize %ld m_bIsLeftBranch %d nTmpStartPos%ld")) 
			% nTmpDataSize% nTmpBaseSize% bIsLeftBranch% nTmpStartPos);

		if (*nLevel > 22 || (nTmpStartPos + nSize) > nTmpDataSize) { // sanity
			(*nLevel) = 0;
			break;
		}else if (nTmpStartPos == 0 && nSize == nTmpDataSize){
			break;
		}else if (nTmpDataSize <= nTmpBaseSize) { // sanity
			break;
		}else {			
			nBlocks = nTmpDataSize / nTmpBaseSize + ((nTmpDataSize % nTmpBaseSize != 0 )? 1:0); 
			nLeft = (((bIsLeftBranch) ? nBlocks+1:nBlocks) / 2)* nTmpBaseSize;
			nRight = nTmpDataSize - nLeft;
//				AddDebugLogLineN(logSHAHashSet, CFormat(_("nLeft %ld nRight %ld nBlocks %ld  nLevel %d")) 
//					%nLeft% nRight% nBlocks% *nLevel);
			if (nTmpStartPos < nLeft) {
				if (nTmpStartPos + nSize > nLeft) { // sanity
					(*nLevel) = 0;
					break;
				}
				...
				nTmpDataSize = nLeft;
				bIsLeftBranch = true;
				nTmpBaseSize = (nLeft <= PARTSIZE) ? EMBLOCKSIZE : PARTSIZE;
				
			} else {
				nTmpStartPos -= nLeft;
				if (nTmpStartPos + nSize > nRight) { // sanity
					(*nLevel) = 0;
					break;
				}
				nTmpDataSize = nRight;
				bIsLeftBranch = false;
				nTmpBaseSize = (nRight <= PARTSIZE) ? EMBLOCKSIZE : PARTSIZE;
...
//		AddLogLineC(CFormat(_("m_nDataSize %ld m_nBaseSize %ld nStartPos%ld m_bIsLeftBranch %d nLevel %d")) 
//					% m_nDataSize % m_nBaseSize % nStartPos % m_bIsLeftBranch % *nLevel);
...
//			AddLogLineC(CFormat(_("nLeft %ld nRight %ld nBlocks %ld")) % nLeft % nRight %  nBlocks);
		if (nStartPos < nLeft) {
...
bool CAICHHashTree::VerifyHashTree(CAICHHashAlgo* hashalg, bool bDeleteBadTrees)
{
	...
	if ((m_pRightTree && m_pRightTree->m_bHashValid) && (m_pLeftTree && m_pLeftTree->m_bHashValid)) {			
		...
		if (m_Hash != CmpHash) {
			...
			AddDebugLogLineN(logSHAHashSet, wxT("VerifyHashSet failed - m_Hash != CmpHash"));
			return false;
		}
...
bool CAICHHashTree::VerifyHashPartTree(CAICHHashAlgo* hashalg, bool bDeleteBadTrees)
{
	// calculated missing hashs without overwriting anything
	if (m_pLeftTree && !m_pLeftTree->m_bHashValid) {
		m_pLeftTree->ReCalculateHash(hashalg, true);
	...
	if ((m_pRightTree && m_pRightTree->m_bHashValid) ^ (m_pLeftTree && m_pLeftTree->m_bHashValid)) {
		...
		AddDebugLogLineN(logSHAHashSet, wxT("VerifyHashPartTree failed - Hashtree incomplete"));
		return false;
	}

	//get part hash and verify part tree
	if ((m_pRightTree && m_pRightTree->m_bHashValid) && (m_pLeftTree && m_pLeftTree->m_bHashValid)) {			
	    // check verify the hashs of both child nodes against my hash 
		...
		AddDebugLogLineN(logSHAHashSet, wxT("VerifyHashPartTree failed - can't calc part hash"));
		return false;
	}
}
...
//		AddLogLineC(CFormat(_("wHashIdent%x level %d m_bIsLeftBranch %d m_nDataSize %ld m_nBaseSize %ld")) 
//					%wHashIdent% nLevel% m_bIsLeftBranch% m_nDataSize% m_nBaseSize);
...
// write the hash, specified by wHashIdent, with Data from fileInput.
bool CAICHHashTree::SetPartHash(CFileDataIO* fileInput, uint32 wHashIdent, sint8 nLevel, uint8 nLevelFlag)
{
	if (nLevel == (-1)) {
//			AddDebugLogLineN(logSHAHashSet, CFormat(_("CAICHHashTree::SetPartHash - input wHashIdent %x level %d m_bIsLeftBranch %d nLevelFlag %d")) 
//						% wHashIdent % nLevel % m_bIsLeftBranch % nLevelFlag);
		// check how many level we need to go
		uint8 i = 0;
		for (; i != 32 && (wHashIdent & 0x80000000) == 0; ++i) {
			wHashIdent <<= 1;
		}
		if (i > 31) {
			AddDebugLogLineN(logSHAHashSet, wxT("CAICHHashTree::SetPartHash - found invalid HashIdent (0)"));
			return false;
		} else {
			nLevel = 31 - i;
			//not this part must nLevel>=nLevelFlag not include the root
			if ((nLevelFlag - 1) >= nLevel){
				CAICHHash tmpHash;
				tmpHash.Read(fileInput);
				AddDebugLogLineN(logSHAHashSet, CFormat(_("CAICHHashTree::SetPartHash - not this part wHashIdent %x level %d m_bIsLeftBranch %d nLevelFlag %d")) 
							% wHashIdent % nLevel % m_bIsLeftBranch % nLevelFlag);
				return true;
			}else{
				nLevel = nLevel - (nLevelFlag - 1);
				wHashIdent <<= (nLevelFlag - 1);
			}
		}
	}

	AddDebugLogLineN(logSHAHashSet, CFormat(_("CAICHHashTree::wHashIdent%x level %d m_bIsLeftBranch %d m_nDataSize %ld m_nBaseSize %ld")) 
				%wHashIdent% nLevel% m_bIsLeftBranch% m_nDataSize%  m_nBaseSize);
	...
	if (nLevel == 0) {
		...
		if ((wHashIdent & 0x80000000) > 0) {
			if (m_pLeftTree == NULL) {
				...
				if (m_pLeftTree->m_nDataSize != nLeft){
					AddDebugLogLineN(logSHAHashSet, CFormat(_("CAICHHashTree::wHashIdent %x level %d m_bIsLeftBranch %d nLeft %ld nRight %ld")) 
								% wHashIdent % nLevel% m_bIsLeftBranch % nLeft % nRight);
					return false;
				}
			}
			return m_pLeftTree->SetPartHash(fileInput, wHashIdent, nLevel, nLevelFlag);
		} else {
			if (m_pRightTree == NULL) {
				m_pRightTree = new CAICHHashTree(nRight, false, (nRight <= PARTSIZE) ? EMBLOCKSIZE : PARTSIZE);
			} else {
				if (m_pRightTree->m_nDataSize != nRight ){
					AddDebugLogLineN(logSHAHashSet, CFormat(_("CAICHHashTree::wHashIdent %x level %d m_bIsLeftBranch %d nLeft %ld nRight %ld")) 
								% wHashIdent % nLevel% m_bIsLeftBranch % nLeft % nRight);
					return false;
			...
			return m_pRightTree->SetPartHash(fileInput, wHashIdent, nLevel, nLevelFlag);
		}
	}
}
/////////////////////////////////////////////////////////////////////////////////////////
...
//			return false;
//		}
	if (!(m_pOwner->GetAICHHashset()->GetStatus() == AICH_VERIFIED || 
		m_pOwner->GetAICHHashset()->GetStatus() == AICH_TRUSTED) ) {
		wxFAIL;
		return false;
	}
	...
	*/
	...
	uint32 nPartSize = min<uint64>(PARTSIZE, m_pOwner->GetFileSize()-nPartStartPos);
	//init the part tree
	SetFileSize(nPartSize);
	
	bool tmpIsLeft = m_pHashTree.FindLevel(m_pOwner->GetFileSize(), nPartStartPos, nPartSize, &nLevel);;
	//m_pHashTree.FindHash(nPartStartPos, nPartSize,&nLevel);
	if (nLevel == 0){
		AddDebugLogLineN(logSHAHashSet,
			CFormat(wxT("ReadRecoveryData Get level is 0")));
		//printf("ReadRecoveryData ReadRecoveryData Get level is 0\n");
		return false;
	}
	//set the part tree branch is left or right and hasvalid hash
	SetIsLeftBranchTree(tmpIsLeft);

	AddDebugLogLineN(logSHAHashSet,
		CFormat(wxT("ReadRecoveryData level %d leftflag %d partsize %d")) 
		% nLevel %tmpIsLeft  % nPartSize);
	...
	for (uint32 i = 0; i != nHashsAvailable; i++) {
		...
		if (wHashIdent == 1 /*never allow masterhash to be overwritten*/
			|| !m_pHashTree.SetPartHash(fileDataIn, wHashIdent,(-1), nLevel))			
			//|| !m_pHashTree.SetHash(fileDataIn, wHashIdent,(-1), false))
		{
			AddDebugLogLineN(logSHAHashSet,
				CFormat(wxT("Failed to read RecoveryData for '%s' 16bits id - Error when trying to read hash into tree"))
					% m_pOwner->GetFileName());
...
// TODO:			theApp->QueueDebugLogLine(/*DLP_VERYHIGH,*/ false, _T("Failed to read RecoveryData for %s - Received datasize/amounts of hashs was invalid (2)"), m_pOwner->GetFileName() );
			AddDebugLogLineN(logSHAHashSet,
				CFormat(wxT("Failed to read RecoveryData for '%s' 32bits id - Error when trying to read hash into tree"))
					% m_pOwner->GetFileName());
			return false;
		}
...
// TODO:		theApp->QueueDebugLogLine(/*DLP_VERYHIGH,*/ false, _T("Failed to read RecoveryData for %s - Error when trying to read hash into tree (2)"), m_pOwner->GetFileName() );
				VerifyHashTree(true); // remove invalid hashes which we have already written
				AddDebugLogLineN(logSHAHashSet,
					CFormat(wxT("Failed to read RecoveryData for '%s' 32bits set - Error when trying to read hash into tree"))
						% m_pOwner->GetFileName());
				return false;
			}
		}
...
// TODO:		theApp->QueueDebugLogLine(/*DLP_VERYHIGH,*/ false, _T("Failed to read RecoveryData for %s - Packet didn't contained any hashs"), m_pOwner->GetFileName() );
		AddDebugLogLineN(logSHAHashSet,
			CFormat(wxT("Failed to read RecoveryData for %s - Packet didn't contained any hashs"))
				% m_pOwner->GetFileName());
		return false;
	}
	
	//if (VerifyHashTree(true)) {
	if (VerifyHashPartTree(true)) {
		...
		for (uint32 nPartPos = 0; nPartPos < nPartSize; nPartPos += EMBLOCKSIZE) {
			...
			AddDebugLogLineN(logSHAHashSet, CFormat(_("nPartPos %ld nSize %ld nPartSize %ld"))
					% nPartPos % (min<uint64>(EMBLOCKSIZE, nPartSize-nPartPos)) % nPartSize);
			CAICHHashTree* phtToCheck = m_pHashTree.FindHash(nPartPos, min<uint64>(EMBLOCKSIZE, nPartSize-nPartPos));
		...
		// all done
		AddDebugLogLineN(logSHAHashSet, wxT("ReadRecoveryData check true"));
		return true;
	} else {
		AddDebugLogLineN(logSHAHashSet,
...
bool CAICHHashSet::VerifyHashPartTree(bool bDeleteBadTrees)
{
	CAICHHashAlgo* hashalg = GetNewHashAlgo();
	bool bResult = m_pHashTree.VerifyHashPartTree(hashalg, bDeleteBadTrees);
	delete hashalg;
	return bResult;
}
