...
#if 0 //zengwei mod restrict share number
...
{	
	...
	if (errorOccured) {
		...
		//check the completed file whether in the sharedfilelist already or 
		//reach the max shared number. 
		CKnownFile *kfile = theApp->sharedfiles->GetFileByID(this->GetFileHash());

		if ((theApp->sharedfiles->GetFileCount() < thePrefs::GetMaxDownloadFileNum() || 
			    (kfile != NULL && theApp->sharedfiles->GetFileCount() == thePrefs::GetMaxDownloadFileNum()))
			&& thePrefs::GetMaxDownloadFileNum() > 0){
			AddLogLineC(CFormat(_("Shared file add %s now shared files num %d.")) 
               % GetFileName() % theApp->sharedfiles->GetFileCount());
			theApp->sharedfiles->SafeAddKFile(this);
			theApp->knownfiles->SafeAddKFile(this);
		}else{
			AddLogLineC(CFormat(_("complete shared full file number(%d) is up to max(%d).")) 
               % theApp->sharedfiles->GetFileCount() % thePrefs::GetMaxDownloadFileNum());
			shareFlag = false;		
        ...
        theApp->amuleUbusRpc->AmuleUbusDownloadFin(this->GetFileHash());
#endif
#endif
        // remove the file from the download queue
        theApp->downloadqueue->RemoveFile(this, shareFlag);	
		...
		//republish that file to the ed2k-server to update the 'FT_COMPLETE_SOURCES' counter on the server.
		//reduce that file to the ed2k-server
		theApp->sharedfiles->RepublishFile(this);		
...
void CPartFile::PerformFileComplete()
{
	...
	// close permanent handle
//zengwei del for bug when err happened in <CCompletionTask> 
//leading <CreateNextBlockPackage> recognize part file can read
//but actually m_hpartfile free moved this to <CompleteFileEnded>
//		if (m_hpartfile.IsOpened()) {
//			m_hpartfile.Close();
//		}
...
void CPartFile::FlushBuffer(bool fromAICHRecoveryDataAvailable)
{
	...
	while ( !m_BufferedData_list.empty() ) {
		...
		try {
			...
			AddLogLineC(CFormat(_("Seeking failed: filename(%s) partname(%s) completesize(%ld) offset(%ld) lendata(%d)")) 
				% GetFileName()
				% GetPartMetFileName()
				% GetCompletedSize()
				% item->start
...
void CPartFile::AICHRecoveryDataAvailable(uint16 nPart, CAICHHashTree* pVerifiedHash)
{
	...
	if (pVerifiedHash == NULL){
		AddDebugLogLineC( logAICHRecovery, wxT("Processing AICH Recovery data: Input verified hashset is null") );
		wxFAIL;
		return;
	}
