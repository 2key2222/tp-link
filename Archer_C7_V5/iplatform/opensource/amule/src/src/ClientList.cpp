...
#else
			if (pCurClient->GetSourceFrom() == SF_SOURCE_SEEDS && cur_tick - (uint32)theApp->GetStartTime()< MIN2MS(10)) continue;
#endif
...
bool CClientList::IsClientCreditExist(void* credit)
{
	for (IDMap::iterator current_it = m_clientList.begin(); current_it != m_clientList.end(); current_it++) {
		CUpDownClient* pCurClient = current_it->second.GetClient();
		if (pCurClient->GetCreditsHash() == credit){
			return true;
		}
	}
