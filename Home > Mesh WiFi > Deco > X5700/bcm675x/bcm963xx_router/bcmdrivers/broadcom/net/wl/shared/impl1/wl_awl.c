...
/*******************************************************************************************************
Please try the following checking in wl_awl_rx_sendup() and  see which one can help.

1.
if (!(skb->dev && DEV_PRIV(skb->dev) && WL_DEV_IF(skb->dev) && (WL_DEV_IF(skb->dev))->dev_registered)) {
return PKT_DONE;
}
skb->protocol = eth_type_trans(skb, skb->dev);

2.
if (!(skb->dev && skb->dev->state && netif_device_present(skb->dev)){
return PKT_DONE;
}
skb->protocol = eth_type_trans(skb, skb->dev);
...
*******************************************************************************************************/
	...
	/* function 1 */
	if (!(skb->dev && DEV_PRIV(skb->dev) && WL_DEV_IF(skb->dev) && (WL_DEV_IF(skb->dev))->dev_registered)) {
		printk("%s-%d:we dropped a abnormal pkt when dev is stopped!!!===========================", __func__, __LINE__);
		return PKT_DONE;
	}
#else
	/* function 2 */
	if (!(skb->dev && skb->dev->state && netif_device_present(skb->dev))) {
		printk("%s-%d:we dropped a abnormal pkt when dev is stopped!!!==============002=============", __func__, __LINE__);
		return PKT_DONE;
	}
#endif
