...
_wl_pktfwd_sys_dump(void)
{
    ...
    printk(PKTFWD_VRP_FMT " Dump, radios %u wlifs %u tx_accel %s init %s\n",
...
#if defined(WLCFP)
    if ( likely(cfp_flowid != ID16_INVALID))
    {
...
* 2020-03-27
* 1. cs10071900_patch from BRCM Fred Cai, Fix Bug 354940.
* 2. Fix Details:
*    Station roaming from DHD interface to NIC interface,
*    NIC interface shoud not deleted acceleration entry of DHD interface
*/
bool
wl_pktfwd_wlif_is_reg(d3fwd_wlif_t* wlif)
{
    int dev;
    d3fwd_wlif_t * d3fwd_wlif;
    wl_pktfwd_t  * wl_pktfwd = &wl_pktfwd_g;
    ...
    {
        if ((d3fwd_wlif->unit != ~0) && (d3fwd_wlif == wlif))
            return true;
    }
...
wl_pktfwd_lut_del(uint8_t * d3addr, struct net_device * net_device)
{
    ...
    if (net_device != NULL)
    {
        ...
        if (is_wlan)
        {
            ...
            if (lut_del_eligible && (strncmp(net_device->name, "wds", 3) == 0))
                lut_del_eligible = false; /* exclude WDS endpoints */
...
*    NIC interface shoud not deleted acceleration entry of DHD interface
*/
        /* exclude DHD/WDS endpoints */
        if (d3lut_elem->ext.wlan) { 
            if (!wl_pktfwd_wlif_is_reg(d3lut_elem->ext.d3fwd_wlif)){
                D3LUT_UNLK(wl_pktfwd->d3lut);
                PKTFWD_PRINT("d3fwd_wlif is dhd, skip!");
                return;
            }
        }
/* end add 2020-03-27*/
        ...
        _wl_pktfwd_lut_del(d3addr, d3lut_elem, d3domain);
    }
...
wl_pktfwd_lut_hit(uint8_t * d3addr, struct net_device * net_device)
{
    ...
    if (net_device != NULL)
    {
        ...
        if (is_wlan)
        {
            ...
            if (lut_hit_eligible && (strncmp(net_device->name, "wds", 3) == 0))
                lut_hit_eligible = false; /* exclude WDS endpoints */
...
void
wl_pktfwd_pktqueue_add_pkt(wl_info_t * wl, struct net_device * net_device,
...
wl_pktfwd_upstream(wl_info_t * wl, struct sk_buff * skb)
{
    ...
    FOREACH_CHAINED_PKT(skb, nskb)
    {
        ...
        PKTCCLRFLAGS(skb);
        wl_pktfwd_pktqueue_add_pkt(wl, skb->dev, skb, ID16_INVALID);
    }
