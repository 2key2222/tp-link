...
#define CHANNGED_BSS_NONE 0x00
#define CHANNGED_BSS_WL1  0x01
#define CHANNGED_BSS_WL2  0x02
#define CHANNGED_BSS_WL3  0x04
#define CHANNGED_BSS_WL4  0x08
#define CHANNGED_BSS_WL5  0x10

#define DECO_BSS_NUM 5
#define BIGGER(A, B) ((A)>(B)?(A):(B))
typedef struct deco_bss{
	...
	size_t ssid_len;
	int ignore_broadcast_ssid;
	int auth_algs; /* bitfield of allowed IEEE 802.11 authentication algorithms, WPA_AUTH_ALG_{OPEN,SHARED,LEAP} */
	int wpa;
	int wpa_key_mgmt;
...
typedef struct deco_iface{
	char iface_name[IFNAMSIZ + 1];
	u8 channel;
	u16 beacon_int;
	enum hostapd_hw_mode hw_mode; /* HOSTAPD_MODE_IEEE80211A, .. */
	struct deco_bss deco_bss[DECO_BSS_NUM];
}deco_iface_t;
#endif // endif
