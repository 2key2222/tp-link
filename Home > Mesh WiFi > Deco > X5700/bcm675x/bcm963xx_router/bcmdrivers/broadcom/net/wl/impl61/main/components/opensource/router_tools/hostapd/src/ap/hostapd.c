...
#ifdef SUPPORT_TP_DECO
static void dump_deco_iface(struct deco_iface *deco_conf)
{
	int i;
	wpa_printf(MSG_ERROR, "iface_name %s\n", deco_conf->iface_name);
	wpa_printf(MSG_ERROR, "iface_channel %d\n", deco_conf->channel);
	for (i = 0; i < DECO_BSS_NUM; i++ )
	{
		wpa_printf(MSG_ERROR, "bss %s:\n", deco_conf->deco_bss[i].bss_name);
		wpa_printf(MSG_ERROR, "\t ssid %s\n", deco_conf->deco_bss[i].ssid);
		wpa_printf(MSG_ERROR, "\t auth_algs %d\n", deco_conf->deco_bss[i].auth_algs);
		wpa_printf(MSG_ERROR, "\t wpa %d\n", deco_conf->deco_bss[i].wpa);
		if (deco_conf->deco_bss[i].wpa != 0)
		{
			wpa_printf(MSG_ERROR, "\t wpa_key_mgmt %d\n", deco_conf->deco_bss[i].wpa_key_mgmt);
			wpa_printf(MSG_ERROR, "\t wpa_pairwise %d\n", deco_conf->deco_bss[i].wpa_pairwise);
			wpa_printf(MSG_ERROR, "\t wpa_passphrase %s\n", deco_conf->deco_bss[i].wpa_passphrase);
		}
	}
}

static int get_deco_iface(struct hostapd_config *hapd_conf, struct deco_iface *deco_iface_conf)
{
	int i = 0;
	wpa_printf(MSG_ERROR, "-->enter get_deco_iface %s, bcn_int %d\n", hapd_conf->bss[0]->iface, hapd_conf->beacon_int);
	deco_iface_conf->beacon_int = hapd_conf->beacon_int;
	deco_iface_conf->channel = hapd_conf->channel;
	deco_iface_conf->hw_mode = hapd_conf->hw_mode;
	strncpy(deco_iface_conf->iface_name, hapd_conf->bss[0]->iface, IFNAMSIZ);
	for (i = 0; i < hapd_conf->num_bss && i < DECO_BSS_NUM ; i++ )
	{
		strncpy(deco_iface_conf->deco_bss[i].bss_name, hapd_conf->bss[i]->iface, IFNAMSIZ);
		strncpy(deco_iface_conf->deco_bss[i].ssid, hapd_conf->bss[i]->ssid.ssid, SSID_MAX_LEN);
		deco_iface_conf->deco_bss[i].ssid_len = hapd_conf->bss[i]->ssid.ssid_len;
		deco_iface_conf->deco_bss[i].ignore_broadcast_ssid = hapd_conf->bss[i]->ignore_broadcast_ssid;
		deco_iface_conf->deco_bss[i].dtim_period = hapd_conf->bss[i]->dtim_period;
		deco_iface_conf->deco_bss[i].auth_algs = hapd_conf->bss[i]->auth_algs;
		deco_iface_conf->deco_bss[i].wpa = hapd_conf->bss[i]->wpa;
		if (hapd_conf->bss[i]->wpa != 0)
		{
			deco_iface_conf->deco_bss[i].wpa_key_mgmt = hapd_conf->bss[i]->wpa_key_mgmt;
			deco_iface_conf->deco_bss[i].wpa_pairwise = hapd_conf->bss[i]->wpa_pairwise;
			if (hapd_conf->bss[i]->ssid.wpa_passphrase == NULL)
			{
				strncpy(deco_iface_conf->deco_bss[i].psk, hapd_conf->bss[i]->ssid.wpa_psk->psk, PMK_LEN*2);
				deco_iface_conf->deco_bss[i].wpa_passphrase = os_strdup("0");
			}else{
				deco_iface_conf->deco_bss[i].wpa_passphrase = os_strdup(hapd_conf->bss[i]->ssid.wpa_passphrase);
				memset(deco_iface_conf->deco_bss[i].psk, 0, PMK_LEN * 2);
			}
		}
		if (hapd_conf->bss[i]->mobility_domain[0] == '\0')
			deco_iface_conf->deco_bss[i].dot11r_enable = 0;
		else
			deco_iface_conf->deco_bss[i].dot11r_enable = 1;
	}
	return 0;
}
 ...
 *@return: 0 is
*/
static u16 deco_iface_conf_changed(struct deco_iface *new_conf, struct deco_iface *old_conf, int *password_changed)
{
	u16 status = 0;
	int i = 0;
	if ((new_conf->beacon_int != old_conf->beacon_int) ||
		(new_conf->hw_mode != old_conf->hw_mode))
	{
		status |= CHANNGED_BSS_WL1 | CHANNGED_BSS_WL2 | CHANNGED_BSS_WL3 | CHANNGED_BSS_WL4 | CHANNGED_BSS_WL5;
		return status;
	}
	for (i = 0; i < DECO_BSS_NUM; i++ )
	{
		if (strncmp(new_conf->deco_bss[i].bss_name, old_conf->deco_bss[i].bss_name, IFNAMSIZ))
		{
			wpa_printf(MSG_ERROR, "-->!!!bss name mismatch, why? need reload all bss!!\n");
			status = CHANNGED_BSS_WL1 | CHANNGED_BSS_WL2 | CHANNGED_BSS_WL3 | CHANNGED_BSS_WL4 | CHANNGED_BSS_WL5;
			return status;
		}
		if ( new_conf->deco_bss[i].ssid_len != old_conf->deco_bss[i].ssid_len ||
			(strncmp(new_conf->deco_bss[i].ssid, old_conf->deco_bss[i].ssid, new_conf->deco_bss[i].ssid_len)))
		{
			wpa_printf(MSG_INFO, "-->bss %d ssid changed, new:%s, old: %s\n", i,new_conf->deco_bss[i].ssid, old_conf->deco_bss[i].ssid);
			status |= 1<<i;
			continue;
		}

		if (new_conf->deco_bss[i].ignore_broadcast_ssid != old_conf->deco_bss[i].ignore_broadcast_ssid)
		{
			status |= 1<<i;
		}

		if (new_conf->deco_bss[i].wpa == old_conf->deco_bss[i].wpa)
		{
			if (new_conf->deco_bss[i].wpa != 0)
			{
				if (strncmp(new_conf->deco_bss[i].wpa_passphrase, old_conf->deco_bss[i].wpa_passphrase, BIGGER(strlen(new_conf->deco_bss[i].wpa_passphrase), strlen(old_conf->deco_bss[i].wpa_passphrase))))
				{
					wpa_printf(MSG_ERROR, "-->bss %d passphrase changed, new:%s, old:%s\n", i, new_conf->deco_bss[i].wpa_passphrase, old_conf->deco_bss[i].wpa_passphrase);
					status |= 1<<i;
					(*password_changed) = 1;
					continue;
				}
				if (strncmp(new_conf->deco_bss[i].psk, old_conf->deco_bss[i].psk, PMK_LEN * 2))
				{
					status |= 1<<i;
					(*password_changed) = 1;
					continue;
				}
				if ((new_conf->deco_bss[i].wpa_key_mgmt != old_conf->deco_bss[i].wpa_key_mgmt) ||
					(new_conf->deco_bss[i].wpa_pairwise != old_conf->deco_bss[i].wpa_pairwise))
				{
					wpa_printf(MSG_ERROR, "-->bss %d wpa_key_mgmt or wpa_pairwise changed, new:%d, old:%d\n", i, new_conf->deco_bss[i].wpa_key_mgmt, old_conf->deco_bss[i].wpa_pairwise);
					status |= 1<<i;
					continue;
				}
			}
		...
		{
			/*wpa version changed, reload bss*/
			status |= 1<<i;
			continue;
		}

		if ((new_conf->deco_bss[i].dtim_period != old_conf->deco_bss[i].dtim_period) ||
			(new_conf->deco_bss[i].auth_algs != old_conf->deco_bss[i].auth_algs) ||
			(new_conf->deco_bss[i].dot11r_enable != old_conf->deco_bss[i].dot11r_enable))
		{
			status |= 1<<i;
		}
	}
...
int hostapd_reload_config(struct hostapd_iface *iface)
{
...
#ifdef SUPPORT_TP_DECO
	struct deco_iface old_iface_param, new_iface_param;
	int deco_conf_state = 0;
	...
	iface->conf = newconf;
#ifdef SUPPORT_TP_DECO
	get_deco_iface(oldconf, &old_iface_param);
	get_deco_iface(newconf, &new_iface_param);
	//dump_deco_iface(&old_iface_param);
	//dump_deco_iface(&new_iface_param);
	deco_conf_state = deco_iface_conf_changed(&new_iface_param, &old_iface_param, &password_changed);
#endif // endif

	for (j = 0; j < iface->num_bss; j++) {
#ifdef SUPPORT_TP_DECO
		if (old_iface_param.deco_bss[j].wpa != 0)
		{
			os_free(old_iface_param.deco_bss[j].wpa_passphrase);
		}
		if (new_iface_param.deco_bss[j].wpa != 0)
		{
			os_free(new_iface_param.deco_bss[j].wpa_passphrase);
		}
#endif // endif
...
#ifdef SUPPORT_TP_DECO
		if (deco_conf_state & (1<<j))
		{
			wpa_printf(MSG_ERROR, "bss %d need reload for state  0x%x\n", j, deco_conf_state);
			/*
	 		 * Deauthenticate all stations since the new configuration may not
			 * allow them to use the BSS anymore.
			 */
			...
			if (1 == password_changed)
			{
				hostapd_ctrl_iface_pmksa_flush(iface->bss[j]);
			}
		}
#endif // endif
...
#ifdef SUPPORT_TP_DECO
	cmd_len = strlen("bss") + 1;
	buf_len = cmd_len + sizeof(setbuf);
	buf = os_zalloc(buf_len);
	if (buf == NULL) {
		wpa_printf(MSG_DEBUG, "BRCM: Failed to allocate memory for bss ioclt\n");
		return -1;
	}
#endif // endif
	for (j = 0; j < iface->num_bss; j++) {
...
#ifdef SUPPORT_TP_DECO
		/*atfer initialized bss, set wl bss default to down, the tplink_brcm.sh or wifiupdate.sh will handle interfaces down/up  --by tongqiujie*/
		memset(buf, '\0', buf_len);
		setbuf.bsscfg_idx = j + 1;   /*ap bsscfg_idx is start from 1*/
		setbuf.enable = 0;
		memcpy(buf, "bss", cmd_len);
		memcpy(buf + cmd_len, &setbuf, sizeof(setbuf));
		wpa_printf(MSG_ERROR, "call ioctl to set bss %s idx %d down", hapd->conf->iface, j + 1);
		wl_ioctl(hapd->conf->iface, WLC_SET_VAR, buf, buf_len);
#endif // endif
