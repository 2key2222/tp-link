...
static int hostapd_wps_set_credential(struct hostapd_data *hapd,
				      struct wps_context *wps)
{
	wps->ssid_len = hapd->conf->ssid.ssid_len;
	os_memcpy(wps->ssid, hapd->conf->ssid.ssid, hapd->conf->ssid.ssid_len);
	if (hapd->conf->wpa & WPA_PROTO_RSN) {
		...
		if (hapd->conf->rsn_pairwise & (WPA_CIPHER_CCMP | WPA_CIPHER_GCMP |
					  WPA_CIPHER_CCMP_256 |
					  WPA_CIPHER_GCMP_256)) {
	...
	if (hapd->conf->ssid.security_policy == SECURITY_PLAINTEXT) {
		wps->encr_types |= WPS_ENCR_NONE;
		wps->auth_types |= WPS_AUTH_OPEN;
	}

	if (hapd->conf->ssid.wpa_psk_file) {
		/* Use per-device PSKs */
	} else if (hapd->conf->ssid.wpa_passphrase) {
		wps->network_key = (u8 *) os_strdup(hapd->conf->ssid.wpa_passphrase);
		wps->network_key_len = os_strlen(hapd->conf->ssid.wpa_passphrase);
	} else if (hapd->conf->ssid.wpa_psk) {
	...
	} else if (hapd->conf->ssid.wep.keys_set && hapd->conf->ssid.wep.key[0]) {
		/*WPS 2.0 DO NOT support WEP*/
		wps->network_key = os_malloc(hapd->conf->ssid.wep.len[0]);
		if (wps->network_key == NULL)
			return -1;
		os_memcpy(wps->network_key, hapd->conf->ssid.wep.key[0],
			  hapd->conf->ssid.wep.len[0]);
		wps->network_key_len = hapd->conf->ssid.wep.len[0];
	}

	if (hapd->conf->ssid.wpa_psk) {
		os_memcpy(wps->psk, hapd->conf->ssid.wpa_psk->psk, PMK_LEN);
		wps->psk_set = 1;
	}
	return 0;
...
#endif /* CONFIG_WPS_UPNP */
	wpa_printf(MSG_INFO, "WPS: update wps credential when bss reload");
	hostapd_wps_set_credential(hapd, hapd->wps);
