...
/* wan dev for qos should use dev->priv_flags & IFF_WANDEV */
//#if defined(CONFIG_TP_BCM_QOS_PATCH) && defined(CONFIG_WAN_DETECTION)
//#define DEV_ISWAN(dev) (dev ? (dev->priv_wan_flags & IFF_WAN_DEV) : 0)
//#else
//#define DEV_ISWAN(dev) (dev ? (dev->priv_flags & IFF_WANDEV) : 0)
//#endif

#if defined(CONFIG_TP_BCM_QOS_PATCH)
#define DEV_ISWAN(dev) (dev ? (dev->priv_flags & IFF_WANDEV) : 0)
#endif
...
#if defined(CONFIG_TP_MULTI_NET_RX_THREAD)
static uint32 bcm63xx_rx_process(void *ptr, int budget);
#endif
...
#if defined(CONFIG_TP_MULTI_NET_RX_THREAD)
    BCMENET_WAKEUP_DSP_WORKER(chan);
#else
...
#if defined(CONFIG_TP_AVIRA_PATCH)
    if(BROADSTREAM_AVIRA_ENABLE() && !(dev->priv_flags & IFF_WANDEV))
    {
        local_bh_disable();
        blog_action = netif_blog_check(skb);
        local_bh_enable();
        switch(blog_action) {
        case PKT_BLOG:
            break;
        ...
        /* Notice: Set flag for enqueue after blog */
        PKTSETFCDONE(skb);
        blog_action = blog_sinit_args(skb, skb->dev, TYPE_ETH, port->n.set_channel_in_mark ? rx_info->flow_id :
        ...
        {
            INC_STAT_DBG(port,rx_packets_blog_done);
            bcm_tcp_v4_recv((void*)CAST_REAL_TO_VIRT_PNBUFF(skb,SKBUFF_PTR), &fc_args);
            goto done;
        }
...
static inline int rx_pkt_from_q(int hw_q_id, int budget)
{
    ...
    do
    {
        ...
        if((dev->priv_flags & IFF_WANDEV)
            || (!BROADSTREAM_AVIRA_ENABLE() && PKT_BLOG == fkb_pctl_check(fkb)))
#elif defined(CONFIG_TP_FC_PCTL_SUPPORT)            
        if((dev->priv_flags & IFF_WANDEV)
            || (PKT_BLOG == fkb_pctl_check(fkb)))
#endif
        {
...
#if defined(CONFIG_TP_AVIRA_PATCH) || defined(CONFIG_TP_FC_PCTL_SUPPORT)
            /* 
             * for xt_pctl support 
             * set is_rx_hw_acc_en 1 by deafult
             * set is_rx_hw_acc_en 0 on demand by xt_pctl
             */
            if (blog_action == PKT_BLOG && !(dev->priv_flags & IFF_WANDEV) && port->n.blog_phy == BLOG_ENETPHY)
                fkb->blog_p->wl_hw_support.is_rx_hw_acc_en = 1;
#endif
...
struct MULTI_RX_PACKETS
{
	...
	FkBuff_t * pFkb;
	uint32_t blog_chnl;
	uint32_t blog_phy; /* used if CONFIG_BLOG enabled */
    enetx_rx_info_t rx_info;
    enetx_port_t *port;
	int is_wifi_port;
#endif
...
static int rx_pkt_cnt = 0;

#define RING_BUF_NEXT(pos)	((pos + 1) & ( MULTI_RX_BUFFER_NUM - 1))
#define RING_BUF_NUM(head, tail) ( (head>=tail)?(head-tail):(head+MULTI_RX_BUFFER_NUM-tail))

static uint32 multi_head;
static uint32 multi_tail; 
spinlock_t multi_lock;
static struct MULTI_RX_PACKETS multi_packets[MULTI_RX_BUFFER_NUM];

static uint32 bcm63xx_rx_process(void *ptr, int budget)
{
    struct sk_buff *skb = NULL;
    struct MULTI_RX_PACKETS *pkt;
    
    uint32 head = 0;
    uint32 tail = 0;

    int count = 0;
    int ret = MULTI_RX_CONSUME_ROUND_DONE;
    int idx = 0;
    
    struct MULTI_RX_PACKETS pkts[NETDEV_WEIGHT];// = {0};

    spin_lock(&multi_lock);
    while(count < NETDEV_WEIGHT) {
        
		head = ACCESS_ONCE(multi_head);
		tail = multi_tail;

		/* no more packets in ring */
		if (head == tail) {
                        spin_unlock(&multi_lock);
			ret = MULTI_RX_CONSUME_ALL;
                        goto PKT_PROCESS;
                }

		pkts[count++].skb = multi_packets[tail].skb;
                multi_tail = RING_BUF_NEXT(tail);        
    }
    spin_unlock(&multi_lock);
...
PKT_PROCESS:
    ...
    if(count) {
            ...
            for(idx = 0; idx < count; idx++) {
                        ...
                        //printk("%s-%d:we get a packet deliver to linux network stack now\r\n", __func__, __LINE__);
                        if(skb) {
                            local_bh_disable();
#if defined(BCM_EAPFWD)
                            if (eap_receive_skb_hook != NULL)
        ...
        if(ret == MULTI_RX_CONSUME_ALL) {
            ret = MULTI_RX_CONSUME_ROUND_DONE;
            /* no more packets in ring */
            if (ACCESS_ONCE(multi_head) == multi_tail) {
                ret = MULTI_RX_CONSUME_ALL;
            }
        }
    }
...
static inline int chan_rx_skb(FkBuff_t *fkb, enetx_port_t *port, enetx_rx_info_t *rx_info, struct MULTI_RX_PACKETS* rx_pkts)
{
    struct net_device *dev = port->dev;
    struct sk_buff *skb;

    struct MULTI_RX_PACKETS *pkt = NULL;
...
static inline int chan_rx_pkt_from_q(int hw_q_id, int budget, struct MULTI_RX_PACKETS* rx_pkts)
{
    int rc, count = 0;
    enetx_port_t *port;
    FkBuff_t *fkb;
    ...
    do
    {
        ...
        rc = chan_rx_skb(fkb, port, &rx_info, rx_pkts);
        count++;
unlock:
        rcu_read_unlock();
    }
...
static inline int chan_rx_pkt(enetx_channel *chan, int budget)
{
    int i, rc , count;
    uint32 head = 0;
    uint32 tail = 0;
    ...
    struct MULTI_RX_PACKETS *pkt = NULL;
    struct MULTI_RX_PACKETS rx_pkts[NETDEV_WEIGHT] = {0};
    rx_pkt_cnt = 0;
    ...
    for (count = 0, i = 0; i < chan->rx_q_count && count < budget; i++)
    {
        ...
        rc = chan_rx_pkt_from_q(chan->rx_q[i], budget - count, rx_pkts);
        count += rc;
...
#if 0
	if (fetch_ret == MULTI_RX_PRODUCE_SRC_EMPTY) {
            chan->rx_dispatch_avail = 0;
	}
	else if (fetch_ret == MULTI_RX_PRODUCE_DST_FULL) {
            chan->rx_dispatch_avail = 0;
            break;
	}
#endif
    ...
    spin_lock(&multi_lock);
    for(i = 0; i < rx_pkt_cnt && rx_pkts[i].skb; i++) {
        
        head = multi_head;
        tail = ACCESS_ONCE(multi_tail);
    
        /* ring buffer full */
        if (tail == RING_BUF_NEXT(head)) {
            spin_unlock(&multi_lock);
            goto done;
        }
    
        pkt = & multi_packets[head];
        pkt->skb = rx_pkts[i].skb;
        multi_head = RING_BUF_NEXT(head);
    }
    spin_unlock(&multi_lock);
    ...
    if (count) {
        for(i = 0; i < MULTI_RX_THREAD_NUM; i++) {
            if(chan->rx_consumer_thread[i].rx_consumer_avail == 0) {
                set_bit(0, &(chan->rx_consumer_thread[i].rx_consumer_avail));
                wake_up_interruptible(&(chan->rx_consumer_thread[i].rx_consumer_wqh));                
            }
        }
    }
...
int chan_rx_dispatcher_thread(void *data)
{
    int work = 0;
    int reschedule;
    int i;
    ...
    while (1)
    {
        wait_event_interruptible(chan->rx_dispatcher_wqh, chan->rx_dispatcher_avail | kthread_should_stop());

        /*read budget from all queues of the channel*/
        work += chan_rx_pkt(chan, enetx_weight_budget);
        reschedule = 0;
        ...
        if (work < enetx_weight_budget)
        {
            ...
            {
                work = 0;
                clear_bit(0, &chan->rx_dispatcher_avail);
                for (i = 0; i < chan->rx_q_count; i++)
                {
                    enetxapi_queue_int_enable(chan, i);
                }
...
int chan_rx_consumer_thread(void *data)
{
    multi_rx_thread *rx_thread = (multi_rx_thread *)data;
    uint32 ret;
    ...
    while (1)
    {
        wait_event_interruptible(rx_thread->rx_consumer_wqh, rx_thread->rx_consumer_avail | kthread_should_stop());

        ret = bcm63xx_rx_process(rx_thread->chan, budget);
		
		/* consumed all packets, 
		 * consumer thread can be sceduled out now, 
		 * dispatcher thead wake up it once again when any new packets in the ring buffer.
		 */
        if (ret == MULTI_RX_CONSUME_ALL) {
            //printk("%s-%d:we start to sleep here how\r\n", __func__, __LINE__);
            rx_thread->rx_consumer_avail = 0;
        }

		//smp_mb();

		/* here wake up producer thread unconditionly
		 * the producer thread may sleep because of ring buffer full,
		 * but also, At the point of sleep, ring may become full once again. 
		 * Producer will check that and wake up me.
		 */
		BCMENET_WAKEUP_DSP_WORKER(rx_thread->chan);
...
#endif /* CONFIG_TP_MULTI_NET_RX_THREAD */
...
#if defined(CONFIG_TP_AVIRA_PATCH)
    /*
     * BROADSTREAM_QOS_TC_ON: TM or qos_v2 on
     * BROADSTREAM_IQOS_ENABLE: TM or qos_v2 or avira on
     */
    if (BROADSTREAM_QOS_TC_ON() && pNBuff && DEV_ISWAN(dev)) {
#else
	/* If Broadstream iqos enable, for WAN egress packets, need to call dev_queue_xmit */
    if (BROADSTREAM_IQOS_ENABLE() && pNBuff && DEV_ISWAN(dev)) { //(dev->priv_flags & IFF_WAN_DEV)
#endif
...
#if 0
                /*Hanjiayan@20190202 modified, when IQOS enable, wlan -> lan package need call blog_emit to enable its blog*/
                if(DEV_ISWAN(dev)){ //dev->priv_flags & IFF_WAN_DEV
                    if(!PKTISFKBTOQMIT(skb))
                        blog_emit(pNBuff, dev, TYPE_ETH, port->n.set_channel_in_mark ? dispatch_info.channel : port->n.blog_chnl, port->n.blog_phy);
                }else if(!DEV_ISWAN(((struct net_device *)(blog_ptr(PNBUFF_2_SKBUFF(pNBuff))->rx_dev_p))))
                    blog_emit(pNBuff, dev, TYPE_ETH, port->n.set_channel_in_mark ? dispatch_info.channel : port->n.blog_chnl, port->n.blog_phy);            
#else

                    if (((dev->priv_flags & IFF_WANDEV) && !PKTISFKBTOQMIT(skb)) ||
                        !DEV_ISWAN(((struct net_device *)(PNBUFF_2_SKBUFF(pNBuff)->blog_p->rx_dev_p))))
                        blog_emit(pNBuff, dev, TYPE_ETH, port->n.set_channel_in_mark ? dispatch_info.channel : port->n.blog_chnl, port->n.blog_phy);            
#endif
            }
            else
...
void _enet_dev_role_update(enetx_port_t *self, int first_time)
{
    ...
    /* don't set mac as netifd will handle it, wanglian, 29Mar19 */ 
