...
#ifdef CONFIG_TP_MULTI_NET_RX_THREAD

#define MULTI_RX_BUFFER_NUM     1024
#define NETDEV_WEIGHT 32

#define MULTI_RX_THREAD_NUM 2

#define MULTI_RX_DISPATCHER_THREAD_NAME   "bcmsw_dsp"

#define MULTI_RX_CONSUMER_THREAD_NAME   "bcmsw_rx"

// small buffer will cause more thread schedule

enum 
{
	MULTI_RX_PRODUCE_ROUND_DONE = 0,
	MULTI_RX_PRODUCE_DST_FULL,
	MULTI_RX_PRODUCE_SRC_EMPTY,
};
	
enum
{
	MULTI_RX_CONSUME_ROUND_DONE = 0,	/* more packets */
	MULTI_RX_CONSUME_ALL,				/* no more packets */
};

struct _multi_rx_thread;
...
typedef struct _enetx_channel {
    ...
    struct task_struct *rx_dispatcher_thread;
    wait_queue_head_t   rx_dispatcher_wqh;
    volatile unsigned long             rx_dispatcher_avail;

    struct _multi_rx_thread* rx_consumer_thread;
...
struct _multi_rx_thread {
    struct task_struct *rx_consumer_thread;
    wait_queue_head_t   rx_consumer_wqh;
    volatile unsigned long             rx_consumer_avail;
    enetx_channel *chan;
};

typedef struct _multi_rx_thread multi_rx_thread;
...
#define BCMENET_WAKEUP_DSP_WORKER(x) do { \
			if ((x)->rx_dispatcher_avail == 0) { \
				set_bit(0, &((x)->rx_dispatcher_avail)); \
				wake_up_interruptible(&((x)->rx_dispatcher_wqh)); \
			} \
	} while (0)

#if 0
#define BCMENET_WAKEUP_RXWORKER_0(x) do { \
			if ((x)->rx_consumer_avail_0 == 0) { \
				set_bit(0, &((x)->rx_consumer_avail_0)); \
				wake_up_interruptible(&((x)->rx_consumer_wqh_0)); \
			} \
	} while (0)

#define BCMENET_WAKEUP_RXWORKER_1(x)	do { \
	if ( (x)->rx_consumer_avail_1 == 0 )	{ \
		set_bit(0, &((x)->rx_consumer_avail_1)); \
		wake_up_interruptible(&((x)->rx_consumer_wqh_1)); }} while (0)
#endif
#endif

#if defined(CONFIG_TP_MULTI_NET_RX_THREAD)
#define MAX_BUFFERS_IN_RING_CACHE   MULTI_RX_BUFFER_NUM
#endif
