...
extern spinlock_t multi_lock;
extern int chan_rx_dispatcher_thread(void *data);
extern int chan_rx_consumer_thread(void *data);
#endif
...
int enetxapi_queues_init (enetx_channel ** _chan)
{
    ...
    *_chan = chan;
        ...
        chan->rx_dispatcher_avail = 0;
        init_waitqueue_head(&chan->rx_dispatcher_wqh);
        chan->rx_dispatcher_thread = kthread_create(chan_rx_dispatcher_thread, chan, MULTI_RX_DISPATCHER_THREAD_NAME);        
        wake_up_process(chan->rx_dispatcher_thread);

        chan->rx_consumer_thread = kzalloc(MULTI_RX_THREAD_NUM * sizeof(multi_rx_thread), GFP_KERNEL);
        if(!chan->rx_consumer_thread) {
            kfree(chan);
            return -1;
        }

        for(idx = 0; idx < MULTI_RX_THREAD_NUM; idx++) {
            chan->rx_consumer_thread[idx].rx_consumer_avail = 0;
            init_waitqueue_head(&chan->rx_consumer_thread[idx].rx_consumer_wqh);
            chan->rx_consumer_thread[idx].chan = chan;
            snprintf(multi_rx_name, sizeof(multi_rx_name) -1, "%s%d", MULTI_RX_CONSUMER_THREAD_NAME, idx);
            chan->rx_consumer_thread[idx].rx_consumer_thread = kthread_create(chan_rx_consumer_thread, 
                &(chan->rx_consumer_thread[idx]), multi_rx_name);
            
            wake_up_process(chan->rx_consumer_thread[idx].rx_consumer_thread);
        }

        //multirx_add_proc_file();
#else
