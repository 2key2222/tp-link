...
static const struct snd_pcm_hardware bcm63xx_pcm_hardware = {
   ...
   .formats = SNDRV_PCM_FMTBIT_S16_LE,/* support S16 only */
...
static int bcm63xx_pcm_prepare(struct snd_pcm_substream *substream)
{
   ...
   if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) 
   {
      pdma_desc[STREAM_PLAYBACK]->dma_len  = snd_pcm_lib_period_bytes(substream);
      pdma_desc[STREAM_PLAYBACK]->dma_addr = runtime->dma_addr;
      pdma_desc[STREAM_PLAYBACK]->dma_area = runtime->dma_area;

      regmap_write(regmap_i2s, I2S_TX_DESC_IFF_LEN, pdma_desc[STREAM_PLAYBACK]->dma_len * 2);
      regmap_write(regmap_i2s, I2S_TX_DESC_IFF_ADDR, runtime->dma_addr + substream->dma_buffer.bytes);
   }
   else
   {
      pdma_desc[STREAM_CAPTURE]->dma_len  = snd_pcm_lib_period_bytes(substream);
      pdma_desc[STREAM_CAPTURE]->dma_addr = runtime->dma_addr;
      pdma_desc[STREAM_CAPTURE]->dma_area = runtime->dma_area;

      regmap_write(regmap_i2s, I2S_RX_DESC_IFF_LEN, pdma_desc[STREAM_CAPTURE]->dma_len * 2);
      regmap_write(regmap_i2s, I2S_RX_DESC_IFF_ADDR, runtime->dma_addr + substream->dma_buffer.bytes);
...
static irqreturn_t i2s_dma_isr(int irq, void *snd_pcm_array)
{
   ...
   if( int_status & I2S_RX_DESC_OFF_INTR )
   {
     ...
     {
         printk(KERN_ERR "invalid capture streams.\n");
         return IRQ_NONE;
     }
     ...
     while( offlevel )
     {
        ...
        //unpadding zeros to 16 bits.
        {
            unsigned short *q = (unsigned short *)(runtime->dma_area + (val_1 - runtime->dma_addr));
            unsigned short *p = (unsigned short *)(runtime->dma_area + (val_1 - runtime->dma_addr - substream->dma_buffer.bytes)/2);
            int i, j;
            for(i = 0, j = 0; j < val_2/sizeof(unsigned short); i+=2, j+=4) {
                p[i] = q[j + 1];
                p[i+1] = q[j + 3];
            }
        }
     ...
     prtd->dma_addr_next = runtime->dma_addr + (val_1 + val_2 - substream->dma_buffer.bytes - runtime->dma_addr)/2;
     ...
     while( availdepth )
     {
         ...
         regmap_write(regmap_i2s, I2S_RX_DESC_IFF_LEN,
                                  snd_pcm_lib_period_bytes(substream) * 2);//pushhw 
         regmap_write(regmap_i2s, I2S_RX_DESC_IFF_ADDR, (prtd->dma_addr - runtime->dma_addr)*2 + runtime->dma_addr + substream->dma_buffer.bytes);
         availdepth--;
     }
   ...
   /* Clear RX interrupt by writing 0 */
   regmap_update_bits(regmap_i2s, I2S_RX_IRQ_CTL,I2S_INTR_MASK & int_status , 0 );
   ...
   if( int_status & I2S_DESC_OFF_INTR )
   {
      ...
      while( offlevel )
      {
         ...
         prtd->dma_addr_next = (val_1 + val_2 - substream->dma_buffer.bytes - runtime->dma_addr)/2 + runtime->dma_addr;
         offlevel--;  
      }
      ...
      while( availdepth )
      {
         ...
         //padding zero to 32 bits.
         {
             unsigned short *p = (unsigned short *)(runtime->dma_area + prtd->dma_addr - runtime->dma_addr);
             unsigned short *q = (unsigned short *)(runtime->dma_area + substream->dma_buffer.bytes + (prtd->dma_addr - runtime->dma_addr) * 2);
             int i, j;
             for(i = 0, j = 0; i < snd_pcm_lib_period_bytes(substream)/sizeof(unsigned short); i+=2, j+=4) {
                 q[j] = 0;
                 q[j+1] = p[i];
                 q[j+2] = 0;
                 q[j+3] = p[i+1];
         ...
         regmap_write(regmap_i2s, I2S_TX_DESC_IFF_LEN,
                               snd_pcm_lib_period_bytes(substream) * 2);//push->hw
         regmap_write(regmap_i2s, I2S_TX_DESC_IFF_ADDR, (prtd->dma_addr - runtime->dma_addr)*2 + runtime->dma_addr + substream->dma_buffer.bytes);
   ...
   /* Clear TX interrupt by writing 0 */
   regmap_update_bits(regmap_i2s, I2S_TX_IRQ_CTL,I2S_INTR_MASK & int_status , 0 );
...
static int bcm63xx_pcm_preallocate_dma_buffer(struct snd_pcm *pcm, int stream)
{
   ...
   buf->area         = dma_alloc_writecombine(pcm->card->dev, size * 3,&buf->addr,
                                              GFP_KERNEL);
   if (!buf->area)
   {
     return -ENOMEM;
...
void bcm63xx_pcm_free_dma_buffers(struct snd_pcm *pcm)
{
   ...
   for (stream = 0; stream < 2; stream++) 
   {
      ...
      if (!buf->area)
         continue;
      dma_free_writecombine(pcm->card->dev, buf->bytes * 3,    buf->area, buf->addr);
      buf->area = NULL;
   }
}
