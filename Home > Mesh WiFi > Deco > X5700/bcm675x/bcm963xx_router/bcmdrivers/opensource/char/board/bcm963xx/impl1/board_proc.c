...
#if defined(CONFIG_TP_IMAGE)
#if defined(CONFIG_TP_MODEL_VOICE_X20V1)
static ssize_t proc_get_speaker_en(struct file *file, char __user *buff, size_t len, loff_t *offset)
{
    int r = 0;
    unsigned short gpio;
    ...
    if ( BpGetSpeakerEnGpio( &gpio ) != BP_SUCCESS ) {
        printk("\nDon't support speaker_en gpio\n");
        return -EFAULT;
    }

    gpio_state = bcm_gpio_get_data(gpio);

    if (((gpio & BP_ACTIVE_LOW) && (gpio_state == 1)) ||
        (!(gpio & BP_ACTIVE_LOW) && (gpio_state == 0))) {
        gpio_state = 0;
    } else {
        gpio_state = 1;
    }
...
static ssize_t proc_set_speaker_en(struct file *f, const char *buf, size_t cnt, loff_t *data)
{
    char __in[8] = {0};
    unsigned short gpio;
    unsigned short gpio_state;
...
#if defined(CONFIG_TP_IMAGE)
#if defined(CONFIG_TP_MODEL_VOICE_X20V1)
    static struct file_operations speaker_en_proc = {
       .read=proc_get_speaker_en,
       .write=proc_set_speaker_en,
    };
#endif
#endif
...
#if defined(CONFIG_TP_IMAGE)
#if defined(CONFIG_TP_MODEL_VOICE_X20V1)
    p1 = proc_create("speaker_en", S_IWUSR | S_IRUSR, NULL, &speaker_en_proc);
    if (p1 == NULL)
        return -1;
#endif
#endif
