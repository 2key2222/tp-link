...
static int button_hotplug_fill_event(struct bh_event *event)
{
	...
	ret = bh_event_add_var(event, 0, "SEEN=%ld", event->time);
	if (ret)
		return ret;

	ret = bh_event_add_var(event, 0, "SEQNUM=%llu", (unsigned long long)(-1));
	if (ret)
		return ret;
...
static void button_hotplug_work(struct bh_event *event)
{
...
static int button_hotplug_create_event(const char *name, int pressed, unsigned long timeInMs)
{
	struct bh_event *event = NULL;
	...
	button_hotplug_work(event);

	return 0;
}

void btnHook_HotplugEvent(unsigned long timeInMs, void* param) {
	char name[16] = {0};
	char op[16] = {0};
	int pressed = 0;
	...
	sscanf((char*)param, "%s %s", name, op);
	//printk("name: %s op:%s\n", name, op);

	// do event broadcast
	if (strcmp("pressed", op) == 0)
	{
		pressed = 1;
	} 
	else if (strcmp("released", op) == 0)
	{
		pressed = 0;
	}
	else {
	...
	button_hotplug_create_event(name, pressed, timeInMs);
...
#if defined(CONFIG_TP_IMAGE)
	[BP_BTN_ACTION_HOTPLUG_EVENT>> BP_BTN_ACTION_SHIFT] 	= (pushButtonNotifyHook_t)btnHook_HotplugEvent,
#endif
