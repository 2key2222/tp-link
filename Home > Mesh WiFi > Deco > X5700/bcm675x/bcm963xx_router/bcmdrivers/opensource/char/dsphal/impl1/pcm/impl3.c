...
# ifdef BRCM_NTR_SUPPORT
   ...
   bcm_set_pinmux(11, PINMUX_6);
   TOP->DslClockSample &= PCM_NTR_CTRL_CLEAR;
   TOP->DslClockSample |= (PCM_NTR_IN_CTRL_SEL_VDSL_NTR_0 << PCM_NTR_IN_CTRL_SHIFT);   
   TOP->DslClockSample |= (1 << 31)| (1 << 28) | 7;
#else   
...
#endif // defined(CONFIG_BCM963178)
...
# endif /* BRCM_NTR_SUPPORT */
...
#ifdef PCM_NTR_DSL
#if defined(CONFIG_BCM963178)
   DHLOG_DEBUG("%s: TOP->DslClockSample [0x%08X] @ 0x%p\n ",
          __func__, (unsigned int)TOP->DslClockSample,
          &TOP->DslClockSample);
#else
   ...
   DPLL->dpll_control |= DPLL_SOFT_INIT;
   DPLL->dpll_control &= ~(DPLL_FORCE_ACQ | DPLL_OPEN_LOOP | DPLL_CONST_K);
...
static void pcm_reg_cfg(void)
{
   ...
   /* We are assuming PCM will be running at 2048KHz and will be using 16KHz Frame Sync.
      So, each 16KHz frame will contain 128 bits.
    */
   switch ((PCM->pcm_ctrl & PCM_SAMPLE_SIZE) >> PCM_SAMPLE_SIZE_SHIFT) {
      case PCM_SAMPLE_SIZE_8:
         /* In 16KHz frame there will be 16 samples of size 8 bits*/
         PCM->pcm_ctrl |= 16 << PCM_FRAME_SIZE_SHIFT;
         break;
      case PCM_SAMPLE_SIZE_16:
         /* In 16KHz frame there will be 8 samples of size 16 bits*/
         PCM->pcm_ctrl |= 8 << PCM_FRAME_SIZE_SHIFT;
         break;
      case PCM_SAMPLE_SIZE_32:
         /* In 16KHz frame there will be 4 samples of size 32 bits*/
         PCM->pcm_ctrl |= 4 << PCM_FRAME_SIZE_SHIFT;
         break;
   }
