...
#else
	{
		/* for c2200, pptpvpn interface is pppdrv+, allow pptpvpn wan forward. Added by CCy, 2017-01-18  */
		if ( !((skb->dev->name) && (0 == strncmp(skb->dev->name, "pppdrv", 6))) &&
			 !((rt->dst.dev->name) && (0 == strncmp(rt->dst.dev->name, "pppdrv", 6))) )
			goto drop;
	}
#endif /* CONFIG_TP_IMAGE */
#endif
