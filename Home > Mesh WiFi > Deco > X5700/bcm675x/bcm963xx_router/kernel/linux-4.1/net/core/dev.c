...
#if defined(CONFIG_WAN_DETECTION)
/* add by wanghao  */
#include "../bridge/br_private.h"
/* add end  */
#endif /* CONFIG_WAN_DETECTION */
...
#if defined(CONFIG_TP_AVIRA_PATCH)
int bcm_avira_enable_g = 0;
EXPORT_SYMBOL(bcm_avira_enable_g);

#define DEV_ISBR(dev) (dev ? (dev->priv_flags & IFF_EBRIDGE) : 0)
#define DEV_ISWAN(dev) (dev ? (dev->priv_flags & IFF_WANDEV) : 0)

fc_flush_t  fc_flush_fn = NULL;
...
/*Hanjiayan@20181109 add*/
#if defined(CONFIG_TP_BCM_QOS_PATCH) && defined(CONFIG_TP_QOS_HW_ONOFF)
int bcm_iqos_hw_accel_enable_g = 0;
EXPORT_SYMBOL(bcm_iqos_hw_accel_enable_g);
#endif
...
#if defined(CONFIG_TP_AVIRA_PATCH)
int fc_flush_register(fc_flush_t fc_flush)
{
	if(fc_flush)
	{
		BUG_ON(fc_flush_fn != NULL);
		RCU_INIT_POINTER(fc_flush_fn, fc_flush);
	}
	else
	{
		RCU_INIT_POINTER(fc_flush_fn, NULL);
	}
	synchronize_rcu();
	return 0;
}
EXPORT_SYMBOL(fc_flush_register);
#endif
#endif
...
void dev_add_pack(struct packet_type *pt)
{
	...
	/*
	 * Only enable for avirasentinelfull
	 * Keep disable when avirasentinellite works
	 */
#if 0//defined(CONFIG_TP_AVIRA_PATCH)
	if (!BROADSTREAM_AVIRA_ENABLE()
			&& (pt->dev && !strncmp(pt->dev->name, "br-lan", 6) 
					&& !list_empty(&pt->dev->ptype_all)))
	{
		BROADSTREAM_AVIRA_SET_ENABLE(1);
...
out:
	...
	if (BROADSTREAM_AVIRA_ENABLE()
			&& (pt->dev && !strncmp(pt->dev->name, "br-lan", 6) 
					&& list_empty(&pt->dev->ptype_all)))
	{
		BROADSTREAM_AVIRA_SET_ENABLE(0);
...
again:
	list_for_each_entry_rcu(ptype, ptype_list, list) {
		...
		/*
		 * For avira: keep blog for the old one.
		 */
...
#if defined(CONFIG_TP_AVIRA_PATCH)
	/* Only Avira use ptype_all now */
	if(strncmp(dev->name, "br-lan", 6) || !speedtest_filter(skb))
#endif
	{
...
#if defined(CONFIG_TP_AVIRA_PATCH)
	RCU_INIT_POINTER(dev->brdev, NULL);
#endif
}
...
#if defined(CONFIG_TP_IMAGE)
pctl_cb_t pctl_skb_cb = NULL;
pctl_cb_t pctl_fkb_cb = NULL;

int pctl_cb_register(pctl_cb_t skb_cb, pctl_cb_t fkb_cb)
{
	if (skb_cb) {
		BUG_ON(pctl_skb_cb != NULL);
		RCU_INIT_POINTER(pctl_skb_cb, skb_cb);
	}
	else
	{
		RCU_INIT_POINTER(pctl_skb_cb, NULL);		
	}

	if (fkb_cb) {
		BUG_ON(pctl_fkb_cb != NULL);
		RCU_INIT_POINTER(pctl_fkb_cb, fkb_cb);
	}
	else
	{
		RCU_INIT_POINTER(pctl_fkb_cb, NULL);		
	}

	synchronize_rcu();
	return 0;
}
EXPORT_SYMBOL(pctl_cb_register);
#endif
...
#if defined(CONFIG_TP_AVIRA_PATCH)
int speedtest_filter(const struct sk_buff *skb)
{
	...
	if((skb->protocol != htons(0x0800))
		|| !iph || iph->protocol != 6
		|| !tcph || (tcph->source != htons(8080) && tcph->dest != htons(8080)))
	{
		return 0;
	}
	...
	{
		if (0 == strncmp(http_payload_start, "GET", 3)
			|| 0 == strncmp(http_payload_start, "HEAD", 4)
			|| 0 == strncmp(http_payload_start, "POST", 4)
			|| 0 == strncmp(http_payload_start, "PUT", 3)
			|| 0 == strncmp(http_payload_start, "DELETE", 6)
			|| 0 == strncmp(http_payload_start, "CONNECT", 7)
			|| 0 == strncmp(http_payload_start, "OPTIONS", 7)
			|| 0 == strncmp(http_payload_start, "TRACE", 5))
		{
			//printk("[%s:%d]HTTP\n", __FUNCTION__, __LINE__);
			return 0;
		}
...
EXPORT_SYMBOL(speedtest_filter);

static int __netif_blog_check(struct sk_buff *skb)
{
	rx_handler_func_t *rx_handler;
	...
	if (skb && skb->dev)
	{
		if(PKTISBRFCDONE(skb))
		{
			//printk("[%s:%d] =====> skb 0x%p goes through again, skip.\n", __FUNCTION__, __LINE__, skb);
			goto normal_path;
		}
		...
		brdev = rcu_dereference(skb->dev->brdev);
		if(!brdev || (!brdev->name) || strncmp(brdev->name, "br-lan", 6) != 0)
		{
			goto blog_path;
		}
...
#if 1
		/*
		 * TODO: add avira specific ptype?
		 */
		if(!speedtest_filter(skb))
		{
			if(!list_empty(&brdev->ptype_all))
			{
				pt_prev = NULL;
				orig_dev = skb->dev;
				skb->dev = brdev;
				list_for_each_entry_rcu(ptype, &skb->dev->ptype_all, list) {
					if (pt_prev)
						ret = deliver_skb(skb, pt_prev, brdev);
					pt_prev = ptype;
				}
		...
		if(pctl_skb_cb && pctl_skb_cb(skb))
		{
			//printk("[%s:%d]Leave a skb to the default Linux stack\n", __FUNCTION__, __LINE__);
			goto normal_path;
		}
	}
...
/*
 * should be used only under BROADSTREAM_AVIRA_ENABLE()
 */
int netif_blog_check(struct sk_buff *skb)
{
	int ret = PKT_BLOG;
	
	rcu_read_lock();
	ret = __netif_blog_check(skb);
	rcu_read_unlock();
	
	return ret;
}
EXPORT_SYMBOL(netif_blog_check);
#endif

#if defined(CONFIG_TP_AVIRA_PATCH) || defined(CONFIG_TP_FC_PCTL_SUPPORT)
int skb_pctl_check(struct sk_buff *skb)
{
	if(skb && skb->dev && !(((struct net_device *)skb->dev)->priv_flags & IFF_WANDEV))
	{
		if(pctl_skb_cb && pctl_skb_cb(skb))
		{
			printk(KERN_DEBUG "[%s:%d]Leave a skb to the default Linux stack\n", __FUNCTION__, __LINE__);
			return PKT_NORM;
		}
	}
...
EXPORT_SYMBOL(skb_pctl_check);

int fkb_pctl_check(struct fkbuff * fkb_p)
{
	if(fkb_p)
	{		
		if(pctl_fkb_cb && pctl_fkb_cb(fkb_p->data))
		{
			printk(KERN_DEBUG "[%s:%d]Leave a fkb to the default Linux stack\n", __FUNCTION__, __LINE__);
			return PKT_NORM;
		}		
	}
...
another_round:
	...
	if ((pData[start - 2] == 0x08 && pData[start - 1] == 0x0) && (pData[start + 20] == 0x00 && pData[start + 20 + 1] == 0x43) && pData[start + 20 + 8] == 0x02)//src port 67 & msg type 0x02 Boot Reply
	{
		...
		if (skb->dev != NULL && (skb->dev->priv_flags & IFF_BRIDGE_PORT))
		{
			...
			{
				skb->dev->priv_wan_flags |= IFF_WAN_DEV;
				skb->dev->priv_wan_flags |= IFF_WAN_DHCP_DEV;
...
#if 0
	else if ((pData[start - 2] == 0x08 && pData[start - 1] == 0x0) && (pData[start + 20] == 0x00 && pData[start + 20 + 1] == 0x35) && (pData[start + 20 + 8 + 2] & 0x80))//src port 53 & flags 0x80 response
	{
		//skb_debugFy(skb);
		if (skb->dev != NULL && !(skb->dev->priv_flags & IFF_EBRIDGE))
		{
			skb->dev->priv_wan_flags |= IFF_WAN_DEV;
			//printWar("%s recv a DNS reponse!skb->dev->priv_flags is 0x%x\n", skb->dev->name, skb->dev->priv_flags);
		}
	}
#endif
	/* add end  */
...
/*Hanjiayan@20161022 modified, For IPTV bridge mode, we use br interface as ppp dev, can not skip rx_handler*/	
...
int register_netdevice(struct net_device *dev)
{
	...
	//printk("[%s:%d] ========== dev(0x%p) %s ============ \n", __FUNCTION__, __LINE__, dev, dev->name);
