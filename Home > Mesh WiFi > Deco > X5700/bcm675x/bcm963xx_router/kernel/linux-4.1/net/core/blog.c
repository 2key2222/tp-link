...
#define blog_log(fmt, arg...)                                         \
    printk( CLRc "[BLOG][%s:%d]" fmt CLRnl, __FUNCTION__, __LINE__, ##arg )
                       ...
                       unsigned long param1, unsigned long param2 )
{
    ...
    if (request >= BLOG_REQUEST_MAX) {
        printk("%s: request >= BLOG_REQUEST_MAX to aviod panic \n", __func__);
        return 0;
    }

    if (net_p == (void *)NULL) {
        printk("%s: net_p == (void *)NULL to aviod panic\n", __func__);
        return 0;
    }
...
//CL267253
        ...
        case NETIF_PUT_STATS:
        {
            ...
            if (sw_bstats_p == (BlogStats_t *)NULL) {
                printk("%s: sw_bstats_p  is null to aviod panic\n", __func__);
                return 0;
            }

            if (hw_bstats_p == (BlogStats_t *)NULL) {
                printk("%s: hw_bstats_p  is null to aviod panic\n", __func__);
                return 0;
            }
            ...
            if ( dev_p->put_stats )
            {
                ...
                blog_log(" ========= illegal dev_p<0x%p>, return =========== \n", dev_p);
                return 0;
#else
                BlogStats_t bstats; 
...
BlogAction_t blog_sinit_args( struct sk_buff * skb_p, void * dev_p,
						  uint32_t encap, uint32_t channel, uint32_t phyHdr, BlogFcArgs_t *fc_args )
{
	 return _blog_sinit(skb_p, dev_p, encap, channel, phyHdr, fc_args);
}
