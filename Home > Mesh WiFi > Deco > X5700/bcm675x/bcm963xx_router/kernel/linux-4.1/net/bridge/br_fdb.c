...
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BLOG)
/*******************************************************************
	to fix problem: fdb entry changed, fcache entry flush
		==> RE_LAN --> AP_WAN throughput drop when dual/tri band
		not to flush fcache entry when fdb changed
********************************************************************/
#if 1
    blog_lock();
...
#if defined(CONFIG_WAN_DETECTION)
	{
		...
		if (old_dev && (old_dev->priv_dwds_flags & IFF_DWDS_DEV)) {
			need_notify = false;
		}

		//if(old_dev && old_dev->name) {
		//	printk("bridge fdb_delete: fdb del form %s, need to notify blog? %u\n",
		//		old_dev->name, need_notify);
		//}
...
********************************************************************/
...
#if defined(CONFIG_WAN_DETECTION)
				{
					...
					if (old_dev && (old_dev->priv_dwds_flags & IFF_DWDS_DEV) &&
						new_dev && (new_dev->priv_dwds_flags & IFF_DWDS_DEV) &&
						((new_dev->priv_dwds_flags & IFF_DWDS_GROUP_MASK) > 0) &&
						((old_dev->priv_dwds_flags & IFF_DWDS_GROUP_MASK) ==
						(new_dev->priv_dwds_flags & IFF_DWDS_GROUP_MASK))) {
						need_notify = false;
					}

					//if(old_dev && old_dev->name && new_dev && new_dev->name) {
					//	printk("bridge: fdb change form  %s to %s, need to notify blog? %u\n",
					//		old_dev->name, new_dev->name, need_notify);
					//}
