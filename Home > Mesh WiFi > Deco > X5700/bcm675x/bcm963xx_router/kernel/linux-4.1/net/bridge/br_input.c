...
static int br_pass_frame_up(struct sk_buff *skb)
{
	...
	{
		skb->dev->priv_wan_flags |= IFF_INDICATE_DEV0;
		skb->dev->priv_wan_flags &= ~IFF_INDICATE_DEV1;
	}
	...
	*/
	...
	if (is_broadcast_ether_addr(dest)) {
...
#if defined(CONFIG_WAN_DETECTION)
	} else if ((skb->dev->priv_wan_flags & IFF_WAN_FORWARD_FLAG) ||
	 ((dst = __br_fdb_get(br, dest, vid)) && dst->is_local)) {
		...
		blog_lock();
		if (src)
			blog_link(BRIDGEFDB, blog_ptr(skb), (void*)src, BLOG_PARAM1_SRCFDB, br->dev->ifindex); // CL267253

		if (dst)
			blog_link(BRIDGEFDB, blog_ptr(skb), (void*)dst, BLOG_PARAM1_DSTFDB, br->dev->ifindex); // CL267253
