...
typedef bool (*pctl_cb_t)(void *pBuf);
extern int pctl_cb_register(pctl_cb_t skb_cb, pctl_cb_t fkb_cb);
extern pctl_cb_t pctl_skb_cb;
extern pctl_cb_t pctl_fkb_cb;
#endif
...
/* change for avira by wanglian, 2020.03.27 */
#if defined(CONFIG_TP_AVIRA_PATCH)
extern int bcm_avira_enable_g;

#define BROADSTREAM_AVIRA_SET_ENABLE(x) \
	(bcm_avira_enable_g = x)

#define BROADSTREAM_AVIRA_ENABLE(x) \
	(bcm_avira_enable_g != 0)

#define BROADSTREAM_PATH_CHANGE(x) \
	((bcm_iqos_enable_g != 0) || (bcm_avira_enable_g != 0))

#define BROADSTREAM_QOS_TC_ON(x) \
	(bcm_iqos_enable_g != 0)

typedef int (*fc_flush_t)(void);
extern int fc_flush_register(fc_flush_t fc_flush);
#else
#define BROADSTREAM_PATH_CHANGE(x) \
	(bcm_iqos_enable_g != 0)
#endif

/*Hanjiayan@20181109 modified*/
#if defined(CONFIG_TP_BCM_QOS_PATCH) && defined(CONFIG_TP_QOS_HW_ONOFF)

extern int bcm_iqos_hw_accel_enable_g;

#define BROADSTREAM_IQOS_HW_ACCEL_SET_ENABLE(x) \
	(bcm_iqos_hw_accel_enable_g = x)

#define BROADSTREAM_IQOS_HW_ACCEL_ENABLE(x) \
	(bcm_iqos_hw_accel_enable_g != 0)

#define BROADSTREAM_IQOS_ENABLE(x) \
	(BROADSTREAM_PATH_CHANGE(x) && (bcm_iqos_hw_accel_enable_g == 0))

#else
#define BROADSTREAM_IQOS_ENABLE(x) \
	BROADSTREAM_PATH_CHANGE(x)

#endif /* CONFIG_TP_QOS_HW_ONOFF */
...
#if defined(CONFIG_TP_AVIRA_PATCH)
#define BRFC_PKTDONE (1 << 12)
#define PKTISBRFCDONE(skb) \
    ({ \
        (((struct sk_buff*)(skb))->pktc_flags & BRFC_PKTDONE); \
    })
#define PKTSETBRFCDONE(skb)  \
   ({ \
    (((struct sk_buff*)(skb))->pktc_flags |= BRFC_PKTDONE); \
    })
#define PKTCLRBRFCDONE(skb)  \
   ({ \
    (((struct sk_buff*)(skb))->pktc_flags &= (~BRFC_PKTDONE)); \
    })

#define PKTBLOCKING (1 << 11)
#define PKTISBLOCKING(skb) \
    ({ \
        (((struct sk_buff*)(skb))->pktc_flags & PKTBLOCKING); \
    })
#define PKTSETBLOCKING(skb)  \
   ({ \
    (((struct sk_buff*)(skb))->pktc_flags |= PKTBLOCKING); \
    })
#define PKTCLRBLOCKING(skb)  \
   ({ \
    (((struct sk_buff*)(skb))->pktc_flags &= (~PKTBLOCKING)); \
    })
...
#define BLOG_DEV_STAT_FLAG_INCLUDE_ALL      (BLOG_DEV_STAT_FLAG_INCLUDE_SW|BLOG_DEV_STAT_FLAG_INCLUDE_HW)
    ...
    unsigned int priv_wan_flags;
    unsigned int priv_dwds_flags;
#endif
...
#define NETDEV_BR_JOIN		0x001A
#define NETDEV_BR_LEAVE		0x001B
