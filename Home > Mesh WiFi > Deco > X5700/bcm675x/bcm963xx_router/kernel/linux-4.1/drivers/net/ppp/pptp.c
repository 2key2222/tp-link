...
//#define CONFIG_SFE_PPP_HEADER 1
    ...
    if (sock) 
    {
        ...
        {   
            ...
            } else if (opt->seq_recv && ((*rcv_pktSeq) > opt->seq_recv)) 
            {
                ...
                printk(KERN_DEBUG "pptp_rcv_check():[BLOG_PPTP_RCV_OOS_LT] current seq_recv is %d \n", opt->seq_recv);
#else
...
#if defined(CONFIG_TP_IMAGE)				
                printk(KERN_DEBUG "pptp_rcv_check():[BLOG_PPTP_RCV_OOS_GT] current seq_recv is %d \n", opt->seq_recv);
#else
