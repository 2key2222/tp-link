...
{
    if(_entry >= RDD_PFC_TX_STATUS_TABLE_SIZE)
          return BDMF_ERR_PARM;

    RDD_BYTES_4_BITS_WRITE_G(bits, RDD_PFC_TX_STATUS_TABLE_ADDRESS_ARR, _entry);
...
int rdd_ag_us_tm_pfc_tx_status_table_get(uint32_t _entry, uint32_t *bits)
{
    ...
    RDD_BYTES_4_BITS_READ_G(*bits, RDD_PFC_TX_STATUS_TABLE_ADDRESS_ARR, _entry);
...
{
    RDD_BYTE_1_BITS_WRITE_G(bits, RDD_PFC_TX_ENABLE_ADDRESS_ARR, 0);
...
{
    RDD_BYTE_1_BITS_READ_G(*bits, RDD_PFC_TX_ENABLE_ADDRESS_ARR, 0);
