...
void dump_RDD_MULTICAST_WHITELIST_CFG(bdmf_session_handle session, unsigned char *p)
{
	unsigned int r;
	bdmf_session_print(session, "  Register MULTICAST_WHITELIST_CFG\n");
	...
	bdmf_session_print(session, "\tnatc_tbl_idx             = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_8((uint8_t *)p + 7, r);
	bdmf_session_print(session, "\tddr_ctx_tbl_high         = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");

	MREAD_32((uint8_t *)p + 8, r);
	bdmf_session_print(session, "\tddr_ctx_tbl_low          = 0x%08x", (unsigned int)r);
	bdmf_session_print(session, "\n");
