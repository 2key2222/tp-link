...
int rdd_ag_ds_tm_bbh_tx_ds_fifo_bytes_threshold_set(uint16_t bits)
{
    RDD_BYTES_2_BITS_WRITE_G(bits, RDD_BBH_TX_DS_FIFO_BYTES_THRESHOLD_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_bbh_tx_ds_fifo_bytes_threshold_get(uint16_t *bits)
{
    RDD_BYTES_2_BITS_READ_G(*bits, RDD_BBH_TX_DS_FIFO_BYTES_THRESHOLD_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_tx_exception_dbg_stat_en_set(bdmf_boolean dbg_stat_en)
{
    RDD_TX_EXCEPTION_ENTRY_DBG_STAT_EN_WRITE_G(dbg_stat_en, RDD_TX_EXCEPTION_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_tx_exception_dbg_stat_en_get(bdmf_boolean *dbg_stat_en)
{
    RDD_TX_EXCEPTION_ENTRY_DBG_STAT_EN_READ_G(*dbg_stat_en, RDD_TX_EXCEPTION_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_tx_exception_vlan_cntr_en_set(bdmf_boolean vlan_cntr_en)
{
    RDD_TX_EXCEPTION_ENTRY_VLAN_CNTR_EN_WRITE_G(vlan_cntr_en, RDD_TX_EXCEPTION_ADDRESS_ARR, 0);

    return BDMF_ERR_OK;
}

int rdd_ag_ds_tm_tx_exception_vlan_cntr_en_get(bdmf_boolean *vlan_cntr_en)
{
    RDD_TX_EXCEPTION_ENTRY_VLAN_CNTR_EN_READ_G(*vlan_cntr_en, RDD_TX_EXCEPTION_ADDRESS_ARR, 0);
