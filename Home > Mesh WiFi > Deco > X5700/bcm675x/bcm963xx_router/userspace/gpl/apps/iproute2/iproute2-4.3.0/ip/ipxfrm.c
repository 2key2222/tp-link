		  ...
		  int loose, int *argcp, char ***argvp)
{
	...
	while (1) {
		if (strcmp(*argv, "src") == 0) {
			...
			xf_filter.id_src_mask = src.bitlen;
			...
			xf_filter.id_dst_mask = dst.bitlen;
			...
			xf_filter.id_proto_mask = XFRM_FILTER_MASK_FULL;
			...
			xf_filter.id_spi_mask = XFRM_FILTER_MASK_FULL;
				      ...
				      int *argcp, char ***argvp)
{
	...
	while (1) {
		if (strcmp(*argv, "proto") == 0) {
			...
			xf_filter.upspec_proto_mask = XFRM_FILTER_MASK_FULL;
			...
			xf_filter.upspec_sport_mask = XFRM_FILTER_MASK_FULL;
			...
			xf_filter.upspec_dport_mask = XFRM_FILTER_MASK_FULL;
...
int xfrm_selector_parse(struct xfrm_selector *sel, int *argcp, char ***argvp)
{
	...
	while (1) {
		if (strcmp(*argv, "src") == 0) {
			...
			xf_filter.sel_src_mask = src.bitlen;
			...
			xf_filter.sel_dst_mask = dst.bitlen;
			...
			xf_filter.sel_dev_mask = XFRM_FILTER_MASK_FULL;
...
{
	memset(&xf_filter, 0, sizeof(xf_filter));
