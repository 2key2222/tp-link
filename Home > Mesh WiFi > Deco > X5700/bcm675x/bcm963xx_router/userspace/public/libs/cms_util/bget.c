...
/*
 * BRCM: Now that MDM does zone locking, bget must use its own lock.
 */
#include <sys/ipc.h>
#include <sys/sem.h>
#include <sys/shm.h>
...
#define BGET_SEMAPHORE_KEY  0x2DAB

/* BRCM: do not use BGET_SEMDOWN macro.  must not proceed if there is an error.
 * #define BGET_SEMDOWN() \
...
#endif
{
    ...
    /* BGET_SEMDOWN(); */
    if ( semop(bg_semid, &bg_lock[0], 2) == -1 ) {
        perror("bget: semdown failed!");
        return NULL;
    }
...
void bcm_secondary_bpool(void *buf)
{
    ...
    * pointer to the right address and get the bget lock.
    */
   g_bg = buf;
   
   if( (bg_semid = semget(BGET_SEMAPHORE_KEY, 1, 0666)) == -1) {
      perror("bget: secondary semget\n");
      return;
   }
}
  ...
  bufsize len;
{
    ...
    if( g_bg == NULL ) {
        if( (bg_semid = semget(BGET_SEMAPHORE_KEY, 1, IPC_CREAT|0666)) == -1) {
	        perror("bget: semget\n");
	        return;
        }
...
int isShmAddrInUse(const unsigned char *addr, unsigned int seq)
{
   ...
   /* BGET_SEMDOWN(); */
   if ( semop(bg_semid, &bg_lock[0], 2) == -1 ) {
      perror("isShmAddrInUse: semdown failed!");
      return -1;
   }

   b = (struct bhead *) g_bg->freelist.ql.flink;
