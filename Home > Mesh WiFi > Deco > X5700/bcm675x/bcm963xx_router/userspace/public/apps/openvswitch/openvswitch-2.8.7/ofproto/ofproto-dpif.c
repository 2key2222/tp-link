...
update_rstp_port_state(struct ofport_dpif *ofport)
{
    ...
    if (ofport->rstp_state != state) {
        ...
        netdev_bcm_set_port_rstp_state(ofport->up.netdev, ofport->rstp_state, state);
#endif
...
#if defined(CONFIG_BCM_OVS)
        netdev_bcm_set_port_stp_state(ofport->up.netdev, ofport->stp_state, state);
#endif
...
int manage_mcast_snooping_entry(struct ofproto *ofproto_, 
                                ovs_be32 ip4, 
                                struct in6_addr *ip6,
                                uint16_t vlan, 
                                void *port,
...
{
    struct ofproto_dpif *ofproto = ofproto_dpif_cast(ofproto_);
    struct ofbundle *bundle = bundle_lookup(ofproto, port);
    ...
    if (ofproto && bundle) 
    {
        if (isip4) 
        {
            if (addentry) 
            {
                if (mcast_snooping_add_group4(ofproto->ms, ip4, vlan, bundle, ip4src) == true)
                {
                    return 0;
                }
            }
            ...
            {
                if (mcast_snooping_leave_group4(ofproto->ms, ip4, vlan, bundle, ip4src) == true)
                {
                    return 0;
                }
            }
        ...
        else
        {
            if (addentry) 
            {
                if (mcast_snooping_add_group(ofproto->ms, ip6, vlan, bundle, ip6src) == true) 
                {
                    return 0;
                }
            }
            ...
            {
                if (mcast_snooping_leave_group(ofproto->ms, ip6, vlan, bundle, ip6src) == true) 
                {
                    return 0;
                }
            }
                                    ...
                                    void *aux OVS_UNUSED)
{
...
#if defined(CONFIG_BCM_OVS_MCAST)
    ds_put_cstr(&ds, " port  VLAN  GROUP                Age  SrcAddr\n");
#else
    ...
    LIST_FOR_EACH (grp, group_node, &ofproto->ms->group_lru) {
        LIST_FOR_EACH(b, bundle_node, &grp->bundle_lru) {
            ...
            ipv6_format_mapped(&grp->srcaddr, &ds);
            ds_put_format(&ds, "\n");
#else
            ds_put_format(&ds, "         %3d\n",
                          mcast_bundle_age(ofproto->ms, b));
