                 ...
                 struct dpif_upcall *upcall, int *dp_ifindex)
{
    static const struct nl_policy ovs_packet_policy[] = {
        ...
        /* OVS_PACKET_CMD_MISS only. */
        ,[OVS_PACKET_ATTR_BCM_PKTMETADATA] = { .type = NL_A_UNSPEC, 
                                               .optional = true }
#endif
    };
...
#if defined(CONFIG_BCM_OVS_MCAST)
    if (type == DPIF_UC_MISS && a[OVS_PACKET_ATTR_BCM_PKTMETADATA]) 
    {
        struct nlattr *bcm_pktmetadata_nl = a[OVS_PACKET_ATTR_BCM_PKTMETADATA];
        int    bcm_pktmetadata_len = nl_attr_get_size(bcm_pktmetadata_nl);

        if (bcm_pktmetadata_len < sizeof(struct ovs_bcm_pktmetadata)) {
            VLOG_WARN("bcm_pktmetadata_len has unexpected size %"PRIuSIZE,
                       bcm_pktmetadata_len);
            return EINVAL;
        }
        memset(&upcall->packet.bcm_pktmetadata, 0, sizeof(upcall->packet.bcm_pktmetadata));
        memcpy(((char *)&upcall->packet.bcm_pktmetadata), 
               nl_attr_get(bcm_pktmetadata_nl), 
               bcm_pktmetadata_len);
    }
    else
    {
        /* Set the to_acceldev_ifi to invalid value */
        upcall->packet.bcm_pktmetadata.to_acceldev_ifi = -1;
    }
#endif
