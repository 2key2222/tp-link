    ...
    OVS_REQ_WRLOCK(ms->rwlock)
{
    ...
    case IGMPV2_HOST_MEMBERSHIP_REPORT:
        ...
        if (mcast_snooping_add_group4(ms, ip4, vlan, in_xbundle->ofbundle, 0)) {
#else
    ...
    case IGMP_HOST_LEAVE_MESSAGE:
        ...
        if (mcast_snooping_leave_group4(ms, ip4, vlan, in_xbundle->ofbundle, 0)) {
#else
...
xlate_normal(struct xlate_ctx *ctx)
{
    ...
    if (mcast_snooping_enabled(ctx->xbridge->ms)
        ...
        if (((is_igmp(flow, wc) && mcast_snooping_is_membership(flow->tp_src))
             || is_mld_report(flow, wc)) &&
             ctx->xin->allow_side_effects && ctx->xin->packet)
        {
            /* Notify MCPD of this IGMP/MLD Report message.
               MCPD will asynchronously send cmds to manage the snoop entry */
            ovs_mcpd_notify((unsigned char *)(ctx->xin->packet->base_ + 
                                              ctx->xin->packet->data_ofs),
                            ctx->xin->packet->allocated_,
                            netdev_get_ifindex(in_port->netdev),
                            ctx->xin->packet->bcm_pktmetadata.to_acceldev_ifi,
                            flow->nw_src,
                            &(flow->ipv6_src), 
                            vlan,
                            in_port->xbridge->name,
                            ctx->xin->packet->l4_ofs,
                            is_igmp(flow, wc));     
            mcpdnotified = 1;   
        }
#endif
...
#if defined(CONFIG_BCM_OVS_MCAST)
            /* set srcaddr to 0 */
            grp = mcast_snooping_lookup4(ms, flow->nw_dst, vlan, flow->nw_src);
#else
...
#if defined(CONFIG_BCM_OVS_MCAST)
            /* set srcaddr to 0 */
            grp = mcast_snooping_lookup(ms, &flow->ipv6_dst, vlan, &(flow->ipv6_src));
#else
