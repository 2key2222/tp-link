 ...
 * 'dip', 'vlan' and multicast source 'srcip'. */
struct mcast_group *
    ...
    OVS_REQ_RDLOCK(ms->rwlock)
{
    ...
    uint32_t hash;
#if defined(CONFIG_BCM_OVS_MCAST)
    const struct in6_addr v4mappedv6zero = in6_addr_mapped_ipv4(0);
    const struct in6_addr v6zero = {0};
#endif

    hash = mcast_table_hash(ms, dip, vlan);
    HMAP_FOR_EACH_WITH_HASH (grp, hmap_node, hash, &ms->table) {
#if defined(CONFIG_BCM_OVS_MCAST)
        /* If the snoopentry is for ASM group, then srcaddr
           is zero. For v6 it is all zeros and for v4 it is
           in6_addr_mapped_ipv4(0). The snoopentry corresponds
           to a SSM group if the srcaddr is non-zero in which
           case the non-zero source address must match */
        if (grp->vlan == vlan && ipv6_addr_equals(&grp->addr, dip) &&
            (ipv6_addr_equals(&grp->srcaddr, &v4mappedv6zero) ||
             ipv6_addr_equals(&grp->srcaddr, &v6zero) ||
             ipv6_addr_equals(&grp->srcaddr, srcip))) {
#else
        if (grp->vlan == vlan && ipv6_addr_equals(&grp->addr, dip)) {
#endif
           return grp;
...
#if defined(CONFIG_BCM_OVS_MCAST)
    struct in6_addr srcaddr = in6_addr_mapped_ipv4(srcip4);
    return mcast_snooping_lookup(ms, &addr, vlan, &srcaddr);
#else
    ...
    OVS_REQ_WRLOCK(ms->rwlock)
{
    ...
    learned = false;
#if defined(CONFIG_BCM_OVS_MCAST)
    grp = mcast_snooping_lookup(ms, addr, vlan, srcaddr);
#else
...
mcast_snooping_add_group4(struct mcast_snooping *ms, ovs_be32 ip4,
                          uint16_t vlan, void *port, ovs_be32 srcip4)
#else
bool
mcast_snooping_add_group4(struct mcast_snooping *ms, ovs_be32 ip4,
                         uint16_t vlan, void *port)
...
#if defined(CONFIG_BCM_OVS_MCAST)
    struct in6_addr srcaddr = in6_addr_mapped_ipv4(srcip4);
    return mcast_snooping_add_group(ms, &addr, vlan, port, &srcaddr);
#else
                          ...
                          uint16_t vlan, void *port)
{
    ...
    while (ngrp--) {
        ...
        if (ntohs(record->nsrcs) == 0
...
#if defined(CONFIG_BCM_OVS_MCAST)
            /* When BRCM OVS support is enabled, MCPD manages the snoop entries.
               So, this piece of code would never get executed. However, since
               mcast_snooping_leave_group4() expectes a srcaddr, set srcaddr to 0 */
            ret = mcast_snooping_leave_group4(ms, ip4, vlan, port, 0);
#else
            ...
            /* When BRCM OVS support is enabled, MCPD manages the snoop entries.
               So, this piece of code would never get executed. However, since
               mcast_snooping_add_group4() expectes a srcaddr, set srcaddr to 0 */
            ret = mcast_snooping_add_group4(ms, ip4, vlan, port, 0);
#else
                          ...
                          uint16_t vlan, void *port)
{
    ...
    case MLD_REPORT:
        ...
        /* When BRCM OVS support is enabled, MCPD manages the snoop entries.
           So, this piece of code would never get executed. However, since
           mcast_snooping_add_group() expectes a srcaddr, set srcaddr to 0 */
        ret = mcast_snooping_add_group(ms, addr, vlan, port, &ip6src_zero);
#else
    ...
    case MLD_DONE:
        ...
        /* When BRCM OVS support is enabled, MCPD manages the snoop entries.
           So, this piece of code would never get executed. However, since
           mcast_snooping_leave_group() expectes a srcaddr, set srcaddr to 0 */
        ret = mcast_snooping_leave_group(ms, addr, vlan, port, &ip6src_zero);
#else
...
#if defined(CONFIG_BCM_OVS_MCAST)
    struct in6_addr srcaddr = in6_addr_mapped_ipv4(srcip4);
    return mcast_snooping_leave_group(ms, &addr, vlan, port, &srcaddr);
#else
