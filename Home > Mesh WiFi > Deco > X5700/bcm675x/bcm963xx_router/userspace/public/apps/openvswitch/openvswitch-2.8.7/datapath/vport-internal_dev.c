...
static int internal_dev_xmit(struct sk_buff *skb, struct net_device *netdev)
{
	...
	blog_lock();
	blog_link(IF_DEVICE, blog_ptr(skb), (void*)netdev, DIR_TX, skb->len);
	blog_unlock();
#endif
...
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BLOG)
	netdev->blog_stats_flags |= BLOG_DEV_STAT_FLAG_INCLUDE_ALL;
#endif
}
...
#if defined(CONFIG_BCM_KF_BLOG) && defined(CONFIG_BLOG)
	blog_lock();
	blog_link(IF_DEVICE, blog_ptr(skb), (void*)netdev, DIR_RX, skb->len);
	blog_unlock();
#endif
	skb_dst_drop(skb);
	nf_reset(skb);
