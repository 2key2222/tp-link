...
void netdev_bcm_set_port_stp_state(struct netdev *netdev, int old_st, int new_st)
{
    char cmd[80];
    sprintf(cmd, "ethswctl -c ifstp -i %s -o %s", netdev_get_name(netdev), stp_state_name(new_st));
    system(cmd);
}

void netdev_bcm_set_port_rstp_state(struct netdev *netdev, int old_st, int new_st)
{
    char cmd[80];
    sprintf(cmd, "ethswctl -c ifstp -i %s -o %s", netdev_get_name(netdev), rstp_state_name(new_st));
    system(cmd);
}
