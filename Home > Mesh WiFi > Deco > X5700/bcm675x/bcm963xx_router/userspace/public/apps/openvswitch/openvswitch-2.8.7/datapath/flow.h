...
struct sw_flow {
	...
	int output_count; /* # of output actions */
	u32 blog_key;     /* Associating megaflow to blogged flows  */
	bool blog_match;  /* Is megaflow compatible with blog flow? */
	blog_fast_stats_t cumm_fast_stats; /* cummulative fast stats of all flow 
						* cache flows of a megaflow evicted from flow cache 
						*/
	u64 prev_packet_count; /* previous packet count of active flow cache 
							* flows of a megaflow
							*/
#endif
