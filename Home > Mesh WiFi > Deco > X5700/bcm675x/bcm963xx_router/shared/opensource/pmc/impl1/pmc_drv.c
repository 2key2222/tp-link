...
#if defined(_BCM963178_) || defined(_BCM947622_)
/************************************************************************
	建议先调整10mv AVS margin;
	BCM47622/BCM6755:
	– Default AVS Margin: 80 mV for SS silicon and 40 mV for FF silicon.
	– Maximum Peak to Peak Ripple Allowed: 20 mV

	所以调正10mv  ==>  90 mV for SS silicon and 50 mV for FF silicon.
	   调正20mv  ==> 100 mV for SS silicon and 60 mV for FF silicon.
*************************************************************************/
#if defined (TP_AVS_20MV)
    margin_ss = 100;
    margin_ff = 60;
#elif defined(TP_AVS_10MV)
    margin_ss = 90;
    margin_ff = 50;
#else
    margin_ss = 90;
	...
	res = CloseAVS(0, margin_ss, margin_ff, 0, 0);
	if( res == 0 )
	{
        printk("close avs with [island %d] margin_mv slow %d fast %d "
               "[max_mv %d min_mv %d (0 means using firmware default)] "
               "succeeded\n",
    			0, margin_ss, margin_ff,0, 0);
	}
#endif
#endif 
