...
#ifdef BOARD_SUPPORT_REALTEK_PHY
int __read_status_exphy(phy_dev_t *phy_dev)
{
    int ret = 0;
    uint16_t val = 0;
    int mode = 0;
    ...
    if ((ret = phy_bus_read(phy_dev, MII_BMSR, &val)))
        return ret;
    
    //printk("%s-%d:we get the val[0x%x]\r\n", __func__, __LINE__, val);
    ...
    val = 0;
    if ((ret = phy_bus_read(phy_dev, MII_LPA, &val)))
        return ret;
    ...
    if(mode >> 8) {
        val = 0;
        if ((ret = phy_bus_read(phy_dev, MII_STAT1000, &val)))
            return ret;
        if(val) {
    ...
    for(idx = 0; idx < 16; idx++) {
        ...
        if ((ret = phy_bus_read(phy_dev, reg, &val)))
            return ret;
        reg++;
       printk("%s-%d:we get the val[0x%x] for idx[%d]\r\n", __func__, __LINE__, val, idx);
    }
    ...
    if(val & 0x100)
        ...
        phy_dev->duplex = PHY_DUPLEX_HALF;
#endif
    /*printk("%s-%d:we are here to read status for addr[%d] link[%d] speed[%d] duplex[%d]\r\n", __func__, __LINE__, 
        phy_dev->addr, phy_dev->link, phy_dev->speed, phy_dev->duplex);*/
...
#ifdef BOARD_SUPPORT_REALTEK_PHY
    if(5 == phy_dev->addr) {
        return __read_status_exphy(phy_dev);
    }
#endif
