...
{
    unsigned short led = pin & BP_GPIO_NUM_MASK;
    int slot = g_optled_map[led];

    unsigned mask = 0x3fc0; 	/* bit 06-13 for bright_ctrl*/
    unsigned int value = 0;

    if (slot > 31 || pin & BP_LED_USE_GPIO)
        return;
	...
	/* set brightness */
	value = LED->LedCfg[slot].config[0] & (~ mask);
	value |= (brt << 10);
	LED->LedCfg[slot].config[0] = value;
...
void bcm_led_driver_blink (unsigned short pin, unsigned reg)
{
	...
	unsigned mask = 0x38; 	/* bit 05-03 for flash_ctrl*/
	unsigned int value = 0;
	...
	old_reg = ((LED->LedCfg[slot].config[0] & mask) >> 3);

	/* if reg not change, don't save it */
	if (old_reg != reg) 
	{
	    value = LED->LedCfg[slot].config[0] & ~ mask;
		/* set flash_ctrl reg */
		...
		/* active led channel */
		LED->ChnActive = (LED->ChnActive | (1 << slot));
	}
}
#endif
...
#if defined(CONFIG_TP_IMAGE)
    /* power(red+blue) blink */
    unsigned short power_gpio_red;
    unsigned short power_gpio_green;
    int has_power_led = 0;
    if( BpGetRedLedGpio( &power_gpio_red ) == BP_SUCCESS )
        has_power_led = 1;

    if( BpGetGreenLedGpio( &power_gpio_green ) == BP_SUCCESS )
        has_power_led = 1;
#endif
...
#if defined(CONFIG_TP_IMAGE)
                    /* keep power led blink */
                    if (!has_power_led || (power_gpio_red != gpio && power_gpio_green != gpio))
#endif
...
#if defined(CONFIG_TP_IMAGE)
/* 
 * when soft reset board, LED Controller output some unwanted things,
 * so set all led to GPIO Pinmux, to work around this problem.
 */
void bcm_led_deinit(void)
{
    ...
    for(;;)
    {
        ...
        else if( rc == BP_SUCCESS && gpio != BP_GPIO_NONE )
        {
            if (gpio & BP_LED_USE_GPIO)
                continue;
            //bcm_set_pinmux(gpio & BP_GPIO_NUM_MASK, 4);
        } else {
            token = 0;
            i++;
        }
...
void bcm_common_led_setInitial(void)
{
    unsigned short gpio;
#if defined(CONFIG_TP_IMAGE)
    if( BpGetRedLedGpio( &gpio ) == BP_SUCCESS ) {
        //bcm_led_driver_blink(gpio, BLINK_FREQ_XSLOW);
        bcm_led_driver_set(gpio, 1);
    }
    if( BpGetGreenLedGpio( &gpio ) == BP_SUCCESS ) {
        //bcm_led_driver_blink(gpio, BLINK_FREQ_XSLOW);
        bcm_led_driver_set(gpio, 1);
    }
#else
