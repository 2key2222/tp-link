...
KERN_VERSION := $(shell sed -n '/CONFIG_LINUX_KERNEL_VERSION/p' .config | cut -d "=" -f 2)
...
PATCH_DEP_TAB := $(shell md5sum tools/mconf_deptab_spt_patch/kerncfg_deptab_patch/${KERN_VERSION}/dep_tab.c | cut -d " " -f 1)
BOARD := $(shell if [ -f .config ]; \
		then sed -n '/CONFIG_BOARD_NAME/p' .config | cut -d \" -f 2; fi)

patch_kern_mconf:
	@if [ $(findstring bcm675x,$(BOARD)) != "bcm675x" ] && [ "$(KERN_SRC_PATH)" != "" ] && [ -d $(KERN_SRC_PATH) ] && [ "$(KERN_DEP_TAB)" != "$(PATCH_DEP_TAB)"  ]; then \
		cp -rf tools/mconf_deptab_spt_patch/kerncfg_deptab_patch/$(KERN_VERSION)/* $(KERN_SRC_PATH)/scripts/kconfig; \
	fi	
...
rootfs:
	$(_SINGLE)$(NO_TRACE_MAKE) package/rootfs-prepare
