...
/*
 * Modified by Ye Qianchuan <yeqianchuan@tp-link.net>
 * ChangeLog:
 * Added -t and -T options to specify the timeout and retry times.
 * Added -q option to allow fast return after receiving one PADO packet.
 */
...
***********************************************************************/
int
openInterface(char const *ifname, UINT16_t type, unsigned char *hwaddr, int bindDevice)
{
    int optval=1;
    int fd;
    struct ifreq ifr;
...
waitForPADO(PPPoEConnection *conn, int timeout)
{
	...
	if (BPF_BUFFER_IS_EMPTY) {
	    ...
	    FD_ZERO(&readable);
	    FD_SET(conn->padoSocket, &readable);

	    while(1) {
		r = select(conn->padoSocket+1, &readable, NULL, NULL, &tv);
	...
	/* Get the packet */
	receivePacket(conn->padoSocket, &packet, &len);
...
***********************************************************************/
int
discovery(PPPoEConnection *conn, unsigned int timeout, unsigned int max_attempts)
{
    int padiAttempts = 0;

    conn->discoverySocket =
			openInterface(conn->ifName, Eth_PPPOE_Discovery, conn->myEth, 1);

	if (conn->bindDevice == 0) {
		conn->padoSocket = openInterface(conn->ifName, Eth_PPPOE_Discovery, conn->myEth, 0);
	}
	else {
		conn->padoSocket = conn->discoverySocket;
	}

	do {
		padiAttempts++;
		if (padiAttempts > max_attempts) {
			fprintf(stderr, "Timeout waiting for PADO packets\n");
			if (conn->padoSocket != conn->discoverySocket) {
				close(conn->padoSocket);
				conn->padoSocket = -1;
			}
...
int main(int argc, char *argv[])
{
	...
	unsigned int timeout = PADI_TIMEOUT;
	unsigned int max_attempts = MAX_PADI_ATTEMPTS;
	int ret;
	...
	while ((opt = getopt(argc, argv, "t:T:qi:I:D:VUAS:C:h")) > 0) {
	switch(opt) {
	case 't':
		timeout = strtoul(optarg, NULL, 10);
		break;
	case 'T':
		max_attempts = strtoul(optarg, NULL, 10);
		break;
	case 'q':
		conn->printACNames = 0;
		break;
    ...
    ret = discovery(conn, timeout, max_attempts);
    exit(ret);
}
