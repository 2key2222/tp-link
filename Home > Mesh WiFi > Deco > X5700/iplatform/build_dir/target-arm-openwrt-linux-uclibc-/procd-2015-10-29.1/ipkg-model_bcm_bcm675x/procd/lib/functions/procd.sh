...
_procd_add_delayed_reload_trigger() {
    local timeout=$1
    shift
    ...
    for file in "$@"; do
        ...
        json_add_array
        _procd_add_array_data "eq" "package" "$file"
        json_close_array

        json_add_array
        _procd_add_array_data "run_script" "/etc/init.d/$name" "reload"
        json_close_array
...
_procd_wrapper \
	...
	procd_add_delayed_reload_trigger \
