...
local type, tostring, tonumber, unpack, string = type, tostring, tonumber, unpack, string
...
--- Get the first section with the given type and return as table.
-- @param section	UCI section type
-- @return			UCI value
function Cursor.get_profile_section(self, section)
	local val = {}
	if section then
		self:set_confdir("/etc/profile.d")
		self:foreach("profile", section, function(s)
...
function Cursor._commit(self, ...)
    ...
    local user_config = {}
    local has_dev_config = false
    for _, config in ipairs{...} do
        if cfg.is_user_config(config) or cfg.is_emmc_config(config) then
            user_config[#user_config+1] = config
        else
            success = self:rawcommit(config) and success
            has_dev_config = true
        end
    ...
    if has_dev_config then
        cfg.saveconfig("device-config")
    end

    if #user_config > 0 then
        local dbg = require "luci.tools.debug"
        dbg("WARNING: some configs was NOT saved!! Consider adding them to device-config in profile.")
        dbg("         They are:", table.concat(user_config, ", "))
    end
...
function Cursor.commit_without_write_flash(self, ...)
    local dbg = require "luci.tools.debug"
    dbg("WARNING: Cursor.commit_without_write_flash IS DEPRECATED!")
...
-- Override the original cursor function.
function cursor(confdir, savedir)
    if confdir == nil and savedir == nil then
        return inst
    else
        return new_cursor(confdir, savedir)
    end
end
