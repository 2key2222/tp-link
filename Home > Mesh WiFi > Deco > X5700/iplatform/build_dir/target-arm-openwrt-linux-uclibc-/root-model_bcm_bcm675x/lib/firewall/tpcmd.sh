...
fw_openvpn_access(){
    ...
    fw_s_add 4 f forwarding_rule_vpn ACCEPT 1 { "-i $homeif -o tun0" }
    if [ "$access" == "home" ]; then
        fw_s_add 4 f forwarding_rule_vpn ACCEPT 1 { "-i tun0 -o $homeif" }
    else
        fw_s_add 4 f forwarding_rule_vpn ACCEPT 1 { "-i tun0" }
    fi
}
...
fw_openvpn_block(){
    ...
    fw_s_del 4 f forwarding_rule_vpn ACCEPT 1 { "-i $homeif -o tun0" }
    if [ "$access" == "home" ]; then
        fw_s_del 4 f forwarding_rule_vpn ACCEPT 1 { "-i tun0 -o $homeif" }
    else
        fw_s_del 4 f forwarding_rule_vpn ACCEPT 1 { "-i tun0" }
    fi
}
...
fw_pptp_access(){
    ...
    local homeif="$(uci_get_state firewall core lan_ifname)"
    fw_s_add 4 f zone_wan_vpn_access ACCEPT 1 { "$rule" }
    fw_s_add 4 f forwarding_rule_vpn ACCEPT 1 { "-i $homeif -o ppp+" }
    fw_s_add 4 f forwarding_rule_vpn ACCEPT 1 { "-i ppp+" }
}

fw_pptp_block(){
    local rule="-p tcp -m tcp --dport 1723"
    fw_s_del 4 f zone_wan_vpn_access ACCEPT 1 { "$rule" }
    fw_s_del 4 f forwarding_rule_vpn ACCEPT 1 { "-i $homeif -o ppp+" }
    fw_s_del 4 f forwarding_rule_vpn 1 { "-i ppp+" }
}
