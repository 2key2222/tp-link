...
STATUS k_mcast_block_source(pi_addr *p_mcastgrp, pi_addr *p_origin, char* ifname)
{
    ...
    if (ifname != NULL) {
        ...
        req.gsr_interface = get_up_if_index();/*if ifindex == 0, kernel will select interface*/

    memcpy(&req.gsr_group, p_mcastgrp, sizeof(req.gsr_group));
    memcpy(&req.gsr_source, p_origin, sizeof(req.gsr_source));

    socket = get_udp_socket(p_mcastgrp->ss.ss_family);
    return setsockopt(socket, imp_family_to_level(p_mcastgrp->ss.ss_family), MCAST_BLOCK_SOURCE,
        &req, sizeof(struct group_source_req));
}

#ifdef ENABLE_JOIN_KERNEL_V3
STATUS k_mcast_join_source_group(pi_addr *p_mcastgrp, pi_addr *p_origin, char* ifname)
{
    struct group_source_req req;
    int socket = 0;
    ...
    socket = get_udp_socket(p_mcastgrp->ss.ss_family);
    return setsockopt(socket, imp_family_to_level(p_mcastgrp->ss.ss_family), MCAST_JOIN_SOURCE_GROUP,
        &req, sizeof(struct group_source_req));
}
#endif
...
STATUS k_del_mfc(pi_addr *p_mcastgrp, pi_addr *p_origin)
{
    ...
    /* We must block the originator to prevent kernel from  creating new unresolved cache */
    /* after the MFC is deleted.*/
    k_mcast_block_source(p_mcastgrp, p_origin, NULL);
