...
proto_add_dns_server() {
	local address="$1"
	network_echo $LOG_PRI_DEBUG "ip-config" "add dns server, $address"
	append PROTO_DNS "$address"
}

proto_add_dns_search() {
	local address="$1"
	network_echo $LOG_PRI_DEBUG "ip-config" "add dns search, $address"
...
proto_add_ipv4_address() {
	...
	local broadcast="$3"
	local ptp="$4"
	network_echo $LOG_PRI_DEBUG "ip-config" "add ip, $address/$mask/$broadcast/$ptp"
...
proto_add_ipv6_address() {
	...
	local valid="$4"
	local offlink="$5"
	network_echo $LOG_PRI_DEBUG "ip6-config" "add ipv6 address, $address/$mask/$preferred/$valid/$offlink"
...
proto_add_ipv4_route() {
	...
	local mask="$2"
	local gw="$3"
	network_echo $LOG_PRI_DEBUG "ip-config" "add route, $target/$mask/$gw//"
...
proto_add_ipv6_route() {
	...
	local metric="$4"
	local valid="$5"
	network_echo $LOG_PRI_DEBUG "ip6-config" "add ipv6 route, $target/$mask/$gw/$metric/$valid"
...
_proto_notify() {
	...
	network_echo $LOG_PRI_DEBUG "netifd" "ubus $options call network.interface notify_proto $(json_dump)"
