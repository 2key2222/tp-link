...
void control_xmit (void *b)
{
    ...
    {
        /* set error information into file:/tmp/xxtpConnectionError
         * if retries >= MAX_SCCRQ_ATTEMPTS (NOW IT IS 3) */
        if (buf->retries == MAX_SCCRQ_ATTEMPTS)
        {
            /* add by lxj */
            FILE *fd = NULL;
            unlink(XXTP_FILE_CONN_ERROR);

            fd = fopen(XXTP_FILE_CONN_ERROR, "w+");
            if (fd == NULL)
            {
                exit(-1);
            }
            fprintf(fd, "%d\n", XXTP_FAIL__PPP_SERVER_NORESPONSE);
            fclose(fd);
            /* add end	*/
        }
