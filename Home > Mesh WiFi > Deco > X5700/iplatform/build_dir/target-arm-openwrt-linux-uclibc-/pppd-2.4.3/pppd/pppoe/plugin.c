...
PPPOEConnectDevice(void)
{
	...
	if(hwnat)
	{
		memset(buf, 0 , 256); /* used by wanDetect, wanDetect write to hnat proc when hnat is enabled */
		sprintf(buf,"[ -f '/tmp/qca_switch/nf_athrs17_hnat' ] && echo %d > /tmp/qca_switch/nf_athrs17_hnat_ppp_id", conn->session);
		system(buf);
		memset(buf, 0 , 256); /* if hnat is already enabled, write to hnat directly */
	...
	if(hwnat)
	{
		...
		sprintf(buf,"[ -f '/tmp/qca_switch/nf_athrs17_hnat' ] && echo %02x%02x%02x%02x%02x%02x > /tmp/qca_switch/nf_athrs17_hnat_ppp_peer_mac",
		...
		sprintf(buf,"[ -f '/tmp/qca_switch/nf_athrs17_hnat' ] && echo %02x%02x%02x%02x%02x%02x > /tmp/qca_switch/nf_athrs17_hnat_wan_mac",
    ...
    if(hwnat)
    {
        system("[ -f '/tmp/qca_switch/nf_athrs17_hnat' ] && echo 1 > /tmp/qca_switch/nf_athrs17_hnat_wan_type");
