...
extern unsigned char serverMAC[6];
#ifndef bool
...
sendPADT(PPPoEConnection *conn, char const *msg)
{
    ...
    if ((!conn->session) && (oldSession == 0xffff ) && (oldSession_v6 == 0xffff)) return;
	...
	{
		if (is_ipv6)
            packet.session = (UINT16_t)oldSession_v6;
		else
    ...
    if (conn->useHostUniq && (oldSession == 0xffff ) && (oldSession_v6 == 0xffff)) {
	PPPoETag hostUniq;
	pid_t pid = getpid();
