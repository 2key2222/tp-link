...
function mimedecode_message_body( src, msg, filecb )
	...
	if mode.is_mem_protect_support() == true and mode.is_tm_shn_support() == true then
		sys.fork_exec("/etc/init.d/tm_shn stop")
		os.execute("sleep 7")
    elseif mode.is_mem_protect_support() == true and mode.is_avira_support() == true then
        sys.fork_exec("/etc/init.d/zzzmonit stop")
        sys.fork_exec("/etc/init.d/url_class stop")
 	    sys.fork_exec("/etc/init.d/cloud_report stop")
 	    sys.fork_exec("/etc/init.d/avirawatchdog stop;sleep 1;/etc/init.d/aviraserviceselector stop;sleep 1;\
 	    	/etc/init.d/avirasentinellite stop;sleep 1;/etc/init.d/avirasentinelfull stop")
     	sys.fork_exec("echo 3 > /proc/sys/vm/drop_caches")  	
     	os.execute("sleep 4")
    end
