...
static bool read_uint64_file(int dir_fd, const char *file, uint64_t *val);
...
#ifdef CONFIG_PRODUCT_IS_SGMAC
static int __get_link_status__(const char* if_name, bool * state)
{
	uint64_t link = 0;
	bool ret = false;
	char buf[64];
	...
	snprintf(buf, sizeof(buf) - 1, "/tmp/net/%s", if_name);
...
#ifdef CONFIG_PRODUCT_IS_SGMAC
	if(false == __get_link_status__(dev->ifname, &state))
#endif	
		state = ifi->ifi_flags & IFF_LOWER_UP ? true : false;
...
#ifndef KEEP_BRIDGE
	if (system_addbr(ridge->ifname) < 0)
		return -1;
#else
	(void)system_addbr(bridge->ifname);
#endif
...
system_get_link_state(struct device *dev)
{
	...
	memset(buf, 0, sizeof(buf));
#ifdef CONFIG_PRODUCT_IS_SGMAC
    if(0 == strcmp("br-wan", dev->ifname) || 0 == strncmp("eth0", dev->ifname, strlen("eth0"))) {
        snprintf(buf, sizeof(buf) - 1, "/tmp/net/%s", dev->ifname);
    }
    else
#endif	 
	{
        snprintf(buf, sizeof(buf) - 1, "/sys/class/net/%s", dev->ifname);
    }
	if (link_state_fd < 0)
	{
		link_state_fd = open(buf, O_DIRECTORY);
