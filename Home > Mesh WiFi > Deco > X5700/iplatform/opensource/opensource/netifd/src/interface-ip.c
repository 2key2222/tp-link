...
interface_ip_add_route(struct interface *iface, struct blob_attr *attr, bool v6)
{
	...
	if ((cur = tb[ROUTE_GATEWAY]) != NULL) {
		...
		if (iface->proto_handler
			&& !strcmp(iface->proto_handler->name, "static")) {
			     ...
			     struct vlist_node *node_old)
{
	...
	if (node_new) {
		...
		{
			/* 
			 * Added by zjj, 20160907, IPv6 default route will not take effect with system_add_route,
			 * use "ip -6 route ..." cmd instead. 
			 */
			if (((route_new->flags & DEVADDR_FAMILY) != DEVADDR_INET4) && (route_new->mask == 0) && 
				(route_new->nexthop.in6.s6_addr32[0] || route_new->nexthop.in6.s6_addr32[1] ||
				 route_new->nexthop.in6.s6_addr32[2] || route_new->nexthop.in6.s6_addr32[3]))
			{
				system_exec_fmt("ip -6 route add default via %x:%x:%x:%x:%x:%x:%x:%x dev %s",
								(ntohl(route_new->nexthop.in6.s6_addr32[0]) & 0xffff0000) >> 16, 
								(ntohl(route_new->nexthop.in6.s6_addr32[0]) & 0xffff),
								(ntohl(route_new->nexthop.in6.s6_addr32[1]) & 0xffff0000) >> 16, 
								(ntohl(route_new->nexthop.in6.s6_addr32[1]) & 0xffff),
								(ntohl(route_new->nexthop.in6.s6_addr32[2]) & 0xffff0000) >> 16, 
								(ntohl(route_new->nexthop.in6.s6_addr32[2]) & 0xffff),
								(ntohl(route_new->nexthop.in6.s6_addr32[3]) & 0xffff0000) >> 16, 
								(ntohl(route_new->nexthop.in6.s6_addr32[3]) & 0xffff),
								dev->ifname);
			}
			else
			{
...
/* add by wanghao  */
static int getCurrentMode(char *mode)
{
	...
	if (ctx == NULL)
	{
		D(INTERFACE, "uci_alloc_context failed...\n");
		return 1;
	}

	uci_set_savedir(ctx, "/var/state");

	strncpy(opt_name, "repacd.repacd.DeviceType", sizeof(opt_name));
	...
	strncpy(mode, ptr.o->v.string, 16);
...
/* add by humin  */
static int getSystemMode(char *sysmode)
{
	struct uci_context *ctx = uci_alloc_context();
	struct uci_ptr ptr;
	char opt_name[64];    
	...
	strncpy(opt_name, "sysmode.sysmode.mode", sizeof(opt_name));
	if (uci_lookup_ptr(ctx, &ptr, opt_name, true) != UCI_OK) 
	{
		uci_perror(ctx, "lookup ptr");
		uci_free_context(ctx);
	...
	strncpy(sysmode, ptr.o->v.string, 16);
    D(INTERFACE, "sysmode:%s\n", sysmode);
	uci_free_context(ctx);
...
interface_write_resolv_conf(void)
{
	...
	/* add by wanghao  */
	uint8_t dnsUpdateFlag = 0;
	char mode[16] = {0};
    char sysmode[16] = {0};
	/* add end  */
           
    if ((getSystemMode(sysmode) == 0) && (0 == strncmp(sysmode, "AP", strlen("AP"))))   
    {        
        //netifd_log_message(L_NOTICE, "sysmod:%s\n", sysmode);    
        return ;
    }
	...
	/* add by wanghao  */
	else if (dnsUpdateFlag == 1 || getCurrentMode(mode) < 0 || strncmp(mode, "RE", 2) != 0)
	{
