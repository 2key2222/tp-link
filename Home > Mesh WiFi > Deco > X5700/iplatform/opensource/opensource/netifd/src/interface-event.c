...
interface_queue_event(struct interface *iface, enum interface_event ev)
{
	...
	D(SYSTEM, "Queue hotplug handler for interface '%s' ev:%d\n", iface->name, ev);
	...
	iface->hotplug_ev = ev;
	/*当一次hotplug 未完成时，我们不应该删除与当前hotplug相类型的事件；因为虽然
	  事件的类型相同，但其产生的结果可能不同，若删除则可能会引起新的结果无法被分发
	  出去，例如：Wan口插拔两次可能会获取不同的IP，若第一次Up事件未处理完时，第
	  二次Up的事件被删除，则最新的wan IP无法被分发，引发lan/wan IP冲突检测失效.*/
	if (last_ev == ev && !list_empty(&iface->hotplug_list)  && !(current == iface))
