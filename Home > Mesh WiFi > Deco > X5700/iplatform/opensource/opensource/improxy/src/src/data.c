...
#ifdef ENABLE_JOIN_KERNEL_V3
    if(get_join_v3_enable()) {
        for(p_node = p_src_list; p_node; p_node = p_node->next) {
            k_mcast_join_source_group(&p_gp->group_addr, &p_node->addr, NULL);
            enter = 1;
        }
    }
    if(!enter) 
