...
static STATUS load_config(const char *file)
{
    ...
    while(fgets(buffer, 1024, fd))
    {
        ...
        if (strcmp(p_token, "igmp") == 0) {
        ...
        else if (strcmp(p_token, "join_v3") == 0) {
            ...
            IMP_LOG_DEBUG("%s forwarding packets form downstream if to upstream if using igmp v3\n",
                           mproxy.join_v3 ? "Enable" : "Disable");
        }
#endif
        else {
...
#ifdef ENABLE_JOIN_KERNEL_V3
int get_join_v3_enable()
{
    return mproxy.join_v3 == 1;
}
#endif
...
int main(int argc, char *argv[])
{
    ...
    mproxy.print_level = LOG_PRI_INFO;
    mproxy.syslog_level = LOG_PRI_INFO;
    ...
    IMP_LOG_INFO("improxy start to work.");
