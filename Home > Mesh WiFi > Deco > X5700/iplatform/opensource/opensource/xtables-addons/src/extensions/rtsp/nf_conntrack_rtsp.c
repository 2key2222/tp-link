...
void destroy_rtsp_exp(struct nf_conn *master_ct)
{
	...
	for (i = 0; i < nf_conntrack_htable_size; i++)
	{
		hlist_nulls_for_each_entry(h, n, &net->ct.hash[i], hnnode)
		{
			...
			{
#if (LINUX_VERSION_CODE >= SEPARATE_KERNEL_VERSION)			
           ...
           struct nf_conn *ct, enum ip_conntrack_info ctinfo, unsigned int protoff)
{
    ...
    uint        rbuf1len = 0;   /* Replacement len (one port) */
    char        rbufa[16];      /* Replacement buffer (all ports) */
    uint        rbufalen = 0;   /* Replacement len (all ports) */
    uint        origoff;
    uint        origlen;
	...
	while (dataoff < datalen)
	{
		...
		if (translen)
		{
			...
			if((pTemp = strstr(pTranData, "client_port=")))
			{
				...
				{
#if (LINUX_VERSION_CODE >= SEPARATE_KERNEL_VERSION)				
					nf_nat_mangle_tcp_packet(skb, ct, ctinfo, protoff, origoff, origlen, rbuf1, rbuf1len);
#else
				...
				{
#if (LINUX_VERSION_CODE >= SEPARATE_KERNEL_VERSION)						
					nf_nat_mangle_tcp_packet(skb, ct, ctinfo, protoff, origoff, origlen, rbufa, rbufalen);
#else
		...
		struct nf_conn *ct, enum ip_conntrack_info ctinfo) 
{
	...
	case IP_CT_DIR_REPLY:
		...
		if (NF_ACCEPT == ret)
		{
		    ret = help_in_ex(skb, rb_ptr, datalen, ct, ctinfo, protoff);
		}			
		break;
	}
