				 ...
				 uint tranoff, uint tranlen)
{
    ...
    u_int32_t   newip;
    u_int16_t   loport, hiport, be_loport;
    uint        off = 0;
    ...
    case pb_range:
        ...
        be_loport = htons(prtspexp->loport + 1);
        ...
        for (loport = prtspexp->loport; loport != 0 && loport != 65535; loport += 2) /* XXX: improper wrap? */
        {
            t->dst.u.udp.port = htons(loport);
            if (nf_ct_expect_related(exp) == 0)
            {
