...
static int log_notify(struct blob_attr *msg)
{
	...
	if (log_type == LOG_NET) {
...
#if 0
		char date[32];
		strftime(date, sizeof(date), "%b %e %T", localtime(&t));
#endif		
		if (LOG_PRI(p) <= log_level)
		{
			//*buf = '\0';
			...
			if (hostname)
				snprintf(buf + strlen(buf), sizeof(buf) - strlen(buf), "%s ", hostname);
			if (log_prefix) {

#if CONFIG_STRING_SUPPORT_STRLCAT
				strlcat(buf, log_prefix, sizeof(buf));
				strlcat(buf, ": ", sizeof(buf));
#else
				strncat(buf, log_prefix, sizeof(buf) - strlen(buf) - 1);
				strncat(buf, ": ", sizeof(buf) - strlen(buf) - 1);
#endif /* CONFIG_STRING_SUPPORT_STRLCAT */
			}
			if (blobmsg_get_u32(tb[LOG_SOURCE]) == SOURCE_KLOG)
			{
#if CONFIG_STRING_SUPPORT_STRLCAT
				strlcat(buf, "kernel: ", sizeof(buf));
#else
		...
		if (LOG_PRI(p) <= log_level)
		{
			//printf("filter_source:%d target: %d filter_source < blobmsg_get_u32(tb[LOG_SOURCE]:%d)\n",			
			//	filter_source, blobmsg_get_u32(tb[LOG_SOURCE]), filter_source < blobmsg_get_u32(tb[LOG_SOURCE]));
			if ((-1 == filter_source) || (filter_source < blobmsg_get_u32(tb[LOG_SOURCE])))
			{
...
static int usage(const char *prog)
{
	fprintf(stderr, "Usage: %s [options]\n"
		...
		"    -L <log_level>	Got the messages which level below 'log_level(1-7)'\n"
		...
		"    -i	<filter_source>		Got the message which source level is large 'filter_source(0/1/2/255)' \n"
		"\n", prog);
	return 1;
}
...
int main(int argc, char **argv)
{
	...
	while ((ch = getopt(argc, argv, "ufcs:l:L:r:F:p:S:P:h:i:")) != -1) {
		switch (ch) {
		case 'u':
			log_udp = 1;
			break;
