...
ProcessSSDPData(int s, const char *bufr, int n,
	...
	int mx_value = -1;
#endif
	/* for philips hue discovery */
	static char IPbuffer[8][16] = {{0}};
	static int count = 0;
	char temp[16] = {0};
	int index = 0;
	FILE* bridgeIpfile = NULL;
	/* end */
	...
	if(memcmp(bufr, "NOTIFY", 6) == 0)
	{
		...
		/* for philips hue discovery */
		if (strstr(bufr, "IpBridge") && (sender->sa_family == AF_INET))
		{
			inet_ntop(sender->sa_family,
		          &((struct sockaddr_in *)sender)->sin_addr,
		          temp, sizeof(temp));
			for (index = 0; index < 8; index++)
			{
				if (strcmp(&IPbuffer[index][0], temp) == 0)
				{
					break;
				}
			}
			...
			if (index == 8)
			{
				...
				if ((bridgeIpfile = fopen("/tmp/huebridge.lease", "w")) != NULL)
				{
					for (index = 0; index < 8; index++)
					{
						if (IPbuffer[index][0] != '\0')
						{
							fprintf(bridgeIpfile, "%s\n", &IPbuffer[index][0]);
						}
					}
					fclose(bridgeIpfile);
				}
