			...
			case S_BOOLEAN:
				if (sym_is_changable(sym))
				{
					...
					if (sym->node && sym->node->range
                                                && *(unsigned int*)(sym->node->range) != 0xff)
					{
						item_make("-%c-", val == no ? ' ' : '*');
					}
					else
			...
			case S_TRISTATE:
				...
				if (sym_is_changable(sym)) {
					...
					if (sym->node && sym->node->range
                                                && *(unsigned int*)(sym->node->range) != 0xff)
					{
						if (*(unsigned int*)(sym->node->range) != (yes | mod))
							item_make("-%c-", ch);
						else
							item_make("{%c}", ch);
					} else 
 ...
 */
	...
	dep_find_all_configs();
#endif  /* IPT_LIST_ALL_CFG_SPT */

/*
 *	read && parse && apply deptable
 *	TengFei, 14/07/04.
 */
#ifdef  IPT_DEP_TREE_ONLY_SPT
	deptab_handler(MODE_TREE_ONLY);
#endif  /* IPT_DEP_TREE_ONLY_SPT */
...
int main(int ac, char **av)
{
	...
	*/
#ifdef  IPT_DEP_TABLE_SUPPORT
        res = deptab_handler(MODE_DEP_TAB);
#endif  /* IPT_DEP_TABLE_SUPPORT */
