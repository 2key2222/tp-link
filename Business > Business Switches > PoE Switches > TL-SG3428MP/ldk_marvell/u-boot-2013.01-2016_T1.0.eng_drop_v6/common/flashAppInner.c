...
#define DBG(arg...) do{if(flash_app_inner_debug){printf("%s,%d:",__FUNCTION__, __LINE__);printf(arg);}}while(0)
...
int flashAppInnerInit(void)
{
    ...
    else
    {
        ...
        if (gev_edge == -1)
        {
            ...
            tpFlashErase(gev_edge/sectorSize, 1);
...
int flashAppInnerSet(FLASH_INNER_ITEM_TYPE type, char* buff, int len, int prompt)
{
    ...
    tpFlashErase((flashInnerEdgeOffset-sectorSize)/sectorSize, (newSize+sectorSize)/sectorSize);
...
REFRESH_INNER_AREA:
    ...
    tpFlashErase((FLASH_FIRST_INNER_ITEM-newSize)/sectorSize, newSize/sectorSize);
