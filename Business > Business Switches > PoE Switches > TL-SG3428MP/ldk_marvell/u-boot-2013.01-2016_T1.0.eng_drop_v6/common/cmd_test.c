...
extern int miniXcat3_main(int argc, char * const argv[]);

/* Mickey - test for miniXcat3 application  */
static int do_miniXcat3(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int rc;
	
	rc = miniXcat3_main( argc,  argv); 
	
	return rc;
}

U_BOOT_CMD(
	mini_xcat3,	CONFIG_SYS_MAXARGS,	1,	do_miniXcat3,
	"mini_xcat3 enables RD/WR from Switch/DFX registers + initialize of In-Band port # as OOB interface during U-Boot",
	NULL
);
