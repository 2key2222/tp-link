...
{
	void __iomem *gpio_out_en_addr = ioremap(system_controller_phys_base-SYSTEM_CONTROLLER_OFFSET+GPIO_MPP_DATA_OUT_EN_OFFSET,sizeof(u32));
	void __iomem *gpio_out_data_addr = ioremap(system_controller_phys_base-SYSTEM_CONTROLLER_OFFSET+GPIO_MPP_DATA_OUT_OFFSET,sizeof(u32));
	u32 out_en = 0;
	u32 out_data = 0;
	...
	if (!system_controller_base) {
		...
		if ( ~out_en & (1<<rst_gpio) )
		{
			rst_invt?(out_data &= ~(1<<rst_gpio)):(out_data |= (1<<rst_gpio));
		}
