...
define ROOTFS_CPIO_CMD
	...
	if [ "$(BR2_ARCH)" = "aarch64" ]; then \
		...
		rm -f $(TARGET_DIR)/tplink/lib/libcrypto.so*; \
		rm -f $(TARGET_DIR)/tplink/lib/libssl.so*; \
		cp -af $(TARGET_DIR)/../build/openssl*/libcrypto.so* $(TARGET_DIR)/usr/lib; \
		cp -af $(TARGET_DIR)/../build/openssl*/libssl.so* $(TARGET_DIR)/usr/lib; \
		cp -af $(TARGET_DIR)/../host/usr/*/sysroot/lib/libnss_dns*  $(TARGET_DIR)/lib; \
		cp $(TARGET_DIR)/../host/usr/*/sysroot/lib/libSegFault.so $(TARGET_DIR)/lib/libSegFault.so; \
		cp -af $(TARGET_DIR)/../host/usr/*/sysroot/lib/libnss_dns*  $(TARGET_DIR)/lib; \
		rm -f $(TARGET_DIR)/usr/lib/libbfd*; \
		rm -f $(TARGET_DIR)/usr/lib/libopcodes*; \
		rm -rf $(TARGET_DIR)/tplink/lib/libbfd*; \
		rm -rf $(TARGET_DIR)/tplink/lib/libopcodes*; \
		rm -f $(TARGET_DIR)/usr/lib/libncurses*; \
		rm -rf $(TARGET_DIR)/tplink/lib/libncurses*; \
		rm -rf $(TARGET_DIR)/lib/libpthread*; \
		rm -rf $(TARGET_DIR)/tplink/lib/libpthread*; \
		cp $(TARGET_DIR)/../host/usr/*/sysroot/lib/libpthread-2.22.so  $(TARGET_DIR)/lib; \
		ln -s libpthread-2.22.so $(TARGET_DIR)/lib/libpthread.so.0; \
		rm -f $(TARGET_DIR)/tplink/app/sbin/gdb*; \
	else \
