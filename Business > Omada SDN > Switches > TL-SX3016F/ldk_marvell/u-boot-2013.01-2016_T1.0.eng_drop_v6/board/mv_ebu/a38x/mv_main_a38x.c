...
int board_init(void)
{
	...
	bd = getenv_ulong("baudrate", 10, CONFIG_BAUDRATE);*/
	
	clock_divisor = (CONFIG_SYS_TCLK / 16)/CONFIG_BAUDRATE;
...
void misc_init_r_env(void)
{
	...
	/*sprintf(tmp_buf,"%d",CONFIG_BAUDRATE);
	env = getenv("baudrate");
	if (!env)
	{
		puts("baudrate:  ");
		setenv("baudrate", tmp_buf);
	}*/
	...
	if (!env)
	{
		sprintf(tmp_buf,"console=ttyS0,%d",CONFIG_BAUDRATE);
		setenv("console", tmp_buf);
	}
	...
	if (!env) {
		...
		sprintf(tmp_buf,"spi%d=spi_flash",ENV_SPI_MAIN_CHANNEL);
		setenv("mtdids", tmp_buf);
#endif
	}
	env = getenv("mtdparts");
	if (!env) {
		...
		sprintf(tmp_buf,"mtdparts=spi%d.%d:896K(boot),128K(env),6144K(sys),10240K(usrimg1),10240K(usrimg2),4096K(usrappfs),1024K(para)",ENV_SPI_MAIN_CHANNEL,ENV_SPI_MAIN_INDEX);
		setenv("mtdparts", tmp_buf);
		setenv("mtdparts_lgcy", "mtdparts=spi_flash:896K(boot),128K(env),6144K(sys),10240K(usrimg1),10240K(usrimg2),4096K(usrappfs),1024K(para)");
#endif
	}
