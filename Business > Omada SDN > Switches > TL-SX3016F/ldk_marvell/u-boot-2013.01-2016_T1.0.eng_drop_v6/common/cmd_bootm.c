...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	case IH_COMP_NONE:
		if (load == blob_start || load == image_start) {
			MESSAGE("   XIP %s ... ", type_name);
			no_overlap = 1;
		} else {
			MESSAGE("   Loading %s ... ", type_name);
...
#ifdef CONFIG_GZIP
	case IH_COMP_GZIP:
		MESSAGE("   Uncompressing %s ... ", type_name);
		...
		ulong *os_len)
{
	...
	*os_data = *os_len = 0;
	...
	case IMAGE_FORMAT_FIT:
		...
		MESSAGE("## Booting kernel from FIT Image at %08lx ...\n",
				img_addr);
		...
		MESSAGE("   Trying '%s' kernel subimage\n", fit_uname_kernel);
