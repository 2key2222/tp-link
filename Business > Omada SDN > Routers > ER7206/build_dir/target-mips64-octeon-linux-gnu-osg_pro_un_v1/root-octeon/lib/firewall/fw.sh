...
fw__exec() { # <action> <family> <table> <chain> <target> <position> <vnet> { <rules> }
    local cmd fam tab chn tgt pos vnet
    local i
    for i in cmd fam tab chn tgt pos vnet; do
        if [ "$1" -a "$1" != '{' ]; then
            eval "$i='$1'"
            shift
        else
	...
	local vnet_prefix
	case "$vnet" in
		# 判断iptables条目加在vnete协议栈还是原协议栈
		vnete) vnet_prefix=vnete ;;
		*) vnet_prefix= ;;
	esac
    case "$fam" in
    ...
    local cmdline="$vnet_prefix $app -w --table ${tab} --${cmd} ${chn} ${pol} ${rule_offset:-${pos}} ${tgt:+--jump "$tgt"}"
    while [ $# -gt 1 ]; do
...
fw_check_icmptype4() {
    ...
    [ -z "$FW_ICMP4_TYPES" ] && \
        export FW_ICMP4_TYPES=$(
            iptables -w -p icmp -h 2>/dev/null | \
