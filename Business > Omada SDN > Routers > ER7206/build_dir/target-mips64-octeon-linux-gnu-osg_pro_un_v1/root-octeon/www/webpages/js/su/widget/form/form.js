...
$.su.Widget("form", {
	...
	doSubmit: function(me, params){
		...
		//console.log(me)
		if (proxy){
			proxy.write($.extend({"params": data}, param), function(data, others, status, xhr){
				//obj.submiting = false;
				me.form("hideLoading");
				...
				if (name){
					...
					if (obj.length != 0){
						...
						if (xtype){
							...
							if (dd !== undefined){
								if("note" == name || "comment" == name || "textname" == name){
									if (dd !== undefined){
										obj[xtype]("setValue", unescape(dd));
									}
								}else{
	...
	prompt: function(me, params){
		...
		if (successed){
			...
			if(showPromptFail){
				//失败
				if(formPrompt.prevObject.selector){/*add by wanglei,使form-login失败提示框由从右边逐渐变大改成由中间逐渐变大*/
					if("form#form-login" == formPrompt.prevObject.selector){
						var r = l/2;
						formPrompt.css({
							left: r,
							right: r,
