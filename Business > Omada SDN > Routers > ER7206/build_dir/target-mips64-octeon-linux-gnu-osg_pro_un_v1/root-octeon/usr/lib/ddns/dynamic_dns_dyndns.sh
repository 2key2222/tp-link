...
dyndns_state_init()
{
    uci_set_state ddns "dyndns_${1}" "" ddns_state
    uci_set_state ddns "dyndns_${1}" state $DYNDNS_STATE_CONNECTING
}
...
dyndns_update()
{
    ...
    if [ -n "$prog" -a -n "$url" ]
    then
        request=$( ${prog} -v -0 -A tplink -H "host: members.dyndns.org" -H "Accept:" --interface ${ifname} "${url} " 2>"/tmp/${iface}_dyndns.error" )
        if [ $? -gt 0 ]; then
            dyndns_result=1
        else
            dyndns_result=0        
...
dyndns_parse_retcode()
{
    local service_id="$1"
    local iface_name="$2"
    local http_res=$(cat "/tmp/${iface_name}_dyndns.error"|grep -o "curl")

    dyndns_code=''
    if [ -n "$http_res" -a "$http_res" == "curl" ]; then
        # Only check 4xx http error code
        local http_code=$(cat "/tmp/${iface_name}_dyndns.error"|grep -o "4[0-9][0-9]")
        case $http_code in
            401) dyndns_code=$DYNDNS_RETCODE_BADAUTH ;;
            404) 
                dyndns_code=$DYNDNS_RETCODE_ERR
                ddns_syslog $DDNS_LOG_NTC_CONN_SVR_FAIL "dyndns" "$iface_name" "$3"
            ;;
            *) 
                dyndns_code=$DYNDNS_RETCODE_UNKNOWN
                #ddns_syslog $DDNS_LOG_WNG_SVR_RESET "dyndns" "$iface_name" "$3"
            ;;
        esac
    else
        ...
        #ddns_syslog $DDNS_LOG_INF_RETCODE "dyndns" $result "$iface_name" "$3"
    fi
    
    # After parsing, then remove the temp http response code file
    rm -f "/tmp/${iface_name}_dyndns.error"
...
dyndns_set_state()
{
    ...
    [ -z "$code" -o '' -eq "$code" ] && {
        uci_toggle_state ddns "dyndns_${2}" state $DYNDNS_STATE_STOP
        ddns_syslog $DDNS_LOG_NTC_LOGIN_FAIL "dyndns" "$2" "$3"
        return    
    }

    if [ $code -eq $DYNDNS_RETCODE_GOOD -o $code -eq $DYNDNS_RETCODE_NOCHG ]; then
        uci_toggle_state ddns "dyndns_${2}" state $DYNDNS_STATE_RUN_OK
        ddns_syslog $DDNS_LOG_NTC_LOGIN_SUCC "dyndns" "$2" "$3"
    elif [ $code -eq $DYNDNS_RETCODE_UNKNOWN ]; then
        uci_toggle_state ddns "dyndns_${2}" state $DYNDNS_STATE_CONNECTING
        #ddns_syslog $DDNS_LOG_INF_CONNING "dyndns" "$2" "$3"
    elif [ $code -eq $DYNDNS_RETCODE_BADAUTH ]; then
        uci_toggle_state ddns "dyndns_${2}" state $DYNDNS_STATE_BADAUTH
        ddns_syslog $DDNS_LOG_NTC_ACC_INCORRECT "dyndns" "$2" "$3"
    elif [ $code -ge $DYNDNS_RETCODE_NOHOST -a $code -le $DYNDNS_RETCODE_NOT_YOURS ]; then
        uci_toggle_state ddns "dyndns_${2}" state $DYNDNS_STATE_BADHOST
        ddns_syslog $DDNS_LOG_NTC_DOMAIN_FAIL "dyndns" "$2" "$3"
    else
        uci_toggle_state ddns "dyndns_${2}" state $DYNDNS_STATE_STOP 
        ddns_syslog $DDNS_LOG_NTC_LOGIN_FAIL "dyndns" "$2" "$3"
    fi
...
dyndns_check()
{
    local s=$(uci_get_state ddns "dyndns_${3}" state)
    
    config_get domain "dyndns_${3}" domain
            ...
            for d in $dns; do
                [ -n "$d" ] && {
                    ...
                    [ "$ns" == "$2" ] && {
                        ...
                        uci_toggle_state ddns "dyndns_${3}" state $DYNDNS_STATE_RUN_OK  
                        ddns_syslog $DDNS_LOG_NTC_LOGIN_SUCC "dyndns" "$3" "$4"                
                        return 0
                    }           
                }
            done        
...
{
    dyndns_retry_int=
    local s=$(uci_get_state ddns "dyndns_${1}" state)

    local chk_int chk_unit chk_int_sec
    config_get chk_int "dyndns_${1}" check_interval
    config_get chk_unit "dyndns_${1}" check_unit
