...
STATUS k_mcast_join(pi_addr* p_addr, char* ifname)
{
    ...
    ret = setsockopt(socket, imp_family_to_level(p_addr->ss.ss_family),
        MCAST_JOIN_GROUP, &req, sizeof(struct group_req));
	if(0 == ret)
	{
		if(AF_INET == p_addr->ss.ss_family)
		{
			IMP_LOG_DEBUG("group ip: %s\r\n", imp_pi_ntoa(p_addr));
			sprintf(cmd, "ipset add improxy_ip_group %s", imp_pi_ntoa(p_addr));
			system(cmd);
			
#if 0
			/* add hw multicast entry */
			if(IGMP_ALL_ROUTERS != ntohl(p_addr->v4.sin_addr.s_addr) &&
				IGMP_ALL_ROUTERS_V3 != ntohl(p_addr->v4.sin_addr.s_addr))
			{
				if(STATUS_OK == mcastIpToMac(p_addr, mac))
				{
					if(STATUS_OK == Mac2Str(mac, macStr, 1))
					{
						if_indextoname(req.gr_interface, upIfname);
    					pStr = strchr(upIfname, '.');
    					if(NULL != pStr)
    					{
    						vid = atoi(pStr+1);
							memset(cmd, 0, sizeof(cmd));
							sprintf(cmd, "hwnat -B %d %s 2 2 4", vid, macStr);
							system(cmd);
    					}
					}
				}
...
STATUS k_mcast_leave( pi_addr* p_addr, char* ifname)
{
    ...
    socket = get_udp_socket(p_addr->ss.ss_family);
    ret = setsockopt(socket, imp_family_to_level(p_addr->ss.ss_family), MCAST_LEAVE_GROUP,
	...
	if(0 == ret)
	{
		...
		{
			IMP_LOG_DEBUG("group ip: %s\r\n", imp_pi_ntoa(p_addr));
			sprintf(cmd, "ipset del improxy_ip_group %s", imp_pi_ntoa(p_addr));
			system(cmd);

#if 0
			/* del hw multicast entry */
			if(IGMP_ALL_ROUTERS != ntohl(p_addr->v4.sin_addr.s_addr) &&
				IGMP_ALL_ROUTERS_V3 != ntohl(p_addr->v4.sin_addr.s_addr))
			{
				if(STATUS_OK == mcastIpToMac(p_addr, mac))
				{
					if(STATUS_OK == Mac2Str(mac, macStr, 1))
					{
    					...
    					if(NULL != pStr)
    					{
    						vid = atoi(pStr+1);
							memset(cmd, 0, sizeof(cmd));
							sprintf(cmd, "hwnat -C %d %s 2 2 4", vid, macStr);
							system(cmd);
    					}
					}
				}
