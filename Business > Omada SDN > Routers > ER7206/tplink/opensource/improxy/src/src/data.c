    ...
    pa_list* p_src_list, group_type type, im_version version)
{
    ...
    pa_list *p_node = NULL;
	BOOL imp_membership_first_add = FALSE;
    ...
    if (imp_membership_first_add && (k_mcast_join(&p_gp->group_addr, NULL) < 0))
    {
        imp_membership_db *p_md;
        LIST_HEAD(, imp_group) group_list = {NULL};

        if (p_md = imp_membership_db_find(p_pig)) {
            imp_membership_db_cleanup(p_md);
        }

        LIST_INSERT_HEAD(&group_list, p_gp, link);
        imp_group_cleanup(p_gp);
        return NULL;
    }
