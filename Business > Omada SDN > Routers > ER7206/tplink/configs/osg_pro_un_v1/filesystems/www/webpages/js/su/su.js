...
if(isIE){
	...
	if(parseFloat(fIEVersion) < 8.0){
		location.href = "./error.html";
	}
	if(parseFloat(fIEVersion) < 9.0){
		$.su.ISIE8 = true;
	}
}
...
$.su.func.isLan = function(iface) {
	return iface && ((iface == "LAN") || ((typeof iface == "string") && iface.match("LAN")));
};
...
$.su.vtype.types = {
		...
		validator: function(value){
			...
			if (this.max !== null && this.max !== undefined){
				if (value > this.max){
					...
					{
						var num = this.max + 1;
						var str = $.su.CHAR.VTYPETEXT.NUMBER_MAX.replace("%max", num.toString());
						return 	str;
					}
				};
			};
			...
			if (this.min !== null && this.min !== undefined){
				if (value < this.min){
					...
					{
						var num = this.min - 1;
						var str = $.su.CHAR.VTYPETEXT.NUMBER_MIN.replace("%min", num.toString());
						return 	str;
					}
				};
			};
	...
	ipv6:{
		...
		validator:function(val)
		{
			var reg0 = new RegExp("^[Ff][Ee]80::")
			if (this.canbeLinkLocalAddr)
			{
				if (reg0.test(val))
				{
					return true;
		    ...
		    var reg1 = new RegExp("^([2-3][0-9A-Fa-f]{1,3}:)|(0[2-3][0-9A-Fa-f]{1,2}:)|(0{1,2}[2-3][0-9A-Fa-f]:)");
			if(!reg1.test(val))
			{
				return $.su.CHAR.VTYPETEXT.IPV6_NOT_GLOBAL;
			}
			var reg2 = new RegExp("::$");
			var reg3 = new RegExp("^2002:");
	...
	ip_domain:{
		...
		vtypeText: $.su.CHAR.VTYPETEXT.IP_DOMAIN,
		validator:function(value){
			if( $.su.vtype.types["string_ip_domain"].regex.test(value) && $.su.vtype.types["string_ip_domain"].validator(value) )
			{
				return true;
			}
	...
	domain_ip:{//this can test ip indeed compared with ip_domain
		allowAllZeroFlag:false,
		regex: /^.+$/,
		vtypeText: $.su.CHAR.VTYPETEXT.IP_DOMAIN,
		validator:function(value){
			if( $.su.vtype.types["ip"].regex.test(value) && true == $.su.vtype.types["ip"].validator(value) )
			{
				return true;
			}
			else if($.su.vtype.types["ip"].regex.test(value))
			{
				if(this.allowAllZeroFlag && ($.su.CHAR.VTYPETEXT.IP_NO_ALL_ZERO == $.su.vtype.types["ip"].validator(value)))
				{
					return true;
				}
				else
	...
	domain_port: {
		...
		vtypeText: $.su.CHAR.VTYPETEXT.IP_DOMAIN,
		validator:function(value){
			var parseVal = value.split(":");
			if (parseVal.length == 1)
			{
				/* domain */
				var domain = parseVal[0];
				if ($.su.vtype.types["domain"].regex.test(value))
				{
					return true;
				}
			...
			else if (parseVal.length == 2)
			{
				...
				var domain = parseVal[0];
				var port = parseVal[1];
				var portNum = parseInt(parseVal[1], 10);
				if ($.su.vtype.types["domain"].regex.test(domain) && 
					$.su.vtype.types["number"].regex.test(port) && 0 < portNum && 65536 > portNum)
				{
					return true;
				}
			}
...
////////////////////////////////////
    ...
    for (var index = 0; index < dataMenu.length; index++){
        ...
        if (fstMenu.children.length === 1 && !fstMenu.children[0].name){//First layer menu directly links to the third layer menu
            ...
            inHTML += 	"<li class=\"fst\" id=\""+"menu-"+settings.idTag+"-"+fstMenu.name+"-li"+"\">";
            inHTML += 		"<a class=\"fst\" data-child=\"yes\" data-name=\""+fstMenu.name+"\" name=\""+fstMenu.name+"\">";
            inHTML += 			"<span class=\"icon\"></span>";
            inHTML += 			"<span class=\"text\">"+fstMenu.text+"</span>";
            inHTML += 		"</a>";
    ...
    inHTML += "<li class=\"fst\" id=\"menu-advanced-logout-li\">";
	inHTML += 		"<a class=\"fst none-sec\" id=\"logoutLink\" data-child=\"no\" data-name=\"logout\" style=\"margin:25px 0 0 0;width:inherit;padding:0 0 0 40px;\">";
	inHTML += 			"<span class=\"text\" >Logout</span>";
	inHTML += "</a></li>";


    inHTML += "</ul>";

	var menu_footer = '<div id="menu-footer" onClick="javascript:window.open(\'http://www.tp-link.com/en/\');"  style="cursor:pointer;">\
		<p>Copyright <span class="copyright">&copy;</span><span id="copy_year"> 2021</span></p>\
		<p>TP-Link Corporation Limited.</p>\
    ...
    container.delegate("ul.menu-tree li.fst a.fst", "click", function(e){
        ...
        var $tab = $("#"+tabId);
        tabContainer.find(".tab-menu-ul").hide();
        if(me.data("name") === "wan-setting" || me.data("name") === "ipv6"){
            ...
            wanProxy.read({}, function(data){
                if(data && data.wanmode){
                    ...
                    $.each($lis, function(i, li){
                        ...
                        var wshow = false;
                        if (i > 0) {
                            var wanid = data.wan_names[i-1].index;
                            var wname = data.wan_names[i-1].name;
                            for (var zz = 0; zz < data.wan_numbers.length; zz++) {
                                if (data.wan_numbers[zz] == wanid) {
                                    wshow = true;
                                    break;
                                }
                            }
                            li.wanid = wanid;
                            $li.find('span').text(wname);
                        } else {
                            wshow = true;
                        }
                        if(wshow){
...
$.su.Menu.prototype.goTo = function(fstName, secName){
    ...
    if (!fstMenu.length){
        fstMenu = container.find("ul.menu-tree li.fst a").first();
        fstName = fstMenu.attr("data-name")
        var secMenu = fstMenu.parent().find('li.sec a').first();
        secName = secMenu.attr("data-name")
    }
