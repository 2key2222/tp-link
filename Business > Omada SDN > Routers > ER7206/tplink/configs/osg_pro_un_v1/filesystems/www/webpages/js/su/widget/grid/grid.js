...
*/
		...
		if (store.updateMode == "operation"){
			...
			$(store).on("ev_insertdata", function(e, index, data){
				...
				// }else{
					...
					me.grid("remove", keyArray, indexArray);
				// };
			});
		}else{
			//全部刷新
		...
		for (var kndex = 0; kndex < len; kndex++){
			...
			switch (column.xtype){
				...
				case "statuscolumn":
					...
					inHTML +=			"<span class=\"text\" style=\"width:60px;\">"+res+"</span>";
                    inHTML +=			"<span class=\"icon\"></span>";
					inHTML +=		"</a>";
					inHTML += 	"</td>";
					break;
		...
		if (tar.isPanel){
			...
			function auto_refresh_cb(){
				...
				if (paging){
					...
					me.grid("getStore").load(params, null, null,null);
				}		
			}
			toolbar.find("input.operation-auto-refresh").checkbox({
				...
				if (vNew.length > 0 && vNew[0] === "auto-refesh"){
					...
					window.autoRefreshId = tar.autoRefreshTime;
				};
			});
	...
	remove: function(me, params){
		...
		if (indexes !== undefined) {
			me.trigger("ev_remove", [keys, indexes]);
		} else {
