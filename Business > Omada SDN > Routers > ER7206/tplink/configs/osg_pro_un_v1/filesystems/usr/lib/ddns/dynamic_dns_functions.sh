...
check_iface_conn()
{
    config_get enable "${1}_poe" t_poe_enable
...
start_daemon_for_all_ddns_sections()
{
    ...
    config_load "network"
    uci_revert_state ddns "${service}_${itrface}"

    ! check_iface_conn "$itrface" && {
        echo "Wan is not connected"
        lock -u /var/run/ddns.lock
        ddns_syslog $DDNS_LOG_NTC_INIT_FAIL "$service" "$itrface"
        return
    }
	...
	do
		local iface
		config_get iface "$section" interface "wan1_eth"
		[ "$iface" = "$event_interface" ] || continue
		[ -z "$service" ] || [ "${service}_${itrface}" = "$section" ] || continue
		/usr/lib/ddns/dynamic_dns_updater.sh $service 0 "$3" > /dev/null 2>&1 &
	done
    lock -u /var/run/ddns.lock
}
...
ddns_get_external_ip()
{
    ...
    then
        case $enable in
            0) # Maybe NAT behind no external ip
				local devname=$(echo $1 | awk '{print toupper($0)}')
				local ifname=$(zone_get_effect_main_wan_devices $devname)
				#echo if_name=$ifname > /dev/console
                external_ip=$(echo $( curl -o - --interface ${ifname} http://checkip.dyndns.org 2>/dev/null) | grep -o "$ip_regex")
                [ -z "$external_ip" ] && {
                    external_ip="0.0.0.0"
                    #ddns_syslog $DDNS_LOG_WNG_GET_EXIP_FAIL "checkip.dyndns.org" "$1"
                }
            ;;
            *) external_ip="0.0.0.0" ;;
        esac
...
ddns_parse_retcode()
{
    ...
    ${service}_parse_retcode $service $iface "$3" "$4"
}
...
ddns_update_check()
{
    ...
    config_get enable "${3}_poe" t_poe_enable
    case $enable in
        1) iface="${3}_poe" ;;
        *) iface="${3}_eth" ;;
    esac

    ${service}_check "$iface" "$ip" "$3" "$4"    
}
...
ddns_retry_interval()
{
    ...
    ddns_retry_int=''
    ddns_retry_int=$(${service}_retry_interval $iface)
