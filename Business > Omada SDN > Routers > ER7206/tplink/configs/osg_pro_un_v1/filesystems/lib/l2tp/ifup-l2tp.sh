...
[ "${enable}" == "yes" ] && { echo "${l2tpifname} has runned, no need up again"; exit 0; }
uci set ${l2tp_client_configname}.${l2tpifname}.enable="yes"
uci commit ${l2tp_client_configname}
/lib/l2tp/l2tp-reload.sh  modify lac ${l2tpid}
lua /lib/l2tp/l2tp-ipsec-setstatus.lua ${l2tpid} on "${l2tpifname}"
