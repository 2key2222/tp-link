...
static char *number(char *buf, char *end __attribute__((unused)),
		    unsigned NUM_TYPE num, int base, int size,
...
static char *string(char *buf, char *end __attribute__((unused)),
...
static char *pointer(const char *fmt __attribute__((unused)), char *buf,
		     char *end, void *ptr, int field_width, int precision,
		     int flags)
{
	if (!ptr)
