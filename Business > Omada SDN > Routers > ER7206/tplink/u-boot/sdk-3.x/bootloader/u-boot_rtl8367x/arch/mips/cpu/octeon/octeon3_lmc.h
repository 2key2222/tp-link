static void perform_ddr3_init_sequence(uint32_t cpu_id, int rank_mask,
                       int ddr_interface_num);

void perform_octeon3_ddr3_sequence(uint32_t cpu_id, int rank_mask, int ddr_interface_num, int sequence)
{
    /*
...
#undef DEBUG_PERFORM_DDR3_SEQUENCE
    ...
    ddr_config_write_csr(CVMX_LMCX_SEQ_CTL(ddr_interface_num), seq_ctl.u64);
                               ...
                               )
{
    ...
    */
...

    {
        ...
        lmc_control.u64 = cvmx_read_csr(CVMX_LMCX_CONTROL(0));
        lmc_scramble_cfg0.u64 = cvmx_read_csr(CVMX_LMCX_SCRAMBLE_CFG0(0));
        lmc_scramble_cfg1.u64 = cvmx_read_csr(CVMX_LMCX_SCRAMBLE_CFG1(0));
        ...
        ddr_config_write_csr(CVMX_LMCX_SCRAMBLE_CFG0(0), lmc_scramble_cfg0.u64);
        ...
        ddr_config_write_csr(CVMX_LMCX_SCRAMBLE_CFG1(0), lmc_scramble_cfg1.u64);

        ddr_config_write_csr(CVMX_LMCX_CONTROL(0), lmc_control.u64);
    }
...
#define DDR3_tWLDQSEN      25         /* 25 nCK */
    ...
    /* LMC(0)_CONFIG */
    {
        ...
        lmcx_config.cn78xx.early_dqx       = 0; /* Default to enabled */

        if ((s = lookup_env_parameter("ddr_early_dqx")) == NULL)
            s = lookup_env_parameter("ddr%d_early_dqx", ddr_interface_num);
        if (s != NULL) {
            lmcx_config.cn70xx.early_dqx = simple_strtoul(s, NULL, 0);
        }
        ...
        ddr_config_write_csr(CVMX_LMCX_CONFIG(ddr_interface_num), lmcx_config.u64);
    }
    ...
    {
        /* .cn70xx. */
        dual_memcfg.cn70xx.cs_mask  = 0;
        dual_memcfg.cn70xx.row_lsb  = 0;
    }
#endif
...

    {
        ...
        ddr_config_write_csr(CVMX_LMCX_EXT_CONFIG(ddr_interface_num), ext_config.u64);
    }
...
#pragma pack(pop)
        ...
        ** 4=30, 5=20 */
        ...
        if (rlevel_comp_offset != 0) {
            ...
            lmc_comp_ctl2.cn78xx.ptune += rlevel_comp_offset/2;
            lmc_comp_ctl2.cn78xx.ptune -= rlevel_comp_offset/2;
        ...
        for (rodt_ctl=max_rodt_ctl; rodt_ctl>=min_rodt_ctl; --rodt_ctl) {
            ...
            for (rankx = 0; rankx < dimm_count*4; rankx++) {
                ...
                while (average_loops--) {
...

                    {
                        ...
                        if (ecc_ena) {
                            ...
                            rlevel_byte[8].delay = lmc_rlevel_rank.cn70xx.byte8; /* unused */
                            rlevel_byte[7].delay = lmc_rlevel_rank.cn70xx.byte7; /* unused */
                            rlevel_byte[6].delay = lmc_rlevel_rank.cn70xx.byte6; /* unused */
                            rlevel_byte[5].delay = lmc_rlevel_rank.cn70xx.byte5; /* unused */
                            rlevel_byte[4].delay = lmc_rlevel_rank.cn70xx.byte4; /* ECC */
                        }
                        ...
                        if (ddr_rlevel_compute) {
                            ...
                            if (ecc_ena) {
                                ...
                                rlevel_byte[4].delay = lmc_rlevel_rank.cn70xx.byte8; /* unused */
                                rlevel_byte[7].delay = lmc_rlevel_rank.cn70xx.byte7; /* unused */
                                rlevel_byte[6].delay = lmc_rlevel_rank.cn70xx.byte6; /* unused */
                                rlevel_byte[5].delay = lmc_rlevel_rank.cn70xx.byte5; /* unused */
                                rlevel_byte[4].delay = lmc_rlevel_rank.cn70xx.byte4; /* ECC */
                ...
                if (ecc_ena) {
                    ...
                    lmc_rlevel_rank.cn70xx.byte8 = rlevel_byte[8].delay;
                    lmc_rlevel_rank.cn70xx.byte7 = rlevel_byte[7].delay;
                    lmc_rlevel_rank.cn70xx.byte6 = rlevel_byte[6].delay;
                    lmc_rlevel_rank.cn70xx.byte5 = rlevel_byte[5].delay;
                    lmc_rlevel_rank.cn70xx.byte4 = rlevel_byte[4].delay;
...

        {
            ...
            for (rankx = 0; rankx < dimm_count * 4;rankx++) {
                ...
                /*ddr_print(" manually training for resovling power-onoff error. by liwei\n");*/
                for (byte_idx=0; byte_idx<9; ++byte_idx) {
                    if (!(ddr_interface_bytemask&(1<<byte_idx)))
                        continue;
                    update_rlevel_rank_struct(&lmc_rlevel_rank, byte_idx, 5);
                }

                ddr_config_write_csr(CVMX_LMCX_RLEVEL_RANKX(rankx, ddr_interface_num), lmc_rlevel_rank.u64);
                lmc_rlevel_rank.u64 = cvmx_read_csr(CVMX_LMCX_RLEVEL_RANKX(rankx, ddr_interface_num));
                printf("Rank(%d) Rlevel Rank %#5x, 0x%016llX : %5d %5d %5d %5d %5d %5d %5d %5d %5d (%d)\n",
                          rankx,
                          lmc_rlevel_rank.cn70xx.status,
                          lmc_rlevel_rank.u64,
                          lmc_rlevel_rank.cn70xx.byte8,
...

    {
        ...
        lmc_config.u64 = cvmx_read_csr(CVMX_LMCX_CONFIG(ddr_interface_num));
        save_ecc_ena = lmc_config.cn70xx.ecc_ena;
        lmc_config.cn70xx.ecc_ena = 0;
        ddr_config_write_csr(CVMX_LMCX_CONFIG(ddr_interface_num), lmc_config.u64);
        ...
        ** ranks 0 and 2, but only 2 ranks are active. */
        ...
        for (rankx = 0; rankx < dimm_count * 4;rankx++) {
            ...
            if (wlevel_bitmask_errors == 0) {
                ...
                if ((ddr_interface_bytemask&0xff) == 0xff) {
                    if (save_ecc_ena) {
                        int save_byte8 = lmc_wlevel_rank.cn70xx.byte8;
                        byte_test_status[8] = WL_HARDWARE; /* H/W delay value */

                        if ((save_byte8 != lmc_wlevel_rank.cn70xx.byte3) &&
                            (save_byte8 != lmc_wlevel_rank.cn70xx.byte4))
                        {
                            int test_byte8 = divide_nint(lmc_wlevel_rank.cn70xx.byte3
                                                         + lmc_wlevel_rank.cn70xx.byte4
                                                         + 2 /* round-up*/ , 2);
                            lmc_wlevel_rank.cn70xx.byte8 = test_byte8 & ~1; /* Use only even settings */
                        }

                        if (lmc_wlevel_rank.cn70xx.byte8 != save_byte8) {
                            /* Change the status if s/w adjusted the delay */
                            byte_test_status[8] = WL_SOFTWARE; /* Estimated delay */
                        }
                    } else {
                        lmc_wlevel_rank.cn70xx.byte8 = lmc_wlevel_rank.cn70xx.byte0; /* ECC is not used */
                    }
                } else {
                    if (save_ecc_ena) {
                        /* Estimate the ECC byte delay  */
                        if (lmc_wlevel_rank.cn70xx.byte4 < lmc_wlevel_rank.cn70xx.byte3)
                           lmc_wlevel_rank.cn70xx.byte4 = lmc_wlevel_rank.cn70xx.byte3;
                    } else {
                        lmc_wlevel_rank.cn70xx.byte4 = lmc_wlevel_rank.cn70xx.byte0; /* ECC is not used */
                    }
                }
            }
            ...
            if (bytes_failed) {
                ...
                for (byte=0; byte<8; ++byte) {
                    if ((ddr_interface_bytemask&(1<<byte)))
                        continue;
                    update_wlevel_rank_struct(&lmc_wlevel_rank, byte, lmc_wlevel_rank.cn70xx.byte0);
                    byte_test_status[byte] = WL_SOFTWARE;
                }
            ...
            ddr_print("Rank(%d) Wlevel Rank %#5x, 0x%016llX : %5d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %2d%3s %s\n",
                      ...
                      lmc_wlevel_rank.cn70xx.status,
                      lmc_wlevel_rank.u64,
                      lmc_wlevel_rank.cn70xx.byte8, wl_status_strings[byte_test_status[8]],
                      lmc_wlevel_rank.cn70xx.byte7, wl_status_strings[byte_test_status[7]],
                      lmc_wlevel_rank.cn70xx.byte6, wl_status_strings[byte_test_status[6]],
                      lmc_wlevel_rank.cn70xx.byte5, wl_status_strings[byte_test_status[5]],
                      lmc_wlevel_rank.cn70xx.byte4, wl_status_strings[byte_test_status[4]],
                      lmc_wlevel_rank.cn70xx.byte3, wl_status_strings[byte_test_status[3]],
                      lmc_wlevel_rank.cn70xx.byte2, wl_status_strings[byte_test_status[2]],
                      lmc_wlevel_rank.cn70xx.byte1, wl_status_strings[byte_test_status[1]],
                      lmc_wlevel_rank.cn70xx.byte0, wl_status_strings[byte_test_status[0]],
        ...
        lmc_config.cn70xx.ecc_ena = save_ecc_ena;
        ddr_config_write_csr(CVMX_LMCX_CONFIG(ddr_interface_num), lmc_config.u64);
    ...
    if (((s = lookup_env_parameter("ddr_dll_write_offset")) != NULL) ||
        ...
        for (i=0; i<9; ++i) {
            ...
            offset[i] = load_dll_offset(ddr_interface_num, cpu_id, 1 /* write */, byte_offset, i);
        }
    ...
    if (((s = lookup_env_parameter("ddr_dll_read_offset")) != NULL) ||
        ...
        for (i=0; i<9; ++i) {
            ...
            offset[i] = load_dll_offset(ddr_interface_num, cpu_id, 2 /* read */, byte_offset, i);
        }
    ...
    if ((s = lookup_env_parameter("ddr_auto_set_dll_offset")) != NULL) {
        ...
        auto_set_dll_offset( 1 /* 1=write */, ddr_interface_num, cpu_id, ddr_interface_64b);
        auto_set_dll_offset( 2 /* 2=read */, ddr_interface_num, cpu_id, ddr_interface_64b);
