			...
			if (a->w32(phdr->p_type) == PT_LOAD) {
				...
				if (filesz) {
					debug ("  Loading 0x%llx bytes at %llx\n",
					       filesz,
					       octeon_fixup_xkphys (paddr));
					memcpy64(octeon_fixup_xkphys(paddr),
						 (int32_t) image,
						 filesz);
				}
				if (memsz > filesz) {
					...
					debug ("  Clearing 0x%llx bytes at %llx\n",
					       memsz - filesz,
					       octeon_fixup_xkphys(paddr) + filesz);
					memset64(octeon_fixup_xkphys(paddr) + filesz,
						 0,
						 memsz - filesz);
#else
					memset64(paddr + filesz,
						 0,
						 memsz - filesz);
#endif
				}
				if (is_little_endian_elf(addr)) {
					/* swap the bytes */
					uint64_t base = octeon_fixup_xkphys(paddr);
					uint64_t pos = 0;
					uint64_t ptr;
					uint64_t v;
...
#ifdef CONFIG_OCTEON
		if (shdr->sh_type == SHT_NOBITS) {
			memset64 (octeon_fixup_xkphys (shdr->sh_addr),
				  0, shdr->sh_size);
		} else {
			image = (unsigned char *)addr + shdr->sh_offset;
			memcpy64 (octeon_fixup_xkphys (shdr->sh_addr),
				  (u32) image, shdr->sh_size);
		}
#else
		if (shdr->sh_type == SHT_NOBITS) {
			memset64 (shdr->sh_addr, 0, shdr->sh_size);
		} else {
			image = (unsigned char *)addr + shdr->sh_offset;
			memcpy64 (shdr->sh_addr, (u32) image, shdr->sh_size);
		}
#endif
		flush_cache (shdr->sh_addr, shdr->sh_size);
	}
