...
/**
 *
 * $Id: octeon_boot.c 100542 2014-06-25 01:45:26Z terenditsov $
 *
 */
...
#    ifdef CONFIG_CMD_FDT
		const void *fdt_addr = working_fdt;
#    else
...
#    endif
		...
		if (image_flags & OCTEON_BOOT_DESC_LITTLE_ENDIAN) {
			...
			memcpy64((uint64_t)OCTEON_FDT_LE_BASE,
				 (uint64_t)(unsigned long)fdt_addr,
				 (fdt_size + 7) & 0xfffffff8);
			...
			cvmx_bootinfo_array[core].fdt_addr =
					(uint32_t)OCTEON_FDT_LE_BASE;
		}
		else
			cvmx_bootinfo_array[core].fdt_addr =
						(uint32_t)fdt_addr;
...
cvmx_coremask_t *octeon_get_available_coremask(cvmx_coremask_t *pcm)
{
	...
	if (octeon_has_feature(OCTEON_FEATURE_CIU3))
		ciu_fuse = (cvmx_read_csr(CVMX_CIU3_FUSE) & 0x0000FFFFFFFFFFFFULL);
	else
...
void start_cores(cvmx_coremask_t *pcm_to_start)
{
	...
	cvmx_coremask_set64(&invert_coremask, 0x0000FFFFFFFFFFFFULL);
...
int octeon_bootloader_shutdown(void)
{
	...
	int rc = cvmx_bootmem_reserve_memory(0, OCTEON_RESERVED_LOW_BOOT_MEM_SIZE,
					     "__low_reserved", 0);
	if (!rc) {
		printf("Error reserving low 1MB of memory\n");
	}
