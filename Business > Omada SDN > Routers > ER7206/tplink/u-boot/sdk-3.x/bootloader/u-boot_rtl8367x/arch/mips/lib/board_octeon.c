...
{
	puts("Disable watchdog! GPIO[8]=1 \n");
...
static void octeon_enable_all_cores(void)
{
	...
	if (octeon_has_feature(OCTEON_FEATURE_CIU3)) {
		for (node = 0; node < CVMX_MAX_NODES; node++) {
			if (gd->arch.node_mask & (1 << node))
				cvmx_write_csr_node(node, CVMX_CIU3_PP_RST, 0);
		}
	} else {
...
#endif
	octeon_boot_bus_moveable_init,		/* 11 */
	early_board_init,			/* 12 */
...
void board_init_f(ulong bootflag)
{
	...
	ulong image_size = (ulong)&uboot_end_data - CONFIG_SYS_MONITOR_BASE;
	u64 addr;
	 ...
	 */
	if (octeon_has_feature(OCTEON_FEATURE_CIU3))
		cvmx_write_csr_node(0, CVMX_CIU3_WDOGX(0), 0);
	else
...
#ifdef DEBUG
		debug("  start: %p\n", &uboot_start);
		debug("  start: 0x%llx, size: 0x%x\n",
		      cvmx_ptr_to_phys(&uboot_start), image_size);
#endif
		cvmx_l2c_unlock_mem_region(cvmx_ptr_to_phys(&uboot_start),
					   image_size);
		debug(" Done.\n");
	}
	/*
...
void board_init_r(gd_t * id, u64 dest_addr)
{
	...
	for (i = 0; i < num_cores; i++) {
		if (octeon_has_feature(OCTEON_FEATURE_CIU3))
			cvmx_write_csr_node(0, CVMX_CIU_WDOGX(i), 0);
		else
