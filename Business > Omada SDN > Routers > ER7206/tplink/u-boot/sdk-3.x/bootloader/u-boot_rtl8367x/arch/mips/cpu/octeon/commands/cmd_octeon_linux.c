 ...
 * All rights reserved.
 * Copyright (c) 2003-2013 Cavium Inc. (support@cavium.com).
...
extern cvmx_bootinfo_t cvmx_bootinfo_array[CVMX_MIPS_MAX_CORES];
...
int do_bootoctlinux(cmd_tbl_t * cmdtp, int flag, int argc, char *const argv[])
{
	...
	octeon_get_available_coremask(&core_mask);
#else
	...
	for (i = 2; i < argc; i++) {
		...
		//if( i == 8)
		//{
		//	strcpy(argv[i],"mtdparts=octeon_nand0:2M(bootloader),4M(kernel),19M(rootfs),128K(partition-table),128K(device-info),128K(support-list),128K(firmware-info),128K(extra-para),5504K(rootfs_data),1M(panic-oops),32M@0x000000(firmware);spi32766.0:2M(bootloader-factory),128K(tddp),128K(log)\0");
		//	printf("argv[%d]: %s\n", i, argv[i]);
		//}
		//#endif
