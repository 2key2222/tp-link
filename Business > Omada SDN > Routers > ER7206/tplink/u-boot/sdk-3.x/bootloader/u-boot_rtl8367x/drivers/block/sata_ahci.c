 ...
 *
 * Copyright (C) Cavium, Inc. 2011, 2013
			       ...
			       u32 opts)
{
	...
	debug("%s: opts: 0x%x, addr: 0x%x\n", __func__, opts, cmd_tbl_dma);

	pp->cmd_slot[tag].opts = cpu_to_le32(opts);
	pp->cmd_slot[tag].status = 0;
	pp->cmd_slot[tag].tbl_addr = cpu_to_le32(cmd_tbl_dma & 0xffffffff);
	pp->cmd_slot[tag].tbl_addr_hi = cpu_to_le32((cmd_tbl_dma >> 16) >> 16);
	flush_dcache_range(&pp->cmd_slot[tag],
			   &pp->cmd_slot[tag] + sizeof(struct ahci_cmd_hdr));
				...
				unsigned long timeout_msec)
{
	...
	debug("%s(%d pmp: 0x%x, is_cmd: %d, flags: 0x%x)\n", __func__,
	      ap->port_no, pmp, is_cmd, flags);

	/* prep the command */
	ata_tf_to_fis(tf, pmp, is_cmd, fis);

	ahci_fill_cmd_slot(pp, 0, cmd_fis_len | flags | (pmp << 12));
	flush_dcache_range(fis, fis + cmd_fis_len * 4);
		      ...
		      int (*check_ready)(struct ata_link *link))
{
	...
	unsigned long now, msecs = deadline;
	struct ata_taskfile tf;
	int rc;
...
static int ahci_init_one(pci_dev_t pdev)
{
	...
	debug("%s: last port index: %d, number of ports for host %d: %d\n",
	      __func__, last_port_idx, last_host_idx, host->n_ports);
...
static int ahci_exec_internal(struct ata_device *dev, struct ata_taskfile *tf,
			      const u8 *cdb, int cdb_len, int dma_dir,
			      ...
			      unsigned long timeout)
{
	...
	debug("%s(0x%p, 0x%p, 0x%p, %d, %d, 0x%p, 0x%x, %lu)\n",
	      __func__, dev, tf, cdb, cdb_len, dma_dir, buf, buflen, timeout);
	...
	if (is_atapi) {
		...
		memcpy(cmd_tbl + AHCI_CMD_TBL_CDB, cdb, cdb_len);
	}
	...
	if (dma_dir != DMA_NONE) {
		...
		if (dma_dir == DMA_TO_DEVICE || dma_dir == DMA_BIDIRECTIONAL)
			flush_dcache_range(buf, (uint8_t *)buf + buflen);
		psg = &sg;
	}
	...
	if (dma_dir == DMA_FROM_DEVICE || dma_dir == DMA_BIDIRECTIONAL)
		invalidate_dcache_range(buf, (uint8_t *)buf + buflen);
...
int ata_exec_internal(struct ata_device *dev, struct ata_taskfile *tf,
		      ...
		      unsigned long timeout)
{
	return ahci_exec_internal(dev, tf, cdb, cdb_len, dma_dir, buf,
				  buflen, timeout);
}
#endif
			    ...
			    u8 *buffer, int is_write, int is_lba48)
{
	...
	rc = ahci_exec_internal(dev, &tf, NULL, 0,
...
retry:
	...
	rc = ahci_exec_internal(dev, &tf, NULL, 0, DMA_FROM_DEVICE, id,
...
int scan_sata(int dev)
{
	...
	mdelay(1000);
	port_mmio = ahci_port_base(ap);
	if (!(ahci_readl(port_mmio + PORT_CMD) & PORT_CMD_CPS)) {
		debug("Port %d disabled\n", ap->port_no);
		return 1;
	}
	...
	rc = ahci_do_softreset(ahci_dev->link, &class, pmp, 5000,
			       ahci_check_ready);
	if (!rc) {
	...
	strncpy(sata_dev_desc[dev].product, serial,
		sizeof(sata_dev_desc[dev].product));

	/* Firmware version */
	ata_id_c_string(id, firmware, ATA_ID_FW_REV, sizeof(firmware));
	strncpy(sata_dev_desc[dev].revision, firmware,
		sizeof(sata_dev_desc[dev].revision));

	/* Product model */
	ata_id_c_string(id, product, ATA_ID_PROD, sizeof(product));
	strncpy(sata_dev_desc[dev].vendor, product,
...
int ahci_init(u32 base)
{
	...
	debug("%s: Link map is 0x%x\n", __func__, linkmap);

	for (i = 0; i < CONFIG_SYS_SATA_MAX_DEVICE; i++) {
		if (((linkmap >> i) & 0x01)) {
			debug("%s: Starting port %d\n", __func__, i);
			if (ahci_port_start((u8) i)) {
				printf("Can not start port %d\n", i);
				continue;
			}
#ifdef CONFIG_AHCI_SETFEATURES_XFER
			ahci_set_feature((u8) i, 0);
#endif
		}
	}
err_out:
