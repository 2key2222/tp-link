...
static ulong mem_test_bit(vu_long *buf, ulong start_addr, ulong end_addr,
                vu_long pattern, int iteration)
{
    vu_long *end;
    vu_long *addr;
    ...
    for (bit = 0; bit < 32; ++ bit) 
    {
        ...
        printf("\nbit %2d Pattern %08lX  Writing...", bit, pattern);
...
static int do_mem_mtest_quick(cmd_tbl_t *cmdtp, int flag, int argc,
            char * const argv[])
{
    ulong start, end;
    vu_long *buf, *dummy;
...
static int do_mem_mtest_bit(cmd_tbl_t *cmdtp, int flag, int argc,
            char * const argv[])
{
    ulong start, end;
    vu_long *buf, *dummy;
    ...
    for (iteration = 0;
        ...
        if (alt_test) {
            ...
            errs = mem_test_bit(buf, start, end, pattern,
                          iteration);
        }
        if (errs == -1UL)
            break;
...
__weak void board_show_dram(u64 size)
{
    puts("DRAM:  ");
    print_size(size, "\n");
}
...
U_BOOT_CMD(
    mtest_quick,  5,  1,  do_mem_mtest_quick,
    "simple RAM read/write test(pattern, ~pattern)",
    "[start [end [pattern [iterations]]]]"
);
U_BOOT_CMD(
    mtest_bit,  5,  1,  do_mem_mtest_bit,
    "simple RAM read/write test(only set a bit[0-31])",
    "[start [end [pattern [iterations]]]]"
);
#endif  /* CONFIG_CMD_MEMTEST */
