...
int login(struct session * s)
{
	...
	if(err)
	{
		...
		LINE_BACKUP_LOGGER_REG(LOGGER_INFO_ID_BPA_FAIL_AUTH_SERVER, LOGGER_PARAM_STRING, s->zonex);
		closesocket(authsocket);
		return 0;
	}
...
skippo:
	...
	extract_valuestring(s,&t,T_PARAM_RESPONSE_TEXT,s->resptext);
	{
		...
		for(i=0;i<s->tsmcount;i++)
		{
			...
			{
				int port = s->statusport;
				LINE_BACKUP_LOGGER_REG(LOGGER_INFO_ID_BPA_AUTH_SERVER, LOGGER_PARAM_STRING, s->zonex,
					LOGGER_PARAM_STRING, inet_ntoa(s->tsmlist_in[i].sin_addr), LOGGER_PARAM_INT32, port);
			}
		}
	}
	logintime = time(NULL);
	...
	LINE_BACKUP_LOGGER_REG(LOGGER_INFO_ID_BPA_SUCC_CONN, LOGGER_PARAM_STRING, s->zonex);
	s->sequence = 0;
...
int handle_heartbeats(struct session *s)
{
	...
	while(1)
	{
		...
		if(transactiontype == 0xffff)
		{
			...
			LINE_BACKUP_LOGGER_REG(LOGGER_INFO_ID_BPA_TIMEOUT, LOGGER_PARAM_STRING, s->zonex);
			if(!login(s))
				return 0;
			else
...
int logout(INT2 reason,struct session * s)
{
	...
	*/
	...
	LINE_BACKUP_LOGGER_REG(LOGGER_INFO_ID_BPA_SUCC_DISCONN, LOGGER_PARAM_STRING, s->zonex);
	closesocket(authsocket);
