...
/* phy num */
#define AG71XX_PHY_NUM 5

/* Debug port register offsets */
#define AG71XX_REG_DBG_PORT_ADDRESS 29
#define AG71XX_REG_DBG_PORT_DATA 30
...
#ifdef CONFIG_ATH79_SOC_QCA9558
/*
 * qca9558_phy_read - Convenience function for qca9558 to read PHY register
 * @phydev: the phy_device struct
 * @regnum: register number to read
 * @offset: the offset of register base address
 *
 * NOTE: MUST NOT be called from interrupt context,
 * because the bus read/write functions may wait for an interrupt
 * to conclude the operation.
 * add by liyi<liyi@tp-link.net>
 */
static inline int qca9558_phy_read(struct phy_device *phydev, u32 regnum,
								   u16 offset)
{
	return mdiobus_read(phydev->bus, phydev->addr + offset, regnum);
}	

/*
 * qca9558_phy_write - Convenience function for qca9558 to write PHY register
 * @phydev: the phy_device struct
 ...
 * add by liyi<liyi@tp-link.net>
 */
static inline int qca9558_phy_write(struct phy_device *phydev, u32 regnum, 
									u16 val, u16 offset)
{
	return mdiobus_write(phydev->bus, phydev->addr + offset, regnum, val);
}
