...
static int __devinit ag71xx_mdio_probe(struct platform_device *pdev)
{
	...
	/* switch set to power save */
	for (i = 0; i < AG71XX_PHY_NUM; i++){
		am->mii_bus->write(am->mii_bus, i, AG71XX_REG_DBG_PORT_ADDRESS, 5);
		am->mii_bus->write(am->mii_bus, i, AG71XX_REG_DBG_PORT_DATA, 0x2c45);
	}
