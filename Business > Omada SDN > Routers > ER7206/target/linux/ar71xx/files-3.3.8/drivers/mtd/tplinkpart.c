...
#if defined(CONFIG_ATH79_MACH_TL_WVR450G)
				   ...
				   struct mtd_part_parser_data *data)
{
	...
	printk("@@-----debug wvr450g--tplink flash parse--------\n");
	...
	parts[1].size = 0x300000;

	parts[2].name = "rootfs";
	parts[2].offset = 0x320000;
	parts[2].size = 0x4e0000;

	parts[3].name = "firmware";
	parts[3].offset = 0x20000;
	parts[3].size = 0x6a0000;
...
#elif defined(CONFIG_ATH79_MACH_TL_WVR1300G) || defined(CONFIG_ATH79_MACH_TL_R478GV2)
static int tplink_parse_partitions(struct mtd_info *master,
				   struct mtd_partition **pparts,
				   struct mtd_part_parser_data *data)
{
	...
	printk("@@-----debug wvr1300g--tplink flash parse--------\n");
	nr_parts = 10;
	parts = kzalloc(nr_parts * sizeof(struct mtd_partition), GFP_KERNEL);
	if (!parts) {
		ret = -ENOMEM;
	...
	parts[2].size   = 0x00ba0000;

    parts[3].name   = "device-info";
	parts[3].offset = 0x00d30000;
	parts[3].size   = 0x00010000;

    parts[4].name   = "firmware-info";
	...
	parts[6].size   = 0x00020000;

    parts[7].name   = "rootfs_data";
	parts[7].offset = 0x00df0000;
	parts[7].size   = 0x00200000;

    parts[8].name   = "art";
	parts[8].offset = 0x00ff0000;
...
#elif defined(CONFIG_ATH79_MACH_TL_R600VPN_V4)
static int tplink_parse_partitions(struct mtd_info *master,
				   struct mtd_partition **pparts,
				   struct mtd_part_parser_data *data)
{
	...
	printk("@@-----debug r600vpn_v4--tplink flash parse--------\n");
	nr_parts = 10;
	parts = kzalloc(nr_parts * sizeof(struct mtd_partition), GFP_KERNEL);
	if (!parts) {
		ret = -ENOMEM;
...
#elif defined(CONFIG_ATH79_MACH_TL_ER604W_V3)
static int tplink_parse_partitions(struct mtd_info *master,
				   struct mtd_partition **pparts,
				   struct mtd_part_parser_data *data)
{
	...
	printk("@@-----debug er604w_v3--tplink flash parse--------\n");
	nr_parts = 11;
	parts = kzalloc(nr_parts * sizeof(struct mtd_partition), GFP_KERNEL);
	if (!parts) {
		ret = -ENOMEM;
    ...
    parts[8].name   = "rootfs_data";
	parts[8].offset = 0x00d90000;
	parts[8].size   = 0x00200000;

    parts[9].name   = "art";
	parts[9].offset = 0x00ff0000;
	parts[9].size   = 0x00010000;

	parts[10].name  = "firmware";
	parts[10].offset= 0x00000000;
	parts[10].size  = 0x01000000; /* the whole 16MB flash. */
