...
ar8216_phy_enable(struct ar8216_priv *priv)
{
	...
	for (phyid = 0; phyid < AR8327_NUM_PHYS; phyid++) {
		...
		ar8216_phy_dbg_read(priv, phyid, 0, &value);
		value &= (~(1<<12));
		ar8216_phy_dbg_write(priv, phyid, 0, value);
		mdelay(100);
...
#if 0
int
qca_qm_err_recovery(struct ar8216_priv *priv, u32 force_recovery);
int qca_port0_qm_err_recovery(struct ar8216_priv *priv);
#endif
//#endif
static void
...
#if 0
#ifdef MANUAL_S17C_RECOVERY 
	//printk("WJL: qca_ar8327_sw_get_port_link, port-%d.\n", port);

	if (port == 6)
	{
		//printk("WJL: %d, do switch_recovery...\n", port);
		qca_qm_err_recovery(priv, FORCE_RECOVERY);
		return;
	}
#endif
#endif
...
#define AR8327_REG_GLB_INIT0 0x20
...
#define SSDK_GLOBAL_INITIALIZED_STATUS	((1<<24)|(1<<25)|(1<<26)|(1<<27)|(1<<28)|(1<<29))
	do {
		udelay(10);
		value = priv->read(priv, AR8327_REG_CTRL);
	...
	{
		priv->write(priv, AR8327_SGMII_CTRL_REG, 0xc74164de);//set sgmii txrx enable
		priv->write(priv, AR8327_REG_POWER_ON_STRIP, 0x602613a0);//set serdes auto-negotiation disabled, force to 1000M link
	}
    ...
    //printk("WJL: qca_ar8327_hw_init() reg-0x30=0x%08x.2.\n", value);
        ...
        for (i = 0; i < AR8327_NUM_PHYS; i++) {
                ...
                mdiobus_write(bus, i, MII_CTRL1000, ADVERTISE_1000FULL);
                //mdiobus_write(bus, i, MII_BMCR, BMCR_RESET | BMCR_ANENABLE );
                //data = mdiobus_read(bus, i, MII_BMCR);
                //data |= BMCR_PDOWN;
                //mdiobus_write(bus, i, MII_BMCR, data );
#endif                
        }
        ...
        msleep(1000);
#if 0		
		data = priv->read(priv,AR8327_REG_MODULE_EN);
		data &= ~((1<<8)|(1<<9));
		priv->write(priv,AR8327_REG_MODULE_EN,data);
		
        priv->write(priv,AR8327_SGMII_CTRL_REG,0xc74164de);//set sgmii txrx enable
        priv->write(priv,AR8327_REG_POWER_ON_STRIP,0x2613a0);//set serdes auto-negotiation disabled, force to 1000M link
        data=priv->read(priv,AR8327_REG_MAX_FRAME_SIZE);
        priv->write(priv,AR8327_REG_MAX_FRAME_SIZE,(data&(~0x3fff))|(0x5ee));//set untagged max frame size 1518
#endif
        return 0;
}
...
ar8327_init_globals(struct ar8216_priv *priv)
{
        ...
        ar8216_rmw(priv, AR8327_REG_MAX_FRAME_SIZE,
                   AR8327_MAX_FRAME_SIZE_MTU, 1518 );
...
ar8327_init_port(struct ar8216_priv *priv, int port)
{
		...
		/* LAN ports */
		/* 2016/02/21, LAN ports are force mode, and controlled by polling task 
...
static int
ar8216_sw_set_phy_reg_val(struct switch_dev *dev, int phy_index, struct switch_val *val)
{
        struct ar8216_priv *priv = to_ar8216(dev);
        struct mii_bus *bus = priv->phy->bus;
        unsigned int reg = 0;
        ...
        mutex_lock(&bus->mdio_lock);
        bus->write(bus, phy_index, reg, value); 
        mutex_unlock(&bus->mdio_lock);
...
static int
ar8216_sw_get_phy_reg_val(struct switch_dev *dev, int phy_index,  struct switch_val *val)
{
        struct ar8216_priv *priv = to_ar8216(dev);
        struct mii_bus *bus = priv->phy->bus;
        unsigned int reg = 0;
        ...
        mutex_lock(&bus->mdio_lock);
        ret_val =  bus->read(bus, phy_index,  reg);
        mutex_unlock(&bus->mdio_lock);

        val->len = snprintf(buf, sizeof(priv->buf), "%x", ret_val);
        val->value.s = buf;
...
ar8216_sw_hw_apply(struct switch_dev *dev)
{
	...
	if (!priv->init /*|| priv->force_recover*/) {
...
#if 0
		if((value != 0xfe) && (value != 0))
			priv->write(priv, 0x7c, 0xfe);
#else
	value = priv->read(priv, 0x7c);
...
static const struct switch_dev_ops ar8216_sw_ops = {
        ...
        .get_phy_reg_val = ar8216_sw_get_phy_reg_val,
        .set_phy_reg_val = ar8216_sw_set_phy_reg_val,
};
...
#if 0
static
int ar8216_phy_mdi_update(struct ar8216_priv *priv, u32 phy_addr, u32 *mdi, int from_status)
{
	ushort val;
	struct mii_bus *bus;
	...
	if(from_status){
		val = mdiobus_read(bus, phy_addr, AR8216_PHY_SPEC_STATUS);
		*mdi = (val & BIT(6)) >> 6;
	}

	val = mdiobus_read(bus, phy_addr, AR8216_PHY_SPEC_CONTROL);

	val = (val & ~(BITS(5,2))) | ((*mdi & 0x3) << 5);

	val = mdiobus_write(bus, phy_addr, AR8216_PHY_SPEC_CONTROL,val);
	*mdi = val;
...
int qca_qm_err_recovery(struct ar8216_priv *priv, uint32_t force_recovery);
...
ar8216_sw_mac_polling_task(struct ar8216_priv *priv)
{
	...
	if(value)
	{
		printk("qca_qm_error_check return %x\n", value);
		...
		if (link != ar8216_port_old_link[i]) {
			...
			if ((ar8216_port_old_link[i] == AR8216_PORT_LINK_UP) &&
...
#if 0 // 2015/12/08, wait for next cycle
...
#endif
				{
					...
					if (qm_buffer_err) {
						...
						mdelay(10);
						{
							...
							ar8216_phy_dbg_read(priv, i-1, 0, &value);
							//printk("%s, %d : port %d link down phy debug 0x0=0x%x\n", __FUNCTION__, __LINE__, i, value);
						}
					}
				}
				...
				if (!mac_err_flag[i]) {
					...
					priv->write(priv,0xc0, value);
					value = priv->read(priv,0xc4);
					if (value & 0xf) {
						mac_err_flag[i] = 1;
					}
					else
...
#endif
				{
					...
					if(speed == 0x01)/*PHY is link up 100M*/
					{
						...
						ar8216_phy_dbg_write(priv, i-1, 0, value);
						ar8216_phy_dbg_read(priv, i-1, 0, &value);
						//printk("%s, %d : port %d link up phy debug 0x0=0x%x\n", __FUNCTION__, __LINE__, i, value);
					}

                    //value = priv->read(priv, reg);
                    //printk("%s, %d : port %d link up 0x%x\n", __FUNCTION__, __LINE__, i,value);				
				}
			}
...
qca_qm_err_recovery(struct ar8216_priv *priv, uint32_t force_recovery)
{
	...
	ar8216_phy_linkdown(priv);
		...
		//printk("\nWJL: %s, recoverty vlan-qca_ar8327_sw_hw_apply.\n", __func__);
		...
		/*printk("\nWJL: %s, Recovery Start-%ld.%ld.\nWJL: Recovery VLAN-%ld.%ld.\nWJL: Recovery End-%ld.%ld. \n",
				__func__,
				tv_0.tv_sec, tv_0.tv_usec,
