...
static int
swconfig_set_phy_reg(struct switch_dev *dev, const struct switch_attr *attr, struct switch_val *val)
{
	if (val->port_vlan < 0)
		return -EINVAL;

	if (!dev->ops->set_phy_reg_val)
		return -EOPNOTSUPP;

	return dev->ops->set_phy_reg_val(dev, val->port_vlan, val);
}

static int
swconfig_get_phy_reg(struct switch_dev *dev, const struct switch_attr *attr, struct switch_val *val)
{
	if (val->port_vlan < 0)
		return -EINVAL;

	if (!dev->ops->get_phy_reg_val)
		return -EOPNOTSUPP;

	return dev->ops->get_phy_reg_val(dev, val->port_vlan, val);
}
...
swconfig_get_link(struct switch_dev *dev, const struct switch_attr *attr, struct switch_val *val)
{
    ...
    printk("######%s:%d: dev->buf=%s\r\n", __func__, __LINE__, dev->buf);
	val->value.s = dev->buf;
	val->len = len;
...
static struct switch_attr default_phyreg[] = {
	[PHYREG_VAL] = {
		.type = SWITCH_TYPE_STRING,
		.name = "phy_reg",
		.description = "read/write value of switch phy register(debug use only)",
		.set = swconfig_set_phy_reg,
		.get = swconfig_get_phy_reg,
	}
};
...
static void swconfig_defaults_init(struct switch_dev *dev)
{
	...
	if (ops->get_phy_reg_val || ops->set_phy_reg_val)
		set_bit(PHYREG_VAL, &dev->def_phyreg);
...
static const struct nla_policy switch_policy[SWITCH_ATTR_MAX+1] = {
	...
	[SWITCH_ATTR_OP_REG] = { .type = NLA_U32 },
	[SWITCH_ATTR_OP_PHYREG] = { .type = NLA_NUL_STRING },
...
swconfig_list_attrs(struct sk_buff *skb, struct genl_info *info)
{
	...
	case SWITCH_CMD_LIST_REG:
		...
		break;
	case SWITCH_CMD_LIST_PHYREG:
		alist = &dev->ops->attr_phyreg;
		def_list = default_phyreg;
		def_active = &dev->def_phyreg;
		n_def = ARRAY_SIZE(default_phyreg);
		break;
	default:
		WARN_ON(1);
		goto out;
		...
		struct switch_val *val)
{
	...
	case SWITCH_CMD_GET_REG:
		...
		if (val->port_vlan < 0)
			goto done;
		break;
	case SWITCH_CMD_SET_PHYREG:
	case SWITCH_CMD_GET_PHYREG:		
		alist = &dev->ops->attr_phyreg;
		def_list = default_phyreg;
		def_active = &dev->def_phyreg;
		n_def = ARRAY_SIZE(default_phyreg);
		if (!info->attrs[SWITCH_ATTR_OP_PHYREG])		
			goto done;

		val->port_vlan = nla_get_u32(info->attrs[SWITCH_ATTR_OP_PHYREG]);		
		if (val->port_vlan < 0)
			goto done;
