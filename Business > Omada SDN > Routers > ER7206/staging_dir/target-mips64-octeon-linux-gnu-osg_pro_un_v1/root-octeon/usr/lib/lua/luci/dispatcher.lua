...
function dispatch(request)
	...
	if track.sysauth then
		...
		if sdat then
            ...
            elseif ctx.urltoken.stok == sdat.token then
				...
				user = sdat.user
                        ...
                        local macaddr
                        local arptabl=sys.net.arptable()           
                        for _, entry in pairs(arptabl) do
                            if entry["IP address"] == ipaddr then
                                macaddr= string.upper(entry["HW address"]:gsub(":", "%-"))
                            end        
                        end  
		...
		if not user then
                        return error_json(err.ERR_COM_LOGIN_TIMEOUT)
		else
